...
/**
 * NOTE: must be consistent with sys_setting_opcode in vmm's vmcalls.h
 * @typedef sys_setting_opcode_t
 * @brief   enumeration containing the type of sys settings to operate
**/
enum sys_setting_opcode_t {
	OP_GET_OS_VECTORS = 0,
...
/**
 * NOTE: must be consistent with sys_func_opcode in vmm's vmcalls.h
 * @typedef sys_func_opcode_t
 * @brief   enumeration containing the type of sys func to operate
**/
enum sys_func_opcode_t {
		OP_FUNC_NMI_IPI = 0,
		OP_FUNC_MAX
};
...
/** @brief Get vectors belongs to the running VM
 *  @param vectors return vectors bitmap in array
 */
uint32_t mv_get_os_vectors(uint32_t *vectors, uint32_t num);

/** @brief Get OS interrupt guest control setting
 *  @param os_irq_guest_control return irq guest control to guest VM
 */
int32_t mv_get_os_irq_guest_control(uint32_t *os_irq_guest_control);

/** @brief vm set request to send nmi as specific function
 *  @param cpus return irq guest control to guest VM
 */
