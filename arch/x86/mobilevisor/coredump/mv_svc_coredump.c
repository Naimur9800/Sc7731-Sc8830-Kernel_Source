...
#include <coredump/mv_svc_coredump.h>
#endif

static int initialized;
void mv_svc_coredump_init(void)
{
	uint32_t current_version, min_compatible_version;
	uint32_t shared_mem_offset, shared_mem_size;

	if (!mv_svc_get_service_info(VMM_COREDUMP_SERVICE, &current_version,
		&min_compatible_version, &shared_mem_offset, &shared_mem_size))
		return;

	if (min_compatible_version > COREDUMP_API_VER) {
		mv_gal_panic("attempt to invoke a non-available	service: coredump\n");
		return;
	}
...
static void coredump_service_ensure_init(void)
{
	if (!initialized)
		mv_gal_panic("coredump service is not initialized\n");
}

void mv_svc_cd_service(uint32_t cd_opcode, void *cd_data)
{
	coredump_service_ensure_init();
	mv_platform_service(OPCODE_CD, cd_opcode, (uint32_t)(uintptr_t)cd_data,
		0, 0, 0, 0, 0, 0, 0);
}

#ifdef __KERNEL__
EXPORT_SYMBOL(mv_svc_cd_service);
#endif
