...
#include <socwatch/mv_svc_socwatch.h>
#endif

static int initialized;
void mv_svc_socwatch_init(void)
{
	uint32_t current_version, min_compatible_version;
	uint32_t shared_mem_offset, shared_mem_size;

	if (!mv_svc_get_service_info(VMM_SOCWATCH_SERVICE, &current_version,
		&min_compatible_version, &shared_mem_offset,
		&shared_mem_size))
		return;

	if (min_compatible_version > SOCWATCH_API_VER) {
		mv_gal_panic("attempt to invoke a non-available service: socwatch\n");
		return;
	}
...
static void socwatch_service_ensure_init(void)
{
	if (!initialized)
		mv_gal_panic("socwatch service is not initialized\n");
}

uint32_t mv_svc_socwatch_config(uint32_t events,
	struct socwatch_buffer_info *buffer_info)
{
	socwatch_service_ensure_init();
	mv_platform_service(OPCODE_SOCWATCH, SOCWATCH_SET_CONFIG, events,
		(uint32_t)(uintptr_t)buffer_info, 0, 0, 0, 0, 0, 0);
	return 0;
}
#ifdef __KERNEL__
...
{
	socwatch_service_ensure_init();
	mv_platform_service(OPCODE_SOCWATCH, SOCWATCH_RUN_CONTROL,
		run_control, 0, 0, 0, 0, 0, 0, 0);
	return 0;
}
#ifdef __KERNEL__
