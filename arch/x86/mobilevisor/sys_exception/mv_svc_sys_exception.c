...
#include <sys_exception/mv_svc_sys_exception.h>
#endif

static int initialized;
void mv_svc_sys_exception_init(void)
{
	uint32_t current_version, min_compatible_version;
	uint32_t shared_mem_offset, shared_mem_size;

	if (!mv_svc_get_service_info(VMM_SYS_EXCEPTION_SERVICE,
		&current_version, &min_compatible_version,
		&shared_mem_offset, &shared_mem_size))
		return;

	if (min_compatible_version > SYS_EXCEPTION_API_VER) {
		mv_gal_panic("attempt to invoke a non-available service: sys_exception\n");
		return;
	}
...
static void sys_exception_service_ensure_init(void)
{
	if (!initialized)
		mv_gal_panic("sys_exception service is not initialized\n");
}

void mv_svc_sys_exception(uint32_t opcode, void *trap_data)
{
	sys_exception_service_ensure_init();
	mv_platform_service(OPCODE_SYS_EXCEPTION, opcode,
		(uint32_t)(uintptr_t)trap_data, 0, 0, 0, 0, 0, 0, 0);
}
