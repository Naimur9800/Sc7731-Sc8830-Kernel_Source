...
#include <vtimer/mv_svc_vtimer.h>
#endif

static int initialized;
void mv_svc_vtimer_init(void)
{
	uint32_t current_version, min_compatible_version;
	uint32_t shared_mem_offset, shared_mem_size;

	if (!mv_svc_get_service_info(VMM_VTIMER_SERVICE, &current_version,
		&min_compatible_version, &shared_mem_offset, &shared_mem_size))
		return;

	if (min_compatible_version > VTIMER_API_VER) {
		mv_gal_panic("attempt to invoke a non-available service: vtimer\n");
		return;
	}
...
static void vtimer_service_ensure_init(void)
{
	if (!initialized)
		mv_gal_panic("vtimer service is not initialized\n");
}

int32_t mv_svc_vtimer_start(uint32_t num_of_vcycles)
{
	vtimer_service_ensure_init();
	return mv_platform_service(OPCODE_VTIMER_START, num_of_vcycles,
		0, 0, 0, 0, 0, 0, 0, 0);
}
#ifdef __KERNEL__
...
{
	vtimer_service_ensure_init();
	return mv_platform_service(OPCODE_VTIMER_STOP, 0,
		0, 0, 0, 0, 0, 0, 0, 0);
}
#ifdef __KERNEL__
...
int32_t mv_svc_vtimer_get_freq(void)
{
	int32_t freq;
	vtimer_service_ensure_init();
	if (mv_platform_service(OPCODE_VTIMER_GET_FREQ, 0,
		0, 0, 0, 0, 0, 0, 0, &freq) == 0)
		return freq;
	else
		return -1;
}
...
{
	vtimer_service_ensure_init();
	return mv_platform_service(OPCODE_TIMESTAMP_COUNTER_INFO,
				TIMESTAMP_COUNTER_FREQ, 0, 0, 0, 0, 0, 0, 0, 0);
}
#ifdef __KERNEL__
...
uint32_t mv_svc_timestamp_counter_size(void)
{
	vtimer_service_ensure_init();
	return mv_platform_service(OPCODE_TIMESTAMP_COUNTER_INFO,
				TIMESTAMP_COUNTER_SIZE, 0, 0, 0, 0, 0, 0, 0, 0);
}
#ifdef __KERNEL__
