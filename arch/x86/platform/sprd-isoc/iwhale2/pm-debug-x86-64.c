...
{
	int i, j;
	volatile unsigned long raw_status_temp = 0;

	for (i = 0; i < SPRD_INTC_CONTROLLER_NUM; i++) {
		raw_status_temp = (volatile unsigned long)intc_raw_status[i];

		for (j = 0; j < SPRD_HARD_INT_NUM_EACH_INTC; j++) {
			if (test_and_clear_bit(j, &raw_status_temp))
				sprd_hard_irq[i*SPRD_HARD_INT_NUM_EACH_INTC+j]++;
		}
	}
}
...
void print_hard_irq(void)
{
	...
	if (sprd_hard_irq[33])
		pr_info("wake up by gsp_mmu\n");
	if (sprd_hard_irq[34])
		pr_info("wake up by vsp_mmu\n");
	if (sprd_hard_irq[37])
	...
	if (sprd_hard_irq[70])
		pr_info("wake up by cpp_mmu_pf\n");
	if (sprd_hard_irq[71])
		pr_info("wake up by isp_ch0\n");
	if (sprd_hard_irq[96])
		pr_info("wake up by sec_gpio\n");
	if (sprd_hard_irq[100])
		pr_info("wake up by agcp_wdg_rst\n");
	if (sprd_hard_irq[108])
		pr_info("wake up by statalarm\n");
...
void deepsleep_condition_check(void)
{
	...
	AP_AHB_BITS_CLEAR_CHECK(REG_AP_AHB_AHB_EB, BIT_AP_AHB_CE0_EB,
				"ce0_eb");
	AP_AHB_BITS_CLEAR_CHECK(REG_AP_AHB_AHB_EB, BIT_AP_AHB_UFS_EB,
				"ufs_eb");
	...
	AP_AHB_BITS_CLEAR_CHECK(REG_AP_AHB_M9_LPC, BIT_AP_AHB_M9_LP_EB,
				"m9_eb");
	AP_AHB_BITS_CLEAR_CHECK(REG_AP_AHB_M10_LPC, BIT_AP_AHB_M10_LP_EB,
				"m10_eb");
...
static uint32_t pub0_sys_sleep_ctrl, pub1_sys_sleep_ctrl,
		ap_sys_sleep_ctrl, cm4_sys_sleep_ctrl,
		wtlcp_sys_sleep_ctrl, agcp_sys_sleep_ctrl,
		sleep_ctrl, pubcp_sys_sleep_ctrl;
...
static void read_subsys_ctrl(void)
{
	...
	regmap_read(deep_sys_aon_pwu_apb,
		...
		&pub0_sys_sleep_ctrl);
	regmap_read(deep_sys_aon_pwu_apb,
		REG_PMU_APB_PUB1_SYS_SLEEP_CTRL,
		&pub1_sys_sleep_ctrl);
	regmap_read(deep_sys_aon_pwu_apb,
	...
	regmap_read(deep_sys_aon_pwu_apb,
		REG_PMU_APB_AGCP_SYS_SLEEP_CTRL,
		&agcp_sys_sleep_ctrl);
	regmap_read(deep_sys_aon_pwu_apb,
...
static void read_subsys_pwr_cfg(void)
{
	...
	regmap_read(deep_sys_aon_pwu_apb, REG_PMU_APB_PD_AGCP_SYS_PWR_CFG,
			&pd_agcp_sys_pwr_cfg);
	...
	regmap_read(deep_sys_aon_pwu_apb, REG_PMU_APB_PD_GPU_PHANTOM_PWR_CFG0,
			&pd_gpu_phantom_pwr_cfg0);
	regmap_read(deep_sys_aon_pwu_apb, REG_PMU_APB_PD_AGCP_DSP_PWR_CFG,
			&pd_agcp_dsp_pwr_cfg);
	...
	regmap_read(deep_sys_aon_pwu_apb, REG_PMU_APB_PD_GPU_TOP_PWR_CFG1,
			&pd_gpu_top_pwr_cfg1);
	regmap_read(deep_sys_aon_pwu_apb, REG_PMU_APB_PD_GPU_TOP_PWR_CFG0,
			&pd_gpu_top_pwr_cfg0);
	regmap_read(deep_sys_aon_pwu_apb, REG_PMU_APB_PD_PUB1_SYS_PWR_CFG,
			&pd_pub1_sys_pwr_cfg);
	...
	regmap_read(deep_sys_aon_pwu_apb, REG_PMU_APB_PD_CAM_CORE_PWR_CFG,
			&pd_cam_core_pwr_cfg);
	...
	regmap_read(deep_sys_aon_apb, REG_AON_APB_PWR_CTRL, &pwr_ctrl);
}
...
static uint32_t apb_sys_slp_status0, apb_pwr_state_dbg1,
		...
		apb_pwr_state_dbg4, apb_sleep_ctrl;
...
static uint32_t com_pmu_apb, com_pmu_pub_frc_slp,
...
void show_deep_reg_status(void)
{
	...
	pr_info("##--status of power domain 'AGCP_SYS' :0x%08x\n",
		(apb_pwr_state_dbg1 & BIT_PMU_APB_PD_AGCP_SYS_STATE(31))
		>> 10);
	...
	pr_info("##--status of power domain 'AGCP_DSP' :0x%08x\n",
		(apb_pwr_state_dbg2 & BIT_PMU_APB_PD_AGCP_DSP_STATE(31))
		>> 10);
	...
	pr_info("##--status of power domain 'PUB1_SYS' :0x%08x\n",
		(apb_pwr_state_dbg3 & BIT_PMU_APB_PD_PUB1_SYS_STATE(31))
		>> 10);
	...
	pr_info("##--status of power domain 'CAM_CORE' :0x%08x\n",
		(apb_pwr_state_dbg4 & BIT_PMU_APB_PD_CAM_CORE_STATE(31))
		>> 5);
...
static void print_subsys_crtl(void)
{
	pr_info("##--REG_PMU_APB_PUB0_SYS_SLEEP_CTRL : 0x%08x\n",
						pub0_sys_sleep_ctrl);
	pr_info("##--REG_PMU_APB_PUB1_SYS_SLEEP_CTRL : 0x%08x\n",
						pub1_sys_sleep_ctrl);
	...
	pr_info("##--REG_PMU_APB_WTLCP_SYS_SLEEP_CTRL : 0x%08x\n",
						wtlcp_sys_sleep_ctrl);
	pr_info("##--REG_PMU_APB_AGCP_SYS_SLEEP_CTRL : 0x%08x\n",
						agcp_sys_sleep_ctrl);
	pr_info("##--REG_PMU_APB_SLEEP_CTRL : 0x%08x\n", sleep_ctrl);
...
void print_last_reg(void)
{
	...
	pr_info("##--REG_COM_PMU_APB_USB_PMU_CTRL : 0x%08x\n", com_pmu_apb);
	...
	pr_info("##--REG_PMU_PUB0_SYS_SLEEP_CTRL : 0x%08x\n",
						pub0_sys_sleep_ctrl);
	pr_info("##--REG_PMU_PUB1_SYS_SLEEP_CTRL : 0x%08x\n",
						pub1_sys_sleep_ctrl);
	...
	pr_info("##--REG_PMU_WTLCP_SYS_SLEEP_CTRL : 0x%08x\n",
						wtlcp_sys_sleep_ctrl);
	pr_info("##--REG_PMU_AGCP_SYS_SLEEP_CTRL : 0x%08x\n",
						agcp_sys_sleep_ctrl);
	pr_info("##--REG_PMU_SLEEP_CTRL : 0x%08x\n", sleep_ctrl);
	...
	pr_info("##--REG_PMU_APB_PD_PUBCP_SYS_PWR_CFG : 0x%08x\n",
						pd_pubcp_sys_pwr_cfg);
	pr_info("##--REG_PMU_APB_PD_AGCP_SYS_PWR_CFG : 0x%08x\n",
						pd_agcp_sys_pwr_cfg);
	...
	pr_info("##--REG_PMU_APB_PD_GPU_PHANTOM_PWR_CFG1 : 0x%08x\n",
						pd_gpu_phantom_pwr_cfg1);
	pr_info("##--REG_PMU_APB_PD_GPU_PHANTOM_PWR_CFG0 : 0x%08x\n",
						pd_gpu_phantom_pwr_cfg0);
	pr_info("##--REG_PMU_APB_PD_AGCP_DSP_PWR_CFG : 0x%08x\n",
						pd_agcp_dsp_pwr_cfg);
	...
	pr_info("##--REG_PMU_APB_PD_GPU_TOP_PWR_CFG1 : 0x%08x\n",
						pd_gpu_top_pwr_cfg1);
	pr_info("##--REG_PMU_APB_PD_GPU_TOP_PWR_CFG0 : 0x%08x\n",
						pd_gpu_top_pwr_cfg0);
	pr_info("##--REG_PMU_APB_PD_PUB1_SYS_PWR_CFG : 0x%08x\n",
						pd_pub1_sys_pwr_cfg);
...
static void print_debug_info(void)
{
	...
	if (ahb_eb & BIT_AP_AHB_UFS_EB)
		pr_info("##-- BIT_AP_AHB_UFS_EB still set --##\n");
	...
	if (ahb_eb & BIT_AP_AHB_USB3_EB)
		pr_info("###---- BIT_AP_AHB_USB3_EB still set ----###\n");
