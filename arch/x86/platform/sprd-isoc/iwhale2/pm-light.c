...
static void light_doze_enable(void)
{
	...
	/*enable ap sys lightsleep0/1 en*/
	regmap_update_bits(sys_pwu_apb,
		REG_PMU_APB_LIGHT_SLEEP_ENABLE0,
		BIT_PMU_APB_AP_SYS_LSLP0_ENA | BIT_PMU_APB_AP_SYS_LSLP1_ENA,
		BIT_PMU_APB_AP_SYS_LSLP0_ENA | BIT_PMU_APB_AP_SYS_LSLP1_ENA);

	/*enable ap sys lightsleep0/1 wakeup en*/
	regmap_update_bits(sys_pwu_apb,
		REG_PMU_APB_AP_SYS_SLEEP_CTRL,
		BIT_PMU_APB_AP_SYS_LSLP0_WAKEUP_EN
		| BIT_PMU_APB_AP_SYS_LSLP1_WAKEUP_EN,
		BIT_PMU_APB_AP_SYS_LSLP0_WAKEUP_EN
		| BIT_PMU_APB_AP_SYS_LSLP1_WAKEUP_EN);
	...
	regmap_update_bits(sys_aon_apb,
		...
		BIT_AON_APB_AON_APB_IDLE_EN);
	regmap_update_bits(sys_aon_apb,
		REG_AON_APB_MASTER_APB_CLK_FREQ_CTRL,
		BIT_AON_APB_AON_MDAR_APB_FREQ_CTRL_EN
		| BIT_AON_APB_AON_CP_APB_FREQ_CTRL_EN,
		BIT_AON_APB_AON_MDAR_APB_FREQ_CTRL_EN
		| BIT_AON_APB_AON_CP_APB_FREQ_CTRL_EN);
...
static void change_light_mode(int targetmode)
{
	...
	struct regmap *sys_ap_ahb;
	struct regmap *sys_pwu_apb;
	struct regmap *sys_lpc_apb;
	struct regmap *sys_pub0_apb;
	struct regmap *sys_aon_com_pmu_apb;
	...
	/*0xE4290000*/
	sys_lpc_apb = syscon_regmap_lookup_by_phandle(soc_pm_np,
		"sprd,sys-aon-lpc-apb");
	if (IS_ERR(sys_lpc_apb)) {
		pr_err("%s, failed to find sys_lpc_apb\n", __func__);
		return;
	}

	/*0xc0010000*/
	sys_pub0_apb = syscon_regmap_lookup_by_phandle(soc_pm_np,
		"sprd,sys-pub0-apb");
	if (IS_ERR(sys_pub0_apb)) {
		pr_err("%s, failed to find sys_pub0_apb\n", __func__);
		return;
	}
	...
	if (targetmode == 0) {
		...
		/*clear res_reg0[1] [3] [18:17] and [24:21]*/
		regmap_update_bits(sys_aon_apb,
			REG_AON_APB_RES_REG0,
			BIT_AON_APB_RES_REG0(0x1e6000a),
			(unsigned int)(~BIT_AON_APB_RES_REG0(0x1e6000a)));

		/*set ap smart light bia m0/1 lpc cnt to default value 0x401*/
		regmap_update_bits(sys_lpc_apb,
			0x0018,
			0xeffff,
			0x401);
		regmap_update_bits(sys_lpc_apb,
		...
		/*consider all masters in ap top*/
		regmap_update_bits(sys_ap_ahb,
			REG_AP_AHB_PUB0_FRC_LSLP,
			BIT_AP_AHB_PUB0_FRC_LSLP(0x7fc),
			(unsigned int) (~BIT_AP_AHB_PUB0_FRC_LSLP(0x7fc)));
		regmap_update_bits(sys_ap_ahb,
			REG_AP_AHB_PUB1_FRC_LSLP,
			BIT_AP_AHB_PUB1_FRC_LSLP(0x7fc),
			(unsigned int) (~BIT_AP_AHB_PUB1_FRC_LSLP(0x7fc)));

		/*clear vsp/gpu/bia_pub0/1_frce_lsp to consider vsp/gpu/bia */
		regmap_update_bits(sys_aon_com_pmu_apb,
			REG_COM_PMU_APB_AON_PUB_FRC_SLP_CTRL,
			BIT(1) | BIT(2) | BIT(3) | BIT(9) | BIT(10) | BIT(11),
			(unsigned int)(~(BIT(1) | BIT(2) | BIT(3) | BIT(9) | BIT(10) | BIT(11))));
		...
		/*clear ap sys lightsleep0/1 wakeup en*/
		regmap_update_bits(sys_pwu_apb,
			REG_PMU_APB_AP_SYS_SLEEP_CTRL,
			BIT_PMU_APB_AP_SYS_LSLP0_WAKEUP_EN
			| BIT_PMU_APB_AP_SYS_LSLP1_WAKEUP_EN,
		...
		/*set ap smart light bia m0/1 lpc cnt to recommended value 0x12c1*/
		regmap_update_bits(sys_lpc_apb,
			0x0018,
			0xeffff,
			0x12c1);
