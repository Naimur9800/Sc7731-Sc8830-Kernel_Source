...
struct regmap *deep_sys_aon_common_apb;
...
static void bak_restore_ahb(int bak)
{
	...
	if (bak) {
		...
		regmap_read(deep_sys_ap_ahb, REG_AP_AHB_MISC_CKG_EN,
		...
		regmap_read(deep_sys_ap_ahb, REG_AP_AHB_PUB1_FRC_LSLP,
			&(ap_ahb_reg_saved.pub1_frc_lslp));
	} else {
		ap_ahb_reg_saved.ahb_eb |= BIT_AP_AHB_BUSMON3_EB |
									BIT_AP_AHB_BUSMON2_EB |
									BIT_AP_AHB_BUSMON1_EB |
									BIT_AP_AHB_BUSMON0_EB;
		regmap_write(deep_sys_ap_ahb, REG_AP_AHB_AHB_EB,
			ap_ahb_reg_saved.ahb_eb);
		regmap_write(deep_sys_ap_ahb, REG_AP_AHB_MISC_CKG_EN,
			ap_ahb_reg_saved.misc_ckg_en);
		regmap_write(deep_sys_ap_ahb, REG_AP_AHB_MISC_CFG,
			ap_ahb_reg_saved.misc_cfg);
		regmap_write(deep_sys_ap_ahb, REG_AP_AHB_PUB0_FRC_LSLP,
			ap_ahb_reg_saved.pub0_frc_lslp);
		regmap_write(deep_sys_ap_ahb, REG_AP_AHB_PUB1_FRC_LSLP,
			ap_ahb_reg_saved.pub1_frc_lslp);
	}
}
...
static void disable_apb_module(void)
{
	...
	regmap_write(deep_sys_ap_apb, REG_AP_APB_APB_EB, 0);

	regmap_read(deep_sys_ap_apb, REG_AP_APB_APB_EB, &stat);
	stat &= (BIT_AP_APB_UART1_EB | BIT_AP_APB_UART0_EB);
	regmap_write(deep_sys_ap_apb, REG_AP_APB_APB_EB, stat);
...
static void bak_restore_apb(int bak)
{
	...
	if (bak) {
		...
		regmap_read(deep_sys_ap_apb, REG_AP_APB_APB_MISC_CTRL,
			&(ap_apb_reg_saved.apb_misc_ctrl));
	} else {
		regmap_write(deep_sys_ap_apb, REG_AP_APB_APB_EB,
			ap_apb_reg_saved.apb_eb);
		regmap_write(deep_sys_ap_apb, REG_AP_APB_APB_MISC_CTRL,
			ap_apb_reg_saved.apb_misc_ctrl);
	}
}
...
static void configure_for_ap_deepsleep(void)
{
	...
	/* ufs bypass */
	regmap_update_bits(deep_sys_com_pmu_apb,
		REG_COM_PMU_APB_UFS_LP_PWR_GATE_REQ_BYP,
		BIT_COM_PMU_APB_UFS_LP_PWR_GATE_REQ_BYP,
		BIT_COM_PMU_APB_UFS_LP_PWR_GATE_REQ_BYP);

	/* usb by pass */
	regmap_update_bits(deep_sys_com_pmu_apb,
		REG_COM_PMU_APB_USB_PMU_CTRL,
		BIT_COM_PMU_APB_USB_PMU_ACK_BYP,
		BIT_COM_PMU_APB_USB_PMU_ACK_BYP);

	/*vmi clear cpu0 bit5*/
	rdata = readl((void *)deep_bia_vd_cpu0);
	rdata &= ~(0x1 << 5);
	writel(rdata, (void *)deep_bia_vd_cpu0);

	/*vmi clear cpu1 bit5*/
	rdata = readl((void *)deep_bia_vd_cpu1);
	rdata &= ~(0x1 << 5);
	writel(rdata, (void *)deep_bia_vd_cpu1);
}
...
static void configure_for_cpu_deepsleep(void)
{
	...
	rdata = readl((void *)(deep_bia_vd_cpu0 + VT_BASE_OFF));
	rdata |= 0x8d;
	writel(rdata, (void *)(deep_bia_vd_cpu0 + VT_BASE_OFF));
	rdata = readl((void *)(deep_bia_vd_cpu1 + VT_BASE_OFF));
	rdata |= 0x8d;
	writel(rdata, (void *)(deep_bia_vd_cpu1 + VT_BASE_OFF));
...
{
	/* PUB0/1_FRC_LSLP dap_ep*/
	regmap_update_bits(deep_sys_ap_ahb,
		REG_AP_AHB_PUB0_FRC_LSLP,
		BIT(0),
		BIT(0));
	regmap_update_bits(deep_sys_ap_ahb,
		REG_AP_AHB_PUB1_FRC_LSLP,
		BIT(0),
		BIT(0));

	/* PUB0/1_FRC_LSLP bia_lp_eb*/
	regmap_update_bits(deep_sys_ap_ahb,
		REG_AP_AHB_PUB0_FRC_LSLP,
		BIT(1),
		BIT(1));
...
static void sprd_deepsleep_init_regmap(void)
{
	...
	deep_sys_aon_common_apb = syscon_regmap_lookup_by_phandle(deepsleep_np,
							"sprd,sys-aon-common-apb");
	if (IS_ERR(deep_sys_aon_common_apb)) {
		pr_err("%s,failed find to deep_sys_aon_apb\n",
			__func__);
		is_syscon_base_vaddr_ok = 1;
	}
...
unsigned int sprd_get_iwhale2_chipid(void)
{
	unsigned int iwhale2_chipid = 0;

	regmap_read(deep_sys_aon_apb, REG_AON_APB_AON_VER_ID,
			&iwhale2_chipid);

	pr_info("current iwhale2 chipid is %d\n", iwhale2_chipid);
	return iwhale2_chipid;
}

EXPORT_SYMBOL(sprd_get_iwhale2_chipid);
