 ...
 */
static unsigned int dx_buffer_mgr_get_sgl_nents(
	...
	enum dx_sg_cpy_direct direct)
{
	...
	while (sg_index <= to_skip) {
		...
		if (current_sg == NULL) {
			DX_LOG_ERR("reached end of sg list. unexpected \n");
			BUG();
		}
		sg_index += current_sg->length;
		nents--;
...
static int
dx_buffer_mgr_dma_map_sg(struct device *dev, struct scatterlist *sg, uint32_t nents,
	...
	struct dx_drvdata *drvdata, struct ahash_request *req)
{
	...
	uint32_t total_in_len = req->nbytes + *curr_buff_cnt;
	struct buffer_array sg_data;
	struct buff_mgr_handle *buff_mgr = drvdata->buff_mgr_handle;
