...
void dump_byte_array(const char *name, const uint8_t *the_array, unsigned long size)
{
    ...
    if (ret < 0)
	{
		DX_LOG_ERR("snprintf returned %d . aborting buffer array dump\n",ret);
        return;
	}
    line_offset = ret;
...
static int init_cc_resources(struct platform_device *plat_dev)
{
	...
	DX_LOG_DEBUG("CC registers mapped from %pa to 0x%p\n", &new_drvdata->res_mem->start, cc_base);
    ...
    new_drvdata->clk_eb = of_clk_get(np, 0);
	if (IS_ERR_OR_NULL(new_drvdata->clk_eb)) {
        DX_LOG_ERR("get ccp clock failed, errno=%ld\n", PTR_ERR(new_drvdata->clk_eb));
		goto init_cc_res_err;
	} else {
        DX_LOG_DEBUG("get ccp clock success\n");
		clk_prepare_enable(new_drvdata->clk_eb);
	}
...
//#warning Not an ARM(32) platform - Using default DMA operations
   ...
   if (!new_drvdata->plat_dev->dev.coherent_dma_mask) {
       new_drvdata->plat_dev->dev.coherent_dma_mask = DMA_BIT_MASK(64);
   }

    if (!dma_set_mask(&new_drvdata->plat_dev->dev, DMA_BIT_MASK(32))) {
    	DX_LOG_DEBUG("set dma mask=32\n");
    } else if (!dma_set_mask(&new_drvdata->plat_dev->dev, DMA_BIT_MASK(64))) {
    	DX_LOG_DEBUG("set dma mask=64\n");
    } else {
    	DX_LOG_ERR("No suitable DMA available.\n");
    	return -ENOMEM;
    }
	...
	if (new_drvdata != NULL) {
		...
		if (req_mem_cc_regs != NULL) {
				if (irq_registered) {
					...
					if (!IS_ERR_OR_NULL(new_drvdata->clk_eb)) {
        				clk_disable_unprepare(new_drvdata->clk_eb);
					}
...
static void cleanup_cc_resources(struct platform_device *plat_dev)
{
    ...
    struct clk *cc_clk_eb = drvdata->clk_eb;

    clk_prepare_enable(cc_clk_eb);
    ...
    clk_disable_unprepare(cc_clk_eb);
}
