 ...
 * */
#ifndef POLL_WHILE
#define MAX_COM_TIMEOUT_VAL	5000
#endif
...
static int sprd_aes_fill_list(struct sprd_aes_dev *dd)
{
	...
	while (total) {
		...
		count = min_t(int, sg_dma_len(in_sg), total);
...
static void aes_workqueue_handler(struct work_struct *work)
{
	...
	test_and_set_bit(FLAGS_RUNNING, &dd->flags);
	...
	test_and_clear_bit(FLAGS_RUNNING, &dd->flags);
...
static void sprd_aes_done_task(unsigned long data)
{
	struct sprd_aes_dev *dd = (struct sprd_aes_dev *)data;
...
static irqreturn_t aes_irq(int irq, void *dev_id)
{
	...
	busy = test_bit(FLAGS_RUNNING, &dd->flags);
	if (!busy) {
	...
	if (value & CMD_AES_LIST_END_INT) {
		...
		//complete(&dd->op_complete);
		dev_err(dd->dev, "irq status:0x%x: aes error!",
				aes_readl(dd, SPRD_AES_INT_STATUS));
	}
...
static int sprd_aes_probe(struct platform_device *pdev)
{
	...
	tasklet_init(&dd->done_task, sprd_aes_done_task, (unsigned long)dd);
