...
static int eic_gpio_probe(struct platform_device *pdev)
{
...
#ifndef CONFIG_64BIT
	if (match_data == &d_eic_match) {
		sgc->base_addr = SPRD_EIC_BASE;
	}
	else if (match_data == &d_gpio_match) {
		sgc->base_addr = SPRD_GPIO_BASE;
	}
	else if (match_data == &a_eic_match) {
		sgc->base_addr = ANA_EIC_BASE;
	}
	else if (match_data == &a_gpio_match) {
		sgc->base_addr = ANA_GPIO_INT_BASE;
	}
	else
		return -1;
#else
	...
	dev_info(&pdev->dev, "base_addr %x, gpio base %d, ngpio %d, irq %d",
