...
static int pgt_show(struct seq_file *s, void *unused)
{
	struct sprd_iommu_dev *iommu_dev = (struct sprd_iommu_dev *)s->private;
	unsigned long i=0;
	seq_printf(s,"iommu_name:%s  pgt_base:0x%lx  pgt_size:0x%zx  iova_base:0x%lx  iova_size:0x%zx\n",
	...
	for(i=0;i<(iommu_dev->init_data->pgt_size>>2);i++)
	{
		if(i%20==0)
		{
			...
			seq_printf(s,"\n0x%lx[0x%lx]: ",iommu_dev->init_data->pgt_base+i*4,
				iommu_dev->init_data->iova_base+i*4096);
		}
		//seq_printf(s,"%8x,",*((uint32_t*)(iommu_dev->pgt+i)));
		seq_printf(s,"%x,",*(((uint32_t *)iommu_dev->init_data->pgt_base)+i));
	}
