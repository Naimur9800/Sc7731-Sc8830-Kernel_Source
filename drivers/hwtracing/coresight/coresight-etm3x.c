...
static void etm_enable_hw(void *info)
{
	...
	if ((drvdata->ctrl & ETMCR_TIMESTAMP_EN) && (drvdata->ts_base != NULL))
...
static int etm_cpu_callback(struct notifier_block *nfb, unsigned long action,
			    void *hcpu)
{
	struct etm_drvdata *pdata =
			container_of(nfb, struct etm_drvdata, nb);
...
static int etm_cpu_pm_notify(struct notifier_block *self,
				    unsigned long action, void *hcpu)
{
	struct etm_drvdata *pdata =
			container_of(self, struct etm_drvdata, pm_nb);
	int this_cpu = 0;
...
static struct notifier_block etm_cpu_pm_notifier = {
	.notifier_call = etm_cpu_pm_notify,
};
...
static void etm_init_arch_data(void *info)
{
	...
	drvdata->sync_freq = etm_readl(drvdata, ETMSYNCFR);
...
static void etm_init_default_data(struct etm_drvdata *drvdata)
{
	 ...
	 * It could set be 0x1, because trace32 tool can only decode
	 * the trace log when set as 0x1.
	 */
	static int etm3x_traceid = 0x1;
	 ...
	 * framework.
	 */
	drvdata->traceid = etm3x_traceid + drvdata->cpu;
...
static int etm_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	drvdata->nb = etm_cpu_notifier;
	drvdata->pm_nb = etm_cpu_pm_notifier;
