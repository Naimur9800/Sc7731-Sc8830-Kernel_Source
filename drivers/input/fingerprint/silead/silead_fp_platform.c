...
silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	LOG_MSG_DEBUG(INFO_LOG, "[%s] enter cmd_8 is %u\n", __func__, cmd & 0xFF);
	switch (cmd) {
	case SIFP_IOC_INIT:
	...
	case SIFP_IOC_DBG_LEVEL:
		if (arg) {
			...
			LOG_MSG_DEBUG(INFO_LOG, "debug level %d-->%d\n", debug_level, level);
...
static int silfp_release(struct inode *inode, struct file *filp)
{
	...
	mutex_lock(&device_list_lock);
	fp_dev = filp->private_data;
	LOG_MSG_DEBUG(INFO_LOG, "[%s] begin to deinit", __func__);
	silfp_resource_deinit(fp_dev);
	LOG_MSG_DEBUG(INFO_LOG, "[%s] end to deinit", __func__);
...
static int __init silfp_dev_init(void)
{
	...
	LOG_MSG_DEBUG(INFO_LOG, "SILEAD_FP Driver_oren, Version: %s.\n", FP_DEV_VERSION);
