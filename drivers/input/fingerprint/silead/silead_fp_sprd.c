...
#endif	/* #ifndef QSEE_V4*/

	fp_dev->pin.irq = pinctrl_lookup_state(fp_dev->pin.pinctrl, FINGER_PINCTRL_STATE_IRQ);
	/*fp_dev->pin.irq = pinctrl_lookup_state(fp_dev->pin.pinctrl, "test");*/
...
/*	if (atomic_read(&fp_dev->init)) {
		...
		LOG_MSG_DEBUG(DBG_LOG, "[%s] begin to silfp_resource_deinit.\n", __func__);
