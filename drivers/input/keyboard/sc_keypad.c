...
unsigned int powerkey_reset_enable = 0;
extern void sprd_pbint_7s_reset_enable(void);

extern void sprd_pbint_7s_reset_disable(void); //add by ontim
...
static void ontim_powerkey_7s_enable(unsigned char* buf);

DEV_ATTR_DECLARE(powerkey_7s_enable)
DEV_ATTR_EXEC_DEFINE_PARAM("enable",ontim_powerkey_7s_enable)
DEV_ATTR_DECLARE_END;
ONTIM_DEBUG_DECLARE_AND_INIT(powerkey_7s_enable,powerkey_7s_enable,8);
...
static void ontim_powerkey_7s_enable(unsigned char* buf)
{
    if( !strncmp(buf,"1" ,1) )
        ...
        sprd_pbint_7s_reset_disable(); //add by ontim
}
...
{				//TODO: if usign gpio(eic), need add row , cols to platform data.
	...
	if (value) {
		...
		if (0 == powerkey_reset_enable) {
			powerkey_reset_enable = 1;
			sprd_pbint_7s_reset_enable();
			printk("%s: first powerkery up, enable reset function.\n", __func__);
		}
#endif
		input_report_key(sci_kpd->input_dev, key, 0);
		input_sync(sci_kpd->input_dev);
...
static int sci_keypad_probe(struct platform_device *pdev)
{
	...
	__raw_writel(0x7f, KPD_DEBOUNCE_CNT);
...
#ifdef CONFIG_KEYBOARD_SYSDUMP_BYPASS
	powerkey_status = gpio_get_value(PB_INT);
	printk("%s: powerkery voltage %d\n", __func__, powerkey_status);
	if (powerkey_status) {
		powerkey_reset_enable = 0;
		sprd_pbint_7s_reset_disable();	//add by ontim
		printk("%s: powerkery down, disable reset function.\n", __func__);
	}
	else {
		powerkey_reset_enable = 1;
		sprd_pbint_7s_reset_enable();
		printk("%s: powerkery up, enable reset function.\n", __func__);
	}
#endif
