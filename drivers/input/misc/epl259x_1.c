...
//    CONFIG_INPUT_EPL259X_I2C=y  ----arch/arm/config  peizhi  
// HAL use  ap3212c
//dts  2??? v128
...
#if defined(CONFIG_DJT_PRJ_C177_GAL_2N1)
#define LUX_PER_COUNT		100	
#else
...
#if defined(CONFIG_DJT_PRJ_V610)
#define ELAN_INT_PIN  217    /*Interrupt pin setting*/
#elif defined (CONFIG_DJT_PRJ_C177)||defined (CONFIG_DJT_PRJ_C178) ||defined (CONFIG_DJT_PRJ_C179)  || defined (CONFIG_DJT_PRJ_C175)
#define  ELAN_INT_PIN  239 
#elif defined(CONFIG_DJT_PRJ_V151)
#define ELAN_INT_PIN 216
#else
#define ELAN_INT_PIN  239    /*Interrupt pin setting*/
#endif
...
#define EPL_DEV_NAME   		    "ap3212c_pls"
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
...
#if defined(CONFIG_DJT_PRJ_V151_ZXG)||defined(CONFIG_DJT_PRJ_C177_HLG_2P1)
	epl_sensor.als.integration_time = EPL_ALS_INTT_16;  
#elif defined(CONFIG_DJT_PRJ_V610)|| defined (CONFIG_DJT_PRJ_C177_YMT)|| defined (CONFIG_DJT_PRJ_C178_YMT)|| defined (CONFIG_DJT_PRJ_C179_YMT)
	epl_sensor.als.integration_time = EPL_ALS_INTT_8;  
#else
	epl_sensor.als.integration_time = EPL_ALS_INTT_32;                     // DT??1a?D?y¡¤?¨º¡À?? ¦Ì¡Â?¨²1a?D¨¦?¡À¡§?¦Ì
#endif
	...
	epl_sensor.als.report_type = CMC_BIT_PRE_COUNT; //CMC_BIT_RAW; //CMC_BIT_DYN_INT
...
#if defined(CONFIG_DJT_PRJ_V151_ZXG)||defined(CONFIG_DJT_PRJ_V152_YMT)||defined(CONFIG_DJT_PRJ_C177_HLG_2P1)
	epl_sensor.ps.integration_time = EPL_PS_INTT_520;
#elif defined(CONFIG_DJT_PRJ_V151_CAM)||defined(CONFIG_DJT_PRJ_V151_WJT)||defined(CONFIG_DJT_PRJ_V152_TMK)
	epl_sensor.ps.integration_time = EPL_PS_INTT_272;
#elif defined(CONFIG_DJT_PRJ_V120_TTC_2C1)||defined(CONFIG_DJT_PRJ_V120_CAM)||defined(CONFIG_DJT_PRJ_V120_TMK_221)
	epl_sensor.ps.integration_time = EPL_PS_INTT_2064;
#else
	epl_sensor.ps.integration_time = EPL_PS_INTT_80;	// 80		//  DT???¨¤¨¤??y¡¤?¨º¡À?? ¦Ì¡Â?¨²?¨¤¨¤??D¨®|¡À?¨ºy
#endif
	epl_sensor.ps.gain = EPL_GAIN_LOW;
#if PS_DYN_K
    dynk_min_ps_raw_data = 0xffff; //min raw data
    dynk_max_ir_data = 50000;      //max ch0
#if defined(CONFIG_DJT_PRJ_V152_TMK)
    dynk_low_offset = 200;         //low thd offset	far
    dynk_high_offset = 800;        //high thd offset   near
#else
