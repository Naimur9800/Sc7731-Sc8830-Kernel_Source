...
#define CONFIG_INPUT_HEADSET_AVOID_INVALID_BUTTON
#ifdef CONFIG_INPUT_HEADSET_AVOID_INVALID_BUTTON
unsigned long g_headset_irq_enable_delay = 1500;
                ...
                if(0 == button_state_last) {
                        ...
                        if (BIT_HEADSET_MIC == ht->type) {
                            if (0 == headset_button_valid(gpio_get_value(ht->platform_data->gpio_detect))) {
                                PRINT_ERR("headset_button_work_func: button is unvalid!!!\n");
                                goto out;
                            } else {
                                set_adc_to_headmic(0);
                                msleep(50);

                                adc_left_average = adc_get_average(pdata->gpio_detect, gpio_detect_value_last);

                                PRINT_INFO("headset_button_work_func: adc_mic_average = %d, adc_left_average = %d\n",
                                        adc_mic_average, adc_left_average);

                                if (adc_left_average > g_adc_left_threshold) {
                                    g_button_invalid_num++;
                                    PRINT_ERR("OHH, headset_button_work_func: button is unvalid for adc left!!!\n");
                                    goto out;
                                }
                             }
						 }
