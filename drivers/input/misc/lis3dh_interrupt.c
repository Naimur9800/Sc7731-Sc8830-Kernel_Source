...
#define ODR1250		0x90	/* 1250Hz output:actually 650hz */
...
CTRL_REG6 (25h)  0为中断 高有效,1为中断低有效
...
#define LOG_FUN()	       pr_info(LOG_TAG"%s()\n", __func__)
...
static struct workqueue_struct *sensor_workqueue;
#endif
...
static int lis3dh_acc_i2c_read(struct lis3dh_acc_data *acc, u8 *buf, int len)
{
	...
	do {
		...
		if (err != 2) {
			LOG_ERR
			    ("*exception read transfer error,tries=%d\n",
			     tries);
...
static int lis3dh_acc_hw_init(struct lis3dh_acc_data *acc)
{
	...
	LOG_INFO(" -- %s entry  :acc-> pdata ->gpio_int1=%d-- !\n",
	...
	LOG_INFO(" -- %s exit :acc-> pdata ->gpio_int1=%d-- !\n",
		 __func__, gpio_get_value(acc->pdata->gpio_int1));
...
error_firstread:
	...
	dev_warn(&acc->client->dev,
		 "Error reading WHO_AM_I: is device vailable/working?\n");
	goto error1;
...
#endif
	LOG_INFO
	    (" -- %s gpio_int1=%d  hw_initialized=%d-- !\n",
	     __func__, gpio_get_value(acc->pdata->gpio_int1),
	     acc->hw_initialized);
...
static int lis3dh_acc_device_power_on(struct lis3dh_acc_data *acc)
{
	...
	LOG_INFO
	    ("-- %s irq1_gpio_number=%d hw_initialized=%d-- !\n",
	     __func__, gpio_get_value(acc->pdata->gpio_int1),
	     acc->hw_initialized);
	buf[0] = CTRL_REG1;
	buf[1] = acc->resume_state[RES_CTRL_REG1];
	...
	if (err < 0)
		return err;
	LOG_INFO
	    ("-- %s  resume_state[RES_CTRL_REG1]=0x%02x-- !\n",
...
static irqreturn_t lis3dh_acc_isr1(int irq, void *dev)
{
...
#ifdef LIS3DH_DBG
	static int count_intr;
	static s64 interrupt_last;
	static s64 interrupt_cur;
	static int poll_interval;
	ktime_t time_mono;

	time_mono = ktime_get();
	if (poll_interval != lis3dh_data->pdata->poll_interval) {
		count_intr = 1;
		poll_interval = lis3dh_data->pdata->poll_interval;
	...
	if (interrupt_interval >
	    (lis3dh_data->pdata->poll_interval * 1000000) +
	    (lis3dh_data->pdata->poll_interval * 1000000) / 4)
		LOG_INFO
		    ("exception:count_intr=%d,interrupt_interval=%d\n",
		     count_intr++, interrupt_interval);
#endif
...
static void lis3dh_acc_irq1_work_func(struct work_struct *work)
{
	...
	LOG_INFO(" -- %s entry :acc->irq1_gpio_number=%d-- !\n",
		 __func__, gpio_get_value(acc->pdata->gpio_int1));
	...
	LOG_INFO(" -- %s exit :acc->irq1_gpio_number=%d-- !\n",
		 __func__, gpio_get_value(acc->pdata->gpio_int1));
	enable_irq(acc->irq1);
}
...
static void lis3dh_acc_update_data_func(void)
{
	int xyz[3] = { 0 };
	int err;
	struct lis3dh_acc_data *acc = lis3dh_data;
...
static int gsensor_event_handler(void *unused)
{
	...
	LOG_INFO("-- %s entry :acc->irq1_gpio_number=%d-- !\n",
		 __func__, gpio_get_value(lis3dh_data->pdata->gpio_int1));

	do {
#ifdef LIS3DH_DBG
		LOG_INFO
		    ("-- %s: gsensor_flag=%d,acc->irq1_gpio_number=%d-- !\n",
		     __func__, gsensor_flag,
		     gpio_get_value(lis3dh_data->pdata->gpio_int1));
#endif
		...
		if (interrupt_interval >
		    (lis3dh_data->pdata->poll_interval * 1000000) +
		    (lis3dh_data->pdata->poll_interval * 1000000) / 4)
			LOG_INFO("excpt:count=%d,interrupt=%d,poll=%d\n",
			     count_intr++, interrupt_interval,
			     lis3dh_data->pdata->poll_interval);
#endif
		gsensor_flag = 0;
		...
		if (interrupt_interval >
		    ...
		    (lis3dh_data->pdata->poll_interval * 1000000) / 4)
			LOG_INFO
			    ("count=%d,interrupt=%d,poll=%d,interrupt=%lld\n",
			     count_intr++, interrupt_interval,
			     lis3dh_data->pdata->poll_interval, interrupt_read);

		LOG_INFO
		    ("interrupt_read=%lld,poll=%d,flag=%d,irq1_gpio=%d\n",
		     interrupt_read, poll_interval, gsensor_flag,
		     gpio_get_value(lis3dh_data->pdata->gpio_int1));
#endif
...
int lis3dh_acc_update_odr(struct lis3dh_acc_data *acc, int poll_interval_ms)
{
	...
	LOG_INFO("%s :ODR  poll_interval_ms=%d, config[1]=0x%02x,cutoff_ms %d",
		 __func__, poll_interval_ms, config[1],
		 lis3dh_acc_odr_table[i].cutoff_ms);
...
static int lis3dh_acc_enable(struct lis3dh_acc_data *acc)
{
	...
	if (!atomic_cmpxchg(&acc->enabled, 0, 1)) {
		...
		LOG_INFO
		    ("-- %s  hw_initialized=%d  irq1=%d-- !\n",
		     __func__, acc->hw_initialized, acc->irq1);
		if (acc->hw_initialized) {
			if (acc->irq1 != 0) {
				LOG_INFO
				    ("%s after init:irq1_gpio_number=%d\n",
				     __func__,
				     gpio_get_value(acc->pdata->gpio_int1));
				enable_irq(acc->irq1);
			}
			if (acc->irq2 != 0)
				enable_irq(acc->irq2);
		 ...
		 */
#ifndef GSENSOR_INTERRUPT_MODE
		queue_delayed_work(sensor_workqueue, &acc->input_work,
...
{
	LOG_INFO(" -- %s --acc->hw_initialized=%d\n", __func__,
		 acc->hw_initialized);
...
#ifdef LIS3DH_DBG
	LOG_INFO
	    ("-- %s data_num =%lld ,poll_interval =%d****\n",
					  ...
					  unsigned long arg, int compat_mode)
{
	...
	LOG_ERR("ioctl cmd %d,compat_mode=%d\n", _IOC_NR(cmd), compat_mode);
	switch (cmd) {
	...
	case LIS3DH_ACC_IOCTL_SET_DELAY:
		...
		LOG_INFO
		    (" %s,poll_interval =%d,interval=%d,min_interval=%d\n",
		     __func__, acc->pdata->poll_interval, interval,
...
static long lis3dh_acc_misc_ioctl(struct file *file, unsigned int cmd,
				  unsigned long arg)
{
	return lis3dh_acc_misc_ioctl_handler(file, cmd, arg, 0);
}
...
static int lis3dh_acc_input_init(struct lis3dh_acc_data *acc)
{
	...
	sensor_workqueue =
	    create_singlethread_workqueue("acc_singlethread");
	if (!sensor_workqueue) {
		err = -ESRCH;
		LOG_ERR("acc: create_singlethread_workqueue\n");
		return err;
	}
#endif
...
static int get_int(char *p, u8 max_len)
{
	...
	if (max_len > 60) {
		LOG_ERR("ERR!max length of  cmd should less  60\n");
		return -1;
	}
...
static ssize_t lis3dh_val_show(struct kobject *kobj,
...
/* adjust poll interval dynamiclly,eg echo 5>val_show ,
	means poll interval is 5ms
*/
static ssize_t lis3dh_val_store(struct kobject *kobj,
				...
				size_t n)
{
	...
	lis3dh_data->pdata->poll_interval =
	    max(interval, lis3dh_data->pdata->min_interval);
	LOG_INFO
	    ("%s,poll_interval =%d,interval=%d,min_interval=%d\n",
	     __func__, lis3dh_data->pdata->poll_interval, interval,
	     lis3dh_data->pdata->min_interval);
	err =
	    lis3dh_acc_update_odr(lis3dh_data,
				  lis3dh_data->pdata->poll_interval);
	/* TODO: if update fails poll is still set */
	if (buff_temp != NULL)
		kfree(buff_temp);
...
*/
static ssize_t sensor_reg_show(struct kobject *kobj,
			       struct kobj_attribute *attr, char *buff)
{
	int err;
	u8 value = 0;
	...
	err =
	    lis3dh_acc_register_read(lis3dh_data, &value,
				     lis3dh_data->debug.diag_reg_addr_wr);
	return sprintf(buff, "Addr: 0x%x Val: 0x%x\n",
		       lis3dh_data->debug.diag_reg_addr_wr, value);
}

static ssize_t sensor_reg_store(struct kobject *kobj,
				struct kobj_attribute *attr, const char *buff,
				size_t n)
{
	char buf[32];
	...
	lis3dh_data->debug.diag_reg_addr_wr = reg_addr;
	lis3dh_data->debug.diag_reg_val_wr = reg_val;

	buf[0] = reg_addr;
	buf[1] = reg_val;

	err = lis3dh_acc_i2c_write(lis3dh_data, buf, 1);
	if (err < 0) {
		LOG_ERR("%s: %d\n", __func__, err);
		return -EIO;
	}
	/* resume_state[index]index is not reg addr,
	   so ONLY CAN config CTRL_REG1~CTRL_REG6
	 */
	lis3dh_data->resume_state[reg_addr - 0x20] = reg_val;
	LOG_INFO("%s,now n=%d\n", __func__, n);
	return n;
}
...
static int lis3dh_sysfs_init(struct lis3dh_acc_data *acc)
{
	...
	lis3dh_kobj =
	    kobject_create_and_add("lis3dh", &(acc->input_dev->dev.kobj));
	if (lis3dh_kobj == NULL) {
		ret = -ENOMEM;
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (err) {
		...
		LOG_INFO
		    (" -- %s  acc->irq1_gpio_number  status is [%d]-- !\n",
	...
	if (acc->irq1 != 0) {
		...
		err =
		    request_irq(acc->irq1, lis3dh_acc_isr1,
	...
	acc->resume_state[RES_CTRL_REG3] = CTRL_REG3_I1_DRDY1;
	acc->resume_state[RES_CTRL_REG4] = 0x00;
	acc->resume_state[RES_CTRL_REG5] = CTRL_REG5_LIR_INT1;
	acc->resume_state[RES_CTRL_REG6] = CTRL_REG6_HLACTIVE;
	...
	LOG_INFO
	    (" -- %s exit :irq1_gpio_number status is [%d]-- !\n",
	     __func__, gpio_get_value(acc->pdata->gpio_int1));
	LOG_INFO(" -- %s -- success !\n", __func__);
	return 0;
...
static int lis3dh_pm_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	pm_message_t mesg = {0};

	lis3dh_acc_suspend(client, mesg);
	return 0;
}

static int lis3dh_pm_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);

	lis3dh_acc_resume(client);
	return 0;
}

static const struct dev_pm_ops lis3dh_pm_ops = {
	.suspend = lis3dh_pm_suspend,
	.resume = lis3dh_pm_resume,
};

#else
#define	lis3dh_pm_suspend	NULL
#define lis3dh_pm_resume	NULL
#endif	/*CONFIG_PM*/
...
static int __init lis3dh_acc_init(void)
{
	...
	LOG_FUN();
	ret = i2c_add_driver(&lis3dh_acc_driver);
	if (ret < 0)
		LOG_ERR("Erro.check compatible at DT\n");
	return ret;
}
