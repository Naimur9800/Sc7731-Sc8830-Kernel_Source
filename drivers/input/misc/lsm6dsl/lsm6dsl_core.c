...
static int acc_runwork_cnt = 1;
static int acc_runtimer_cnt = 1;
static int gyro_runwork_cnt = 1;
static int gyro_runtimer_cnt = 1;
					...
					u8 reg_addr, u8 mask, u8 data, bool b_lock)
{
	...
	if (err < 0) {
		LOG_ERR("err=%d reg_addr=%02x, mask=0x%02x\n", err, reg_addr, mask);
		return err;
	}
				       ...
				       s32 *xyz, int64_t timestamp)
{
...
#if LSM6DSL_TIMESTAMP_DEBUG
	static bool acc_first_entry = true;
	static bool gyro_first_entry = true;
#endif
	if (!sdata->enabled)
		return;

#if LSM6DSL_TIMESTAMP_DEBUG
	if ((acc_first_entry == false && sdata->sindex == LSM6DSL_ACCEL) || (gyro_first_entry == false && sdata->sindex == LSM6DSL_GYRO)) {
		sdata->timestamp_delta = timestamp - sdata->timestamp_last;
		do_div(sdata->timestamp_delta,  1000);
		LOG_INFO("sindex=%d delta=%lld us\n", sdata->sindex, sdata->timestamp_delta);
		/* TODO: handle the case,
		 * modify the timestamp if timestamp_delta is beyond the CTS range
		 */
		if (sdata->timestamp_delta == 0)
			LOG_INFO("except :sindex=%d ts=%lld ns ts_last=%lld ns\n", sdata->sindex, sdata->timestamp, sdata->timestamp_last);
		sdata->timestamp_last = timestamp;
	}

	if (acc_first_entry == true && sdata->sindex == LSM6DSL_ACCEL) {
		acc_first_entry = false;
		sdata->timestamp_last =	timestamp;
		LOG_INFO("first report data :sindex=%d\n", sdata->sindex);
	}
	if (gyro_first_entry == true && sdata->sindex == LSM6DSL_GYRO) {
		gyro_first_entry = false;
		sdata->timestamp_last =	timestamp;
		LOG_INFO("first report data :sindex=%d\n", sdata->sindex);
	}
#endif
	...
	/* set timestamp in driver instead of input core */
...
#if LSM6DSL_TIMESTAMP_DEBUG
	if (sdata->sindex == LSM6DSL_ACCEL)
		LOG_INFO("++ sindex=%d entry %d -->\n", sdata->sindex, acc_runtimer_cnt);
	if (sdata->sindex == LSM6DSL_GYRO)
		LOG_INFO("++ sindex=%d entry %d -->\n", sdata->sindex, gyro_runtimer_cnt);
	sdata->entry_timer_cnt++;
#endif

	/* set timestamp asap */
	sdata->timestamp = lsm6dsl_get_time_ns();

	/* TODO: handle the case, queuing work fail will not
	 * report data this time ,check this log if CTS FIAL
	 */
	ret = queue_work(lsm6dsl_workqueue, &sdata->input_work);
	if (!ret)
...
static void poll_function_work(struct work_struct *input_work)
{
...
#if LSM6DSL_TIMESTAMP_DEBUG
	if (sdata->sindex == LSM6DSL_ACCEL)
		LOG_INFO("++ sindex=%d entry %d -->\n", sdata->sindex, acc_runwork_cnt);
	if (sdata->sindex == LSM6DSL_GYRO)
		LOG_INFO("++ sindex=%d entry %d -->\n", sdata->sindex, gyro_runwork_cnt);
	LOG_INFO("sindex=%d entry_timer_cnt=%d -->\n", sdata->sindex, sdata->entry_timer_cnt);
#endif

	/* time from timer to here */
	delta = ktime_set(0, lsm6dsl_get_time_ns() - sdata->timestamp);

	if (ktime_sub(sdata->ktime, delta).tv64 <= 0) {
		ts_delta = ktime_to_ns(delta) - ktime_to_ns(sdata->ktime);
		do_div(ts_delta, 1000);
		LOG_INFO("ktime=%lld,delta=%lld, ts_delta=%lld us\n", ktime_to_ns(sdata->ktime), ktime_to_ns(delta), ts_delta);
#if 0
		/* kick start the timer immediately to avoid delays */
		hrtimer_start(&sdata->hr_timer, ktime_set(0, 0),
				HRTIMER_MODE_REL);
#endif
	} else {
#if 0
	...
	if (sdata->sample_to_discard) {
		...
		if (sdata->sindex == LSM6DSL_ACCEL) {
			LOG_INFO("acc discard :exit --%d-->\n", acc_runwork_cnt);
			acc_runwork_cnt++;
		}
		if (sdata->sindex == LSM6DSL_GYRO) {
			LOG_INFO("gyro discard :exit --%d-->\n", gyro_runwork_cnt);
			gyro_runwork_cnt++;
		}
		return;
	}
...
static void lsm6dsl_irq_management(struct work_struct *input_work)
{
	...
	LOG_INFO("src_value=0x%x src2_value=0x%x\n", src_value, src2_value);
...
static int lsm6dsl_allocate_workqueue(struct lsm6dsl_data *cdata)
{
	...
	if (!lsm6dsl_workqueue)
		/* don't use create_workqueue(cdata->name);
		 * commented by bao @20170713
		 */
		lsm6dsl_workqueue = alloc_workqueue("%s", WQ_HIGHPRI | WQ_UNBOUND |
					  WQ_MEM_RECLAIM, 2, "lsm6dsl_wq");
	if (!lsm6dsl_workqueue)
		return -EINVAL;
	if (cdata->irq > 0) {
...
static int _lsm6dsl_enable_sensors(struct lsm6dsl_sensor_data *sdata)
{
	...
	case LSM6DSL_GYRO:
		...
		LOG_INFO("sindex=%d c_odr=%dHz poll_interval=%dms\n", sdata->sindex, sdata->c_odr, sdata->poll_interval);
...
static int lsm6dsl_enable_sensors(struct lsm6dsl_sensor_data *sdata)
{
	...
	LOG_INFO("sindex=%d, enabled=%d\n", sdata->sindex, sdata->enabled);
	if (sdata->enabled)
		return 0;
...
static int _lsm6dsl_disable_sensors(struct lsm6dsl_sensor_data *sdata)
{
	...
	case LSM6DSL_ACCEL:
		...
		LOG_INFO("sindex=%d cancel timer\n", sdata->sindex);
		...
		const char *buf, size_t count)
{
	...
	len = sscanf(buf, "%3d %3d", &address, &value);
#if 0
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (reg_addr = 0; reg_addr < 0x75; reg_addr++) {
		err = sdata->cdata->tf->read(sdata->cdata, reg_addr, 1, reg + reg_addr, true);
...
#endif
static DEVICE_ATTR(enable, 0664,  get_enable, set_enable);
static DEVICE_ATTR(sampling_freq, 0664, get_sampling_freq,
		   set_sampling_freq);
static DEVICE_ATTR(polling_rate, 0664, get_polling_rate,
		   set_polling_rate);
static DEVICE_ATTR(reset_steps, S_IWUSR, NULL, reset_steps);
static DEVICE_ATTR(max_delivery_rate, S_IWUSR | S_IRUGO, get_max_delivery_rate,
		   set_max_delivery_rate);
...
static DEVICE_ATTR(scale_avail, S_IRUGO, get_scale_avail, NULL);
static DEVICE_ATTR(scale, S_IWUSR | S_IRUGO, get_cur_scale, set_cur_scale);
static DEVICE_ATTR(regs, 0664, registers_show,
		   register_store);
...
int lsm6dsl_common_probe(struct lsm6dsl_data *cdata, int irq, u16 bustype)
{
	...
	gsensor_class = class_create(THIS_MODULE, "sprd_sensor");
	if (IS_ERR(gsensor_class))
		LOG_INFO("Failed to create class(xr-gsensor)!\n");

	sensor_dev = platform_device_register_simple("sprd_sensor", -1, NULL, 0);
	...
	for (i = 0; i < LSM6DSL_SENSORS_NUMB; i++) {
		...
		if (sysfs_create_group(&sensor_dev->dev.kobj,
				       &lsm6dsl_attribute_groups[i])) {
			LOG_ERR("create sysfs fail!\n");
		}
