...
struct lsm6dsl_sensor_data {
	...
	int64_t timestamp_last; /* ns  for debug */
	int64_t timestamp_delta; /* us for debug */
	u32 entry_timer_cnt; /* for debug */
	u8 enabled;
	u32 c_odr;
	u32 c_gain;
	u8 sindex;
	...
	ktime_t ktime;                  /* poll_interval -> ktime */
};
...
#define LSM6DSL_TIMESTAMP_DEBUG                 0
#define LOG_TAG				        "[lsm6dsl]"
#define LOG_FUN()		        printk(KERN_INFO LOG_TAG"%s\n", __func__)
#define LOG_INFO(fmt, args...)		printk(KERN_INFO  LOG_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
#define LOG_ERR(fmt, args...)		printk(KERN_ERR  LOG_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
