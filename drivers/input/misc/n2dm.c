...
static ssize_t n2dm_chip_info_show(struct device *dev,
									 struct device_attribute *attr, char *buf)
{
	return snprintf(buf, 25, "%s", "n2dm");
}

static DEVICE_ATTR(enable, S_IRUGO | S_IWUSR | S_IWGRP, show_n2dm_enable,
		   store_n2dm_enable);
static DEVICE_ATTR(delay, S_IRUGO | S_IWUSR | S_IWGRP, show_n2dm_delay,
		   store_n2dm_delay);
static DEVICE_ATTR(chip_info, 0440, n2dm_chip_info_show, NULL);

static struct attribute *n2dm_attributes[] = {
...
static struct attribute_group n2dm_attribute_group = {
	.attrs = n2dm_attributes,
};
		...
		const struct i2c_device_id *id)
{
	...
	err = sysfs_create_group(&acc->input_dev->dev.kobj, &n2dm_attribute_group);
	if (err < 0) {
		dev_err(&acc->client->dev, "%s: sysfs_create_group failed\n",
			__func__);
