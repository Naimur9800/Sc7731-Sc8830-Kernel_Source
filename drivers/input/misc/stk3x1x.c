...
{
	file->private_data = stk3x1x_i2c_data;
...
{
	struct i2c_client *client = stk3x1x_i2c_data->client;
	...
	return snprintf(buf, PAGE_SIZE, "stk3311\n");
}
                        ...
                        const struct i2c_device_id *id)
{
    ...
    if((err = misc_register(&stk_device)))
	{
		printk(KERN_ERR "stk_device register failed\n");
		goto exit_misc_device_register_failed;
	}
    if((device_create_file(stk_device.this_device, &dev_attr_chipinfo)) < 0)
	{
...
err_stk3x1x_setup_irq:
    ...
    device_remove_file(&stk_device.this_device, &dev_attr_chipinfo);
