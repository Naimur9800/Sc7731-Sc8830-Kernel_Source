...
#if GTP_VIRTUAL_KEYS
static ssize_t
virtual_keys_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
#if defined(CONFIG_DJT_PRJ_V129) || defined(CONFIG_DJT_PRJ_V130) || defined(CONFIG_LCMSIZE_HD)
	return sprintf(buf, __stringify(EV_KEY) ":" __stringify(KEY_MENU)
		       ":100:1350:80:65" ":" __stringify(EV_KEY) ":"
		       __stringify(KEY_HOMEPAGE) ":280:1350:80:65" ":"
		       __stringify(EV_KEY) ":" __stringify(KEY_BACK)
		       ":470:1350:80:65" "\n");
#else
	return sprintf(buf, __stringify(EV_KEY) ":" __stringify(KEY_MENU)
		       ":100:1020:80:65" ":" __stringify(EV_KEY) ":"
		       __stringify(KEY_BACK) ":280:1020:80:65" ":"
...
static void gt9xx_ts_virtual_keys_init(void)
{
	int ret = 0;
...
static void gt1x_parse_dt(struct device *dev)
{
	...
	ret = of_property_read_u32(np, "goodix_value", &goodix_value_9c10);
	if (ret) {
			printk("fail to get goodix_value\n");
	} else {
			printk("goodix_value_9c10 = %d\n", goodix_value_9c10);
	}
...
int gt1x_power_switch(int on)
{
	...
	if (!vdd_ana) {
		...
		vdd_ana = regulator_get(NULL, "vdd28");
		if (IS_ERR(vdd_ana)) {
		...
		regulator_set_voltage(vdd_ana, 2800000, 2800000);
	}

	if (!vcc_i2c) {
		/* vcc_i2c = regulator_get(&client->dev, "vcc_i2c"); */
		vcc_i2c = regulator_get(NULL, "vdd18");
		if (IS_ERR(vcc_i2c)) {
	...
	if (!vcc_i2c) {
		vcc_i2c = regulator_get(NULL, "vdd18");
		if (IS_ERR(vcc_i2c)) {
			GTP_ERROR("regulator get of vcc_i2c2222 failed");
			ret = PTR_ERR(vcc_i2c);
			vcc_i2c = NULL;
			goto ERR_GET_VCC;
		}
...
#ifdef TP_READ_VER
static ssize_t
tpd_info_read(struct file *f, char __user *buf, size_t count, loff_t *ppos)
{
	GTP_INFO("***FW Version tpd_info_read = 0x%x ***\n", tp_version_code);
 ...
 */
static int
gt1x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	s32 ret = -1;
#if GTP_AUTO_UPDATE
	...
	if (misc_register(&tpd_info) < 0)
		GTP_ERROR("gt9xx_driver: tpd_info_device register failed\n");
#endif
	gt1x_register_powermanger();
	printk(">>>>> gt9xx probe OK!\n");
	return 0;
}
...
#if 1				/* notifier block control the suspend/resume */
...
static int
gtp_adf_notifier_callback(struct notifier_block *noti, unsigned long event,
			  void *data)
{
...
#else /* bus control the suspend/resume */
/**
...
#endif /* notifier block control the suspend/resume */
...
static int gt1x_register_powermanger(void)
{
	...
	gt1x_adf_notifier.priority = 1000;
...
/* static SIMPLE_DEV_PM_OPS(gt1x_pm_ops,gt1x_suspend, gt1x_resume); */
