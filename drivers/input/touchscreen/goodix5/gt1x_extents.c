...
static u16 gGestureWakeupValue = 1;
...
static ssize_t
gt1x_gesture_data_read(struct file *file, char __user *page, size_t size,
		       loff_t *ppos)
{
	return 0;
}
...
static ssize_t
gt1x_gesture_data_write(struct file *filp, const char __user *buff, size_t len,
			loff_t *off)
{
	   return 0;
}
...
{
	printk("--qrp add the gesture_event_handler\n");
	...
	printk("--qrp add the  gesture_doze_status is = %d\n",
	       gesture_doze_status);
	...
	printk("--jiang add the doze_buf[0] is = %d\n", doze_buf[0]);
	...
	if (extra_len > 0) {
		...
		if (likely(need_chk)) {	/* calc checksum */
			...
			val = calc_checksum(ges_data, extra_len + 1, 8);
	...
	if (ges_type == 0 || !QUERYBIT(gestures_flag, ges_type)) {
		...
		if ((doze_buf[0] == 'a') || (doze_buf[0] == 'b')
		    || (doze_buf[0] == 'c') || (doze_buf[0] == 'd')
		    || (doze_buf[0] == 'e') || (doze_buf[0] == 'g')
		    || (doze_buf[0] == 'h') || (doze_buf[0] == 'm')
		    || (doze_buf[0] == 'o') || (doze_buf[0] == 'q')
		    || (doze_buf[0] == 's') || (doze_buf[0] == 'v')
		    || (doze_buf[0] == 'w') || (doze_buf[0] == 'y')
		    || (doze_buf[0] == 'z')) {
...
static long gt1x_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GESTURE_ENABLE_TOTALLY:
		...
		gesture_enabled =
		    (is_all_dead(gestures_flag, sizeof(gestures_flag)) ? 0 : 1);
		break;
	...
	case HOTKNOT_VENDOR_VERSION:
		ret =
		    copy_to_user(((u8 __user *) arg), HOTKNOT_VERSION,
...
static long
gt1x_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
#endif
/* zgq */
struct device *gt9x_firmware_cmd_dev;
struct class *gt9x_firmware_class;
#if GTP_GESTURE_WAKEUP
static ssize_t
gt9x_gesture_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", gesture_enabled);
}

static ssize_t
gt9x_gesture_store(struct device *dev,
		   struct device_attribute *attr, const char *buf, size_t count)
{
	/* unsigned int enable = simple_strtoul(buf, NULL, 10); */
	unsigned long enable = 0;
	kstrtoul(buf, 0, &enable);
...
ssize_t
gt9x_gesturevalue_show(struct device *pDevice, struct device_attribute *pAttr,
		       char *pBuf)
{
	u16 value = gGestureWakeupValue;
	gGestureWakeupValue = 0;
	return sprintf(pBuf, "%x", value);
}

ssize_t
gt9x_gesturevalue_store(struct device *pDevice,
			struct device_attribute *pAttr, const char *pBuf,
			size_t nSize)
{
	u32 nWakeupMode;
	if (pBuf != NULL) {
		sscanf(pBuf, "%x", &gGestureWakeupValue);
	}
}
/*
 * static DEVICE_ATTR(gesture_wakeup_value, 0777, gt9x_gesturevalue_show,
 *		   gt9x_gesturevalue_store);
*/

/* zgq */
/*
 * static DEVICE_ATTR(gesture_wakeup_mode, 0666, gt9x_gesture_show,
 *		   gt9x_gesture_store);
*/
static DEVICE_ATTR(gesture_wakeup_value, 0644, gt9x_gesturevalue_show,
		   gt9x_gesturevalue_store);
/* zgq */
static DEVICE_ATTR(gesture_wakeup_mode, 0644, gt9x_gesture_show,
		   gt9x_gesture_store);
...
/*	proc_entry = proc_create(GESTURE_NODE, 0666, NULL, &gt1x_fops); */
	...
	gt9x_firmware_class = class_create(THIS_MODULE, "touchscreen");
	if (IS_ERR(gt9x_firmware_class)) {
		pr_err("Failed to create class(firmware)!\n");
	}

	gt9x_firmware_cmd_dev =
	    device_create(gt9x_firmware_class, NULL, 0, NULL, "device");
	if (IS_ERR(gt9x_firmware_cmd_dev)) {
		pr_err("Failed to create device(firmware_cmd_dev)!\n");
	}
	if (device_create_file(gt9x_firmware_cmd_dev, &dev_attr_gesture_wakeup_mode) < 0)	/* &client->dev */
		pr_err("Failed to create device file(%s)!\n",
		       dev_attr_gesture_wakeup_mode.attr.name);

	if (device_create_file
	    (gt9x_firmware_cmd_dev, &dev_attr_gesture_wakeup_value) < 0)
		pr_err("Failed to create device file(%s)!\n",
		       dev_attr_gesture_wakeup_value.attr.name);

	dev_set_drvdata(gt9x_firmware_cmd_dev, NULL);
#endif
