 ...
 *	    proc_create(GT1X_DEBUG_PROC_FILE, 0666, NULL, &gt1x_debug_fops);
*/
	gt1x_debug_proc_entry =
	    proc_create(GT1X_DEBUG_PROC_FILE, 0644, NULL, &gt1x_debug_fops);
...
static ssize_t
gt1x_debug_read_proc(struct file *file, char __user *page, size_t size,
		     loff_t *ppos)
{
	char *ptr = page;
...
static ssize_t
gt1x_debug_write_proc(struct file *file, const char __user  *buffer, size_t count,
		      loff_t *ppos)
{
	s32 ret = 0;
...
s32 gt1x_init_panel(void)
{
	...
	GTP_INFO
	    ("++++++++goodix++CTP_CONFIG_GROUP%d used, gt1x_config length: %d++++++goodix++",
	     sensor_id, cfg_len);
	...
	GTP_INFO("Res: %d *%d, trigger: %d", GTP_MAX_WIDTH, GTP_MAX_HEIGHT,
		 GTP_INT_TRIGGER);
...
s32 gt1x_read_version(struct gt1x_version_info *ver_info)
{
	...
	while (retry--) {
		...
		if (!ret) {
			...
			if (checksum == 0 &&	/* first 3 bytes must be number or char */
				...
				GTP_ERROR
				    ("GTP read version failed!(checksum error)");
			}
		} else {
	...
	if (ver_info != NULL) {
		...
		if (1 == goodix_value_9c10) {
			ver_info->sensor_id = 1;
			printk("111****** goodix_value_9c10 = %d, ver_info->sensor_id = %d\n", goodix_value_9c10, ver_info->sensor_id);
		} else {
			ver_info->sensor_id = sensor_id;
			printk("222****** goodix_value_9c10 = %d, ver_info->sensor_id = %d\n", goodix_value_9c10, ver_info->sensor_id);
		}
 ...
 */
s32
gt1x_touch_event_handler(u8 *data, struct input_dev *dev,
...
#if defined(CONFIG_DJT_PRJ_V129) || defined(CONFIG_LCMSIZE_HD)
		switch (key_value) {
		case 0x01:
			gt1x_pen_down(100, 1350, 1, 1);
			break;
		case 0x02:
			gt1x_pen_down(280, 1350, 1, 1);
			break;
		case 0x04:
			gt1x_pen_down(470, 1350, 1, 1);
			break;
		}
#else
		switch (key_value) {
		case 0x01:
			/* gt1x_pen_down(100,1020,1, 1); */
			break;
		case 0x02:
			gt1x_pen_down(280, 1020, 1, 1);
			break;
		case 0x04:
			gt1x_pen_down(470, 1020, 1, 1);
			break;
		}
#endif
		touch_num = 0;
...
static s32
gt1x_ps_operate(void *self, u32 command, void *buff_in, s32 size_in,
...
#ifndef PLATFORM_MTK
static ssize_t
gt1x_ps_enable_show(struct kobject *kobj, struct kobj_attribute *attr,
		    char *buf)
{
...
static ssize_t
gt1x_ps_enable_store(struct kobject *kobj, struct kobj_attribute *attr,
...
static ssize_t
gt1x_ps_state_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d", gt1x_ps_dev->state);
}

static ssize_t
gt1x_ps_state_store(struct kobject *kobj, struct kobj_attribute *attr,
		    const char *buf, size_t count)
{
	unsigned int input;
	if (sscanf(buf, "%u", &input) != 1) {
...
static ssize_t
smart_cover_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct smart_cover_device *dev = gt1x_sc_dev;
...
static ssize_t
smart_cover_store(struct kobject *kobj, struct kobj_attribute *attr,
		  const char *buf, size_t count)
{
	struct smart_cover_device *dev = gt1x_sc_dev;
	int s = (buf[0] - '0');
...
int gt1x_suspend(void)
{
	...
	printk("---qrp add the gesture_enabled is = %d\n", gesture_enabled);
	if (gesture_enabled) {
		gesture_enter_doze();
		/* irq_set_irq_type(gt1x_i2c_client->irq,IRQF_NO_SUSPEND); */
		gt1x_irq_enable();
		gt1x_halt = 0;
	} else
#endif
