...
static ssize_t gt1x_tool_write(struct file *filp, const char __user  *buffer,
			       size_t count, loff_t *ppos);
...
int gt1x_init_tool_node(void)
{
	...
	gt1x_tool_proc_entry =
	 /*   proc_create(procname, 0666, NULL, &gt1x_tool_fops); */
	    proc_create(procname, 0644, NULL, &gt1x_tool_fops);
...
********************************************************/
static ssize_t
gt1x_tool_write(struct file *filp, const char __user *buff, size_t len,
		loff_t *data)
{
	u64 ret = 0;
	GTP_DEBUG_FUNC();
...
********************************************************/
static ssize_t
gt1x_tool_read(struct file *filp, char __user *buffer, size_t count,
	       loff_t *ppos)
{
	GTP_DEBUG_FUNC();
	if (*ppos) {
