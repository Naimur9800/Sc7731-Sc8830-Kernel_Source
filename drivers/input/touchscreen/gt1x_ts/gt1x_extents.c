...
{
	return sprintf(buf, "GT1X Gesture feature is %s\n", (gesture_enabled ? "enable" : "disable"));
}
static ssize_t gestureswitch_set(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	ret = kstrtoint(buf, 10, &gesture_enabled);
	if (ret)
		return ret;
	return count;
}
...
s32 gesture_event_handler(struct input_dev *dev)
{
	...
	static int err_flag1 = 0, err_flag2;
	...
	input_sync(dev);
#endif
	GTP_DEBUG("Gesture: 0x%02X, points: %d, key_code: 0x%x", doze_buf[0], doze_buf[1], ges_type);
	switch (gesture_data.data[0]) {
	case 'c':
		input_report_key(dev, KEY_F13, 1);
		input_sync(dev);
		input_report_key(dev, KEY_F13, 0);
		input_sync(dev);
			break;
	case 'w':
		input_report_key(dev, KEY_F15, 1);
		input_sync(dev);
		input_report_key(dev, KEY_F15, 0);
		input_sync(dev);
			break;
	case 'm':
		input_report_key(dev, KEY_F18, 1);
		input_sync(dev);
		input_report_key(dev, KEY_F18, 0);
		input_sync(dev);
			break;
	case 'o':
		input_report_key(dev, KEY_F14, 1);
		input_sync(dev);
		input_report_key(dev, KEY_F14, 0);
		input_sync(dev);
			break;
	case 'e':
		input_report_key(dev, KEY_F16, 1);
		input_sync(dev);
		input_report_key(dev, KEY_F16, 0);
		input_sync(dev);
			break;
	case 'z':
		...
		input_sync(dev);
			break;
	case 'v':
		input_report_key(dev, KEY_F19, 1);
		input_sync(dev);
		input_report_key(dev, KEY_F19, 0);
		input_sync(dev);
			break;
	case 0xCC:
		input_report_key(dev, KEY_GESTURE_DC, 1);
		input_sync(dev);
		input_report_key(dev, KEY_GESTURE_DC, 0);
		input_sync(dev);
			break;
	default:
			break;
...
s32 gt1x_init_node(void)
{
	...
	device_create_file(&gt1x_i2c_client->dev, &dev_attr_gestureswitch);
#endif
