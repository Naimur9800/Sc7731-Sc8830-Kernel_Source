...
/* static int g_vendorid; */


static ssize_t tp_version_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "gt1x::chipid(0x%x)::fwversion(0x%x)\n",
...
static ssize_t gt1x_debug_read_proc(struct file *file, char *page, size_t size, loff_t *ppos);
