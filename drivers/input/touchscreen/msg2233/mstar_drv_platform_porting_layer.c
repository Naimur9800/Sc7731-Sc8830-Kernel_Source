...
extern struct wake_lock mstar_irq_lock;
...
const int g_TpVirtualKey[] = {TOUCH_KEY_HOME, TOUCH_KEY_MENU, TOUCH_KEY_BACK, TOUCH_KEY_SEARCH};
...
static irqreturn_t _DrvPlatformLyrFingerTouchInterruptHandler(s32 nIrq, void *pDeviceId)
{
	...
	//wake_lock_timeout(&mstar_irq_lock,msecs_to_jiffies(20000));
...
{
    DBG("*** %s() MS_TS_MSG_IC_GPIO_RST=%d***\n", __func__,MS_TS_MSG_IC_GPIO_RST);
...
void DrvPlatformLyrDisableFingerTouchReport(void)
{
...
#endif //CONFIG_ENABLE_HOTKNOT
    {
        if (_gInterruptFlag == 1)  
        {
        	printk("DrvPlatformLyrDisableFingerTouchReport___gIrq=%d\n",_gIrq);
            disable_irq(_gIrq);
