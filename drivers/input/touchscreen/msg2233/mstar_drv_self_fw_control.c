...
#include "msg22xx_zzzz_update_bin.h"
...
//                    input_report_key(g_InputDevice, RESERVER51, 0);
        ...
        { 
        	/*DBG("gesture wakeup fail1111111111111.\n");
        			input_report_key(g_InputDevice, KEY_POWER, 1);
                    input_sync(g_InputDevice);
                    input_report_key(g_InputDevice, KEY_POWER, 0);
                    input_sync(g_InputDevice);*/
...
void _DrvFwCtrlMsg22xxCheckFirmwareUpdateBySwId(void)
{
    ...
    if (nCrcMainA == nCrcMainB && nCrcInfoA == nCrcInfoB) // Case 1. Main Block:OK, Info Block:OK
    {
        ...
        else if(eSwId == MSG22XX_SW_ID_ZZZZ) {
	        nUpdateBinMajor = msg22xx_zzzz_update_bin[0xBFF5]<<8 | msg22xx_zzzz_update_bin[0xBFF4];
            nUpdateBinMinor = msg22xx_zzzz_update_bin[0xBFF7]<<8 | msg22xx_zzzz_update_bin[0xBFF6];
        }
        ...
        if (nUpdateBinMinor > nMinor)
        {
			...
			else if (eSwId == MSG22XX_SW_ID_ZZZZ)
            {
                for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE+1); i ++)
                {
                    if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) // i < 48
                    {
                        _DrvFwCtrlStoreFirmwareData(&(msg22xx_zzzz_update_bin[i*1024]), 1024);
                    }
                    else // i == 48
                    {
                        _DrvFwCtrlStoreFirmwareData(&(msg22xx_zzzz_update_bin[i*1024]), 512);
                    }
                }
            }
            else
