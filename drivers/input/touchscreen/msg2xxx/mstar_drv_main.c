...
extern u16 _gGestureWakeupValue;
...
#if 1//defined(CONFIG_CTP_GESTURE_SUPPORT)
static struct class *_gWakeClass = NULL;
static struct device *_gwakeCmdDev = NULL;
#endif
/*=============================================================*/
...
//zgq
ssize_t DrvMainFirmwareTpVersionShow(struct device *pDevice, struct device_attribute *pAttr, char *pBuf)
{
    u16 nMajor = 0, nMinor = 0;
    DrvIcFwLyrGetCustomerFirmwareVersion(&nMajor, &nMinor, &_gFwVersion);
    DBG("*** %s() _gFwVersion = %s ***\n", __func__, _gFwVersion);
    return sprintf(pBuf, "msg2xxx , firmware id : %s\n", _gFwVersion);
}

ssize_t DrvMainFirmwareTpVersionStore(struct device *pDevice, struct device_attribute *pAttr, const char *pBuf, size_t nSize)
{
    DBG("*** %s() _gFwVersion = %s ***\n", __func__, _gFwVersion);
    return nSize;
}

static DEVICE_ATTR(tp_version, SYSFS_AUTHORITY, DrvMainFirmwareTpVersionShow, DrvMainFirmwareTpVersionStore);
//zgq
...
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
//zgq
ssize_t DrvMainGestureWakeupValueShow(struct device *pDevice, struct device_attribute *pAttr, char *pBuf)
{
    u16 value = _gGestureWakeupValue;
    DBG("*** %s() ***\n", __func__);
...
ssize_t DrvMainGestureWakeupValueStore(struct device *pDevice, struct device_attribute *pAttr, const char *pBuf, size_t nSize)
{
    ...
    if (pBuf != NULL)
    {
        sscanf(pBuf, "%x", &_gGestureWakeupValue);   
    }
}

static DEVICE_ATTR(gesture_wakeup_value, SYSFS_AUTHORITY, DrvMainGestureWakeupValueShow, DrvMainGestureWakeupValueStore);
//zgq
...
//        DBG("Failed to create device file(%s)!\n", dev_attr_packet.attr.name);
   ...
   //zgq	
    if (device_create_file(_gFirmwareCmdDev, &dev_attr_gesture_wakeup_value) < 0)
...
#if 1//defined(CONFIG_CTP_GESTURE_SUPPORT) //zgq
    /* set sysfs for firmware */
    _gWakeClass = class_create(THIS_MODULE, "touchscreen");
    if (IS_ERR(_gWakeClass))
        DBG("Failed to create class(Wake)!\n");

    _gwakeCmdDev = device_create(_gWakeClass, NULL, 0, NULL, "device");
    if (IS_ERR(_gwakeCmdDev))
        DBG("Failed to create device(_gwakeCmdDev)!\n");
//zgq
    if (device_create_file(_gwakeCmdDev, &dev_attr_tp_version) < 0)
        DBG("Failed to create device file(%s)!\n", dev_attr_tp_version.attr.name);
//zgq	
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
    // gesture wakeup mode
    if (device_create_file(_gwakeCmdDev, &dev_attr_gesture_wakeup_mode) < 0)
        DBG("Failed to create device file(%s)!\n", dev_attr_gesture_wakeup_mode.attr.name);
   //zgq	
    if (device_create_file(_gwakeCmdDev, &dev_attr_gesture_wakeup_value) < 0)
        DBG("Failed to create device file(%s)!\n", dev_attr_gesture_wakeup_value.attr.name);
   //zgq
#endif //CONFIG_ENABLE_GESTURE_WAKEUP

	dev_set_drvdata(_gwakeCmdDev, NULL);
