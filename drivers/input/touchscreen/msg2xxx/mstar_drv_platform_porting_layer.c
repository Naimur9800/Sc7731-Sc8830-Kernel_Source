...
#if TS_PS_DEBUG_MSG
#define MSG2XXX_PS_DBG(format, ...)	printk(KERN_INFO "MSG2XXX " format "\n", ## __VA_ARGS__)
#else
#define MSG2XXX_PS_DBG(format, ...)
#endif
#ifdef TPD_PROXIMITY
#define MSG2XXX_PS_DEVICE 		"ap3212c_pls"
#define MSG2XXX_PS_INPUT_DEV	"proximity"
#endif

static int PROXIMITY_STATE=0;
int msg_proximity_flag = 0;
static int msg2xxx_ps_opened=0;
...
#if defined(CONFIG_DJT_PRJ_V210_CQS)
const int g_TpVirtualKey[] = {TOUCH_KEY_HOME , TOUCH_KEY_BACK,TOUCH_KEY_MENU, TOUCH_KEY_SEARCH};
#else
const int g_TpVirtualKey[] = {TOUCH_KEY_HOME , TOUCH_KEY_BACK,TOUCH_KEY_MENU, TOUCH_KEY_SEARCH};
#endif
...
int TP_face_mode_switch(int on)///enable  ctp sensor
{
	...
	DBG("TP_face_mode_switch on=%d",on);
    ...
    {
        data[3] = 0xA0;
        msg_proximity_flag = 1;
    }
    else
    {
        data[3] = 0xA1;
        msg_proximity_flag = 0;
    }
		
    return ((g_I2cClient) ? (IicWriteData(SLAVE_I2C_ID_DWI2C, &data[0], 4)) : (-ENODEV));
}
static int msg2133_ps_open(struct inode *inode, struct file *file)
{
	DBG("%s\n", __func__);
	if (msg2xxx_ps_opened)
		return -EBUSY;
	msg2xxx_ps_opened = 1;
	return 0;
}

static int msg2133_ps_release(struct inode *inode, struct file *file)
{
	DBG("%s", __func__);
	msg2xxx_ps_opened = 0;
...
static int msg2133_ps_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	void __user *argp = (void __user *)arg;
	int flag;
	//unsigned char data;
	DBG("%s: cmd %d", __func__, _IOC_NR(cmd));
	...
	case LTR_IOCTL_SET_LFLAG:
		...
		DBG("%s: set flag=%d", __func__, flag);
		break;
	default:
		break;
	} 
...
static struct file_operations msg2133_ps_fops = {
	.owner				= THIS_MODULE,
	.open				= msg2133_ps_open,
	.release			= msg2133_ps_release,
	.unlocked_ioctl				= msg2133_ps_ioctl,
};
static struct miscdevice msg2133_ps_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = MSG2XXX_PS_DEVICE,
	.fops = &msg2133_ps_fops,
};
...
s32 DrvPlatformLyrInputDeviceInitialize(struct i2c_client *pClient)
{
    ...
    {
		err = -ENOMEM;
        DBG("%s() Line: %d, input_allocate_device failed, err = %d.\n", __FUNCTION__, __LINE__, err);

		goto exit_input_register_device_failed;
	}


	ps_input_dev->name = MSG2XXX_PS_INPUT_DEV;
	...
	err = misc_register(&msg2133_ps_device);
	if (err) {
		printk("%s: msg2133_ps_device register failed\n", __func__);
		goto exit_input_register_device_failed;
	}
#endif
...
#else
        nRetVal = request_irq(_gIrq/*MS_TS_MSG_IC_GPIO_INT*/, _DrvPlatformLyrFingerTouchInterruptHandler,
                      IRQF_TRIGGER_FALLING /* | IRQF_NO_SUSPEND *//* IRQF_TRIGGER_FALLING */,
                      "msg2xxx", NULL);
#endif
        if (nRetVal != 0)
        {
...
void DrvPlatformLyrSetIicDataRate(struct i2c_client *pClient, u32 nIicDataRate)
{
    ...
    // TODO : Please FAE colleage to confirm with customer device driver engineer for how to set i2c data rate on SPRD platform
