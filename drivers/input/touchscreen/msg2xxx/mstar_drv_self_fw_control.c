...
extern struct input_dev        *ps_input_dev;
#endif
...
u16 _gGestureWakeupValue = 0;//zgq
        ...
        else //touch on screen
        {
            if (tInfo.nTouchKeyCode != 0)
            {
                ...
                else if (tInfo.nTouchKeyCode == 2) // TOUCH_KEY_BACK
                {
                 ...
                 else if (tInfo.nTouchKeyCode == 5) // TOUCH_KEY_BACK
                {
                    nTouchKeyCode = g_TpVirtualKey[4];
                } 
...
#if defined(VIRTUAL_KEYS)
    // Virtual key;
                switch (tInfo.nTouchKeyCode)
                {
                #if defined(CONFIG_LCMSIZE_HVGA)
					case 1:
						tInfo.tPoint[0].nX= 60;
						break;
					
					case 2:
						tInfo.tPoint[0].nX = 120;
						break;
					
					case 4:
						tInfo.tPoint[0].nX= 240;
						break;

				#else
				   	case 1:
                        tInfo.tPoint[0].nX= 25;
                        break;

                    case 2:
                        tInfo.tPoint[0].nX = 90;
                        break;

                    case 4:
                        tInfo.tPoint[0].nX= 150;
                        break;
				#endif
                #if defined(CONFIG_DJT_PRJ_C177_YMT) || defined(CONFIG_DJT_PRJ_C168_XLL_051) || defined(CONFIG_DJT_PRJ_C325V_HLG)
                    case 3:
                    case 5:
                        tInfo.tPoint[0].nX= 300;
                        break;
	         #endif
                    case 8:
                    default:
                        // Fixme;
                        tInfo.tPoint[0].nX = 220;
                        break;
                }
          #if defined(CONFIG_LCMSIZE_QHD)||defined (CONFIG_LCMSIZE_FWVGA)
           			tInfo.tPoint[0].nY = 1020;
		  #elif defined(CONFIG_LCMSIZE_HD)
           			tInfo.tPoint[0].nY = 1320;
		  #elif defined(CONFIG_LCMSIZE_HVGA)
		  			tInfo.tPoint[0].nY = 540;
          #else
                tInfo.tPoint[0].nY = 904;
           #endif
                 tInfo.nFingerNum = 1;
		    printk("guorui x=%d y=%d\n",tInfo.tPoint[0].nX,tInfo.tPoint[0].nY);
		if (nLastKeyCode != tInfo.nTouchKeyCode)
		{
	       	DrvPlatformLyrFingerTouchPressed(tInfo.tPoint[0].nX, tInfo.tPoint[0].nY, 0, 0);
		   DBG("jinlili x=%d y=%d\n",tInfo.tPoint[0].nX,tInfo.tPoint[0].nY);
		   nLastKeyCode = tInfo.nTouchKeyCode;
		}
#endif
            }
            else
