...
#include "msg22xx_xxxx_update_bin_5001.h" /*  for MSG22xx */
#include "msg22xx_yyyy_update_bin_5001.h"
...
#include "msg28xx_xxxx_update_bin_oe03.h" /*  for MSG28xx */
#include "msg28xx_yyyy_update_bin_oe03_dj.h"
...
static u16 _gSFR_ADDR3_BYTE0_1_VALUE; /*  for MSG28xx */
static u16 _gSFR_ADDR3_BYTE2_3_VALUE;
...
#ifdef CONFIG_ENABLE_GESTURE_DEBUG_MODE
u8 g_GestureDebugFlag;
u8 g_GestureDebugMode;
...
u8 g_GestureWakeupFlag;
u8 g_GestureSwitch;
#endif /* CONFIG_ENABLE_GESTURE_WAKEUP */

#ifdef CONFIG_ENABLE_PROXIMITY_DETECTION
u8 g_EnableTpProximity;
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
u8 g_FaceClosingTp; /*  for QCOM platform -> 1 : close to, 0 : far away */
...
u8 g_IsHwResetByDriver;
#endif /* CONFIG_ENABLE_ESD_PROTECTION */

u8 g_IsUpdateFirmware;
u8 g_Msg22xxChipRevision;

u8 g_IsDisableFinagerTouch;
	...
	if (g_GestureWakeupFlag == 1) {
		...
		if (bIsCorrectFormat) {
			...
			case 0x58:
				...
				input_report_key(g_InputDevice, KEY_GESTURE_DC, 1);
				input_sync(g_InputDevice);
				input_report_key(g_InputDevice, KEY_GESTURE_DC, 0);
				input_sync(g_InputDevice);
				break;
#if 0
			case 0x60:
...
/* 				input_report_key(g_InputDevice, KEY_LEFT, 1); */
				input_report_key(g_InputDevice, KEY_F19, 1);
				input_sync(g_InputDevice);
/* 				input_report_key(g_InputDevice, KEY_LEFT, 0); */
				input_report_key(g_InputDevice, KEY_F19, 0);
				input_sync(g_InputDevice);
				break;
			case 0x63:
...
/* 				input_report_key(g_InputDevice, KEY_RIGHT, 1); */
				input_report_key(g_InputDevice, KEY_F20, 1);
				input_sync(g_InputDevice);
/* 				input_report_key(g_InputDevice, KEY_RIGHT, 0); */
				input_report_key(g_InputDevice, KEY_F20, 0);
				input_sync(g_InputDevice);
				break;
			case 0x64:
...
/* 				input_report_key(g_InputDevice, KEY_M, 1); */
				input_report_key(g_InputDevice, KEY_F18, 1);
				input_sync(g_InputDevice);
/* 				input_report_key(g_InputDevice, KEY_M, 0); */
				input_report_key(g_InputDevice, KEY_F18, 0);
				input_sync(g_InputDevice);
				break;
			case 0x65:
...
/* 				input_report_key(g_InputDevice, KEY_E, 1); */
				input_report_key(g_InputDevice, KEY_F16, 1);
				input_sync(g_InputDevice);
/* 				input_report_key(g_InputDevice, KEY_E, 0); */
				input_report_key(g_InputDevice, KEY_F16, 0);
				input_sync(g_InputDevice);
				break;
			case 0x68:
#if 0
...
/* 				input_report_key(g_InputDevice, KEY_O, 1); */
				input_report_key(g_InputDevice, KEY_F14, 1);
				input_sync(g_InputDevice);
/* 				input_report_key(g_InputDevice, KEY_O, 0); */
				input_report_key(g_InputDevice, KEY_F14, 0);
				input_sync(g_InputDevice);
				break;
#if 0
			case 0x6A:
	...
	if ((pPacket[nCheckSumIndex] == nCheckSum) && (pPacket[0] == 0x52)) {
		...
		*/
		if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) && (pPacket[4] == 0xFF) && (pPacket[6] == 0xFF)) {
			...
			if ((pPacket[5] != 0x00) && (pPacket[5] != 0xFF)) {
...
#ifdef TOUCH_VIRTUAL_KEYS /* CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM //1 */
#ifdef CONFIG_ENABLE_REPORT_KEY_WITH_COORDINATE
				pInfo->nFingerNum = 1;
				...
				if (pPacket[5] == 1) {
					/*  TOUCH_KEY_MENU */
					pInfo->tPoint[0].nX = pdata->virtualkeys[0];/* g_TpVirtualKeyDimLocal[0][0]; */
					pInfo->tPoint[0].nY = pdata->virtualkeys[1];/* g_TpVirtualKeyDimLocal[0][1]; */
				} else if (pPacket[5] == 2) {
					/*  TOUCH_KEY_HOME */
					pInfo->tPoint[0].nX = pdata->virtualkeys[4];/* g_TpVirtualKeyDimLocal[1][0]; */
					pInfo->tPoint[0].nY = pdata->virtualkeys[5];/* g_TpVirtualKeyDimLocal[1][1]; */
				} else if (pPacket[5] == 4) {
					/*  TOUCH_KEY_BACK */
					pInfo->tPoint[0].nX = pdata->virtualkeys[8];/* g_TpVirtualKeyDimLocal[2][0]; */
					pInfo->tPoint[0].nY = pdata->virtualkeys[9];/* g_TpVirtualKeyDimLocal[2][1]; */
				} else if (pPacket[5] == 8) {
					/*  TOUCH_KEY_SEARCH */
...
#endif /* CONFIG_ENABLE_REPORT_KEY_WITH_COORDINATE */
#endif /* CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM //1 */
...
/* 			if ((nDeltaX == 0) && (nDeltaY == 0)) */
			if (
				...
				/* Finger 2 */
				if (nDeltaX > 2048) {
					/*  transform the unsigned value to signed value */
					nDeltaX -= 4096;
				}
...
/* 				} */
		...
		if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) && (pPacket[4] == 0xFF) && (pPacket[5] == 0xFF) && (pPacket[6] == 0xFF) && (pPacket[7] == 0xFF)) {
			...
			if ((pPacket[8] != 0x00) && (pPacket[8] != 0xFF)) {
			...
			for (i = 0; i < MAX_KEY_NUM; i++) {
				if ((nButton & (1<<i)) == (1<<i)) {
					if (pInfo->nKeyCode == 0) {
						...
						pInfo->nKeyCode = 0xFF;
						pInfo->nCount = 1;
						pInfo->tPoint[0].nX = pdata->virtualkeys[0+i];/* g_TpVirtualKeyDimLocal[i][0]; */
						pInfo->tPoint[0].nY = pdata->virtualkeys[1+i];/* g_TpVirtualKeyDimLocal[i][1]; */
						pInfo->tPoint[0].nP = 1;
...
/* 				szWriteData[1] = 0x00; */
		...
		for (i = 1; i < (nPageNum - 1); i++) {
			/*  Skip the first 128 byte and the last 128 byte of info block */
			if (i == 1) {
				/*  Read first data 4 bytes */
				nLength = MSG28XX_EMEM_SIZE_BYTES_ONE_WORD;
...
static u32 _DrvFwCtrlMsg28xxRetrieveFirmwareCrcFromEFlash(EmemType_e eEmemType)
{
	...
	if (eEmemType == EMEM_MAIN) {
		/*  Read main block CRC(128KB-4) from main block */
...
static s32 _DrvFwCtrlUpdateFirmwareCash(u8 szFwData[][1024], EmemType_e eEmemType)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG21XXA) {
		...
		if (_gTpVendorCode[0] == 'C' && _gTpVendorCode[1] == 'N' && _gTpVendorCode[2] == 'T') {
			/*  for specific TP vendor which store some important information in info block, only update firmware for main block, do not update firmware for info block. */
		...
		if (IS_FORCE_TO_UPDATE_FIRMWARE_ENABLED) {
			/*  Force to update firmware, do not check whether the vendor id of the update firmware bin file is equal to the vendor id on e-flash. */
...
void _DrvFwCtrlMsg22xxCheckFirmwareUpdateBySwId(void)
{
	...
	if (nCrcMainA == nCrcMainB && nCrcInfoA == nCrcInfoB) {
		...
		/*  Case 4. Main Block:FAIL, Info Block:FAIL */
...
static u32 _DrvFwCtrlMsg26xxmCalculateFirmwareCrcFromEFlash(EmemType_e eEmemType, u8 nIsNeedResetHW)
{
	...
	if (eEmemType == EMEM_MAIN) {
		/*  Read calculated main block CRC(32K-8) from register */
		nRetVal = RegGet16BitValue(0x3C80);
		nRetVal = (nRetVal << 16) | RegGet16BitValue(0x3C82);

		DBG(&g_I2cClient->dev, "Main Block CRC = 0x%x\n", nRetVal);
	} else if (eEmemType == EMEM_INFO) {
		/*  Read calculated info block CRC(8K) from register */
...
static u32 _DrvFwCtrlMsg26xxmRetrieveFirmwareCrcFromMainBlock(EmemType_e eEmemType)
{
	...
	szTxData[0] = 0x72;
	if (eEmemType == EMEM_MAIN) {
		/*  Read main block CRC(32K-8) from main block */
		szTxData[1] = 0x7F;
		szTxData[2] = 0xF8;
	} else if (eEmemType == EMEM_INFO) {
		/*  Read info block CRC(8K) from main block */
		szTxData[1] = 0x7F;
		szTxData[2] = 0xFC;
	}
	szTxData[3] = 0x00;
...
static u32 _DrvFwCtrlMsg26xxmRetrieveFrimwareCrcFromBinFile(u8 szTmpBuf[][1024], EmemType_e eEmemType)
{
	...
	if (szTmpBuf != NULL) {
		if (eEmemType == EMEM_MAIN) {
			/*  Read main block CRC(32K-8) from bin file */
			...
			/*  Read info block CRC(8K) from bin file */
...
void _DrvFwCtrlMsg26xxmCheckFirmwareUpdateBySwId(void)
{
	...
	if (nCrcMainA == nCrcMainB && nCrcInfoA == nCrcInfoB) {
		...
		/*  Case 3. Main Block:FAIL, Info Block:FAIL/OK */
		...
		if (nCrcInfoA == nCrcInfoB) {
			/*  Check if info block is actually OK. */
...
/* 		} */
	...
	if (nChipType == CHIP_TYPE_MSG22XX) {
		/*  (0x7A) */
...
#if (defined(CONFIG_ENABLE_CHIP_MSG21XXA) || defined(CONFIG_ENABLE_CHIP_TYPE_MSG22XX))
	szTxData[0] = 0x02;
#else
	szTxData[0] = 0x09;
#endif
...
#ifdef TP_HAVE_PROX
	DBG(&g_I2cClient->dev, "wang interupt _is_call==%d, pPacket[5]=%x\n", is_incall, pPacket[5]);

	if (((pPacket[5] == 0x80) || (pPacket[5] == 0x40)) && (is_incall == 1)) {
		/* near 0 */
		if ((pPacket[5] == 0x80)) {
			DBG(&g_I2cClient->dev, "prox [mstar] face near 0n\n");
			/* printk("prox [mstar] face near 0n\n"); */
			input_report_abs(proximity_input_dev_msg, ABS_DISTANCE, 1);
			input_sync(proximity_input_dev_msg);
		}
		/* far 0 */
		if ((pPacket[5] == 0x40)) {
			DBG(&g_I2cClient->dev, "prox [mstar] face far\n");
			/* printk("prox [mstar] face far\n"); */
			input_report_abs(proximity_input_dev_msg, ABS_DISTANCE, 0);
			input_sync(proximity_input_dev_msg);
		}
