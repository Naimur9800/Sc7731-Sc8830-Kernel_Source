...
u8 *_gFwVersion; /*  customer firmware version */

static u8 _gnDebugLogTimesStamp;
...
static struct proc_dir_entry *_gProcMpTestLogALLEntry;
...
static struct proc_dir_entry *_gProcLeatherSheathModeEntry;
...
static struct proc_dir_entry *_gProcXrLimitFirmwareUpdateEntry;
static struct proc_dir_entry *_gProcForceFirmwareUpdateEntry;
...
{
	return sprintf(buf, "MSG5528 Gesture feature is %s\n", (g_GestureSwitch ? "enable" : "disable"));
}
static ssize_t gestureswitch_set(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	ret = kstrtoint(buf, 10, &g_GestureSwitch);
	if (g_GestureSwitch) {
#ifdef CONFIG_SUPPORT_64_TYPES_GESTURE_WAKEUP_MODE
		g_GestureWakeupMode[0] = 0xffff;
		g_GestureWakeupMode[1] = 0xffff;
#else
		g_GestureWakeupMode[0] = 0xffff;
#endif
	} else {
#ifdef CONFIG_SUPPORT_64_TYPES_GESTURE_WAKEUP_MODE
		g_GestureWakeupMode[0] = 0x0000;
		g_GestureWakeupMode[1] = 0x0000;
#else
		g_GestureWakeupMode[0] = 0x0000;
#endif
	}
...
static const struct file_operations _gProcXrLimitFirmwareUpdate = {
	.read = DrvMainProcfsXrLimitFirmwareUpdateRead,
};
...
#if (defined(CONFIG_ENABLE_CHIP_TYPE_MSG21XXA) || defined(CONFIG_ENABLE_CHIP_TYPE_MSG26XXM) || defined(CONFIG_ENABLE_CHIP_TYPE_MSG28XX))
u32 SLAVE_I2C_ID_DBBUS = (0xC4>>1); /* 0x62 // for MSG21XX/MSG21XXA/MSG26XXM/MSG28XX */
#elif defined(CONFIG_ENABLE_CHIP_TYPE_MSG22XX)
...
u16 DEMO_MODE_PACKET_LENGTH; /*  If project use MSG26xxM/MSG28xx, set MUTUAL_DEMO_MODE_PACKET_LENGTH as default. If project use MSG21xxA/MSG22xx, set SELF_DEMO_MODE_PACKET_LENGTH as default. */
u16 DEBUG_MODE_PACKET_LENGTH; /*  If project use MSG26xxM/MSG28xx, set MUTUAL_DEBUG_MODE_PACKET_LENGTH as default. If project use MSG21xxA/MSG22xx, set SELF_DEBUG_MODE_PACKET_LENGTH as default. */
u16 MAX_TOUCH_NUM; /*  If project use MSG26xxM/MSG28xx, set MUTUAL_MAX_TOUCH_NUM as default. If project use MSG21xxA/MSG22xx, set SELF_MAX_TOUCH_NUM as default. */


struct kset *g_TouchKSet;
struct kobject *g_TouchKObj;
u8 g_IsSwitchModeByAPK;


u8 IS_GESTURE_WAKEUP_ENABLED;
u8 IS_GESTURE_DEBUG_MODE_ENABLED;
u8 IS_GESTURE_INFORMATION_MODE_ENABLED;
u8 IS_GESTURE_WAKEUP_MODE_SUPPORT_64_TYPES_ENABLED;
...
u8 IS_FORCE_TO_UPDATE_FIRMWARE_ENABLED;


#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
#ifdef CONFIG_ENABLE_GESTURE_DEBUG_MODE
struct kset *g_GestureKSet;
struct kobject *g_GestureKObj;
#endif /* CONFIG_ENABLE_GESTURE_DEBUG_MODE */
#endif /* CONFIG_ENABLE_GESTURE_WAKEUP */

#ifdef CONFIG_ENABLE_SEGMENT_READ_FINGER_TOUCH_DATA
u16 g_FwPacketDataAddress;
u16 g_FwPacketFlagAddress;
u8 g_FwSupportSegment;
#endif /* CONFIG_ENABLE_SEGMENT_READ_FINGER_TOUCH_DATA */

#ifdef CONFIG_ENABLE_COUNT_REPORT_RATE
u32 g_IsEnableReportRate;
u32 g_InterruptCount;
u32 g_ValidTouchCount;
u32 g_InterruptReportRate;
u32 g_ValidTouchReportRate;
...
u8 g_IsHotknotEnabled;
u8 g_IsBypassHotknot;
	...
	struct device_attribute *attr, char *buf)
{
	...
	if ((0x85 == g_ChipType) || (0xbf == g_ChipType))
	...
	return snprintf(buf, PAGE_SIZE, "msg2xxx::chipid(0x%x)::fwversion(%s)\n",
		   g_ChipType, _gFwVersion);
}
...
/* 	nLength = sprintf(pBuffer, "%d", _gIsUpdateComplete); */
	nLength = copy_to_user((void __user *)pBuffer, &_gIsUpdateComplete, sizeof(_gIsUpdateComplete));
	DBG(&g_I2cClient->dev, "*** _gIsUpdateComplete = %d ***\n", _gIsUpdateComplete);
...
ssize_t DrvMainProcfsXrLimitFirmwareUpdateRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	int nLength = 0;
	...
	DBG(&g_I2cClient->dev, "FIRMWARE_FILE_PATH_ON_XR_T_CARD = %s\n", FIRMWARE_FILE_PATH_ON_XR_T_CARD);

	if (0 != DrvIcFwLyrUpdateFirmwareBySdCard(FIRMWARE_FILE_PATH_ON_XR_T_CARD)) {
		_gIsUpdateComplete = 0;
		DBG(&g_I2cClient->dev, "Update FAILED\n");
	} else {
		_gIsUpdateComplete = 1;
...
/* 	nLength = sprintf(pBuffer, "%d", _gFeatureSupportStatus); */
	nLength = copy_to_user((void __user *)pBuffer, &_gFeatureSupportStatus, sizeof(_gFeatureSupportStatus));
...
ssize_t DrvMainKObjectPacketShow(struct kobject *pKObj, struct kobj_attribute *pAttr, char *pBuf)
{
	...
	if (strcmp(pAttr->attr.name, "packet") == 0) {
		if (g_FirmwareMode == FIRMWARE_MODE_DEMO_MODE) {
			...
			/* g_FirmwareMode == FIRMWARE_MODE_DEBUG_MODE || g_FirmwareMode == FIRMWARE_MODE_RAW_DATA_MODE */
...
/* 			#ifdef CONFIG_ENABLE_CHIP_MSG21XXA */
			if (g_ChipType == CHIP_TYPE_MSG21XX) {
				ps_store_data[2] = 0x62;/* 78 */
...
/* 			#endif */
			DBG(&g_I2cClient->dev, "mstar_open the ps mod successful\n");
			/* printk("mstar_open the prox mod successful 0x%x\n", ps_store_data[2]); */
		} else {
			ps_store_data[0] = 0x52;
			ps_store_data[1] = 0x00;  /* 01 */
/* 			#ifdef CONFIG_ENABLE_CHIP_MSG21XXA */
...
/* 			#endif */
			DBG(&g_I2cClient->dev, "msatr_close the ps mod successful\n");
			/* printk("msatr_close the prox mod successful 0x%x\n", ps_store_data[2]); */
		}
		/*  IicWriteData(SLAVE_I2C_ID_DBBUS, */
		err = IicWriteData(SLAVE_I2C_ID_DWI2C, &ps_store_data[0], 4);
...
static ssize_t show_proximity_sensor(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	DBG(&g_I2cClient->dev, "wang tp get prox ver\n");
/* 	if (buf != NULL) { */
	return snprintf(buf, PAGE_SIZE, "tp prox is_incall %d\n", is_incall);
/* 	} */
...
static ssize_t store_proximity_sensor(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	DBG(&g_I2cClient->dev, "wang_store_proximity_sensor buf=%d, size=%d, on_off=%d\n", *buf, size, on_off);
	if (buf != NULL && size != 0) {
		if (0 == on_off) {
#ifdef WAKE_LOCK_LPSENSOR
		wake_unlock(&pls_delayed_work_wake_lock_msg);
#endif
			mstar_prox_ctl(0);
			is_incall = 0;
		} else if (1 == on_off) {
#ifdef WAKE_LOCK_LPSENSOR
		wake_lock(&pls_delayed_work_wake_lock_msg);
#endif
			mstar_prox_ctl(1);
			is_incall = 1;
		}
...
static DEVICE_ATTR(proximity, 0644, show_proximity_sensor, store_proximity_sensor);
#endif
/* ------------------------------------------------------------------------------// */
		...
		if (nFeature == FEATURE_GESTURE_WAKEUP_MODE) {
			...
			if (g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA) {
				if (_gFeatureSupportStatus == 1) {
					/*  If the debug mode data log function is supported, then get packet address and flag address for segment read finger touch data. */
...
ssize_t DrvMainProcfsLeatherSheathModeWrite(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	...
	if (pBuffer != NULL) {
		...
		if (nLeatherSheathMode == 0x01) {
			/* open leather sheath mode */
			DrvIcFwLyrOpenLeatherSheathMode();
			g_IsEnableLeatherSheathMode = 1;
		} else if (nLeatherSheathMode == 0x00) {
			/* close leather sheath mode */
...
s32 DrvMainTouchDeviceInitialize(void)
{
	...
	if (g_ChipType == 0) {
		/*  If failed, try to get chip type by SLAVE_I2C_ID_DBBUS(0x59) again. */
...
void DrvMainRemoveProcfsDirEntry(void)
{
	...
	if (_gProcXrLimitFirmwareUpdateEntry != NULL) {
		remove_proc_entry(PROC_NODE_XR_LIMIT_FIRMWARE_UPDATE, _gProcDeviceEntry);
		_gProcXrLimitFirmwareUpdateEntry = NULL;
		DBG(&g_I2cClient->dev, "Remove procfs file node(%s) OK!\n", PROC_NODE_XR_LIMIT_FIRMWARE_UPDATE);
	}
...
static s32 _DrvMainCreateProcfsDirEntry(void)
{
	...
	_gProcXrLimitFirmwareUpdateEntry = proc_create(PROC_NODE_XR_LIMIT_FIRMWARE_UPDATE, PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcXrLimitFirmwareUpdate);
	if (NULL == _gProcXrLimitFirmwareUpdateEntry) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n", PROC_NODE_XR_LIMIT_FIRMWARE_UPDATE);
	} else {
		DBG(&g_I2cClient->dev, "Create procfs file node(%s) OK!\n", PROC_NODE_XR_LIMIT_FIRMWARE_UPDATE);
	}
	...
	if (IS_ERR(tp_class)) {
		/* DBG(&g_I2cClient->dev, "Failed to create class(version)!\n"); */
	}
	tp_cmd_dev = device_create(tp_class, NULL, 0, NULL, "device");/* device */

	if (IS_ERR(tp_cmd_dev)) {
		/* DBG(&g_I2cClient->dev, "Failed to create device(tp_cmd_dev)!\n"); */
	}
	if (device_create_file(tp_cmd_dev, &dev_attr_tp_version) < 0) {
		/*  /sys/class/mtk-tpd/device/version */
		/* DBG(&g_I2cClient->dev, "Failed to create device file(%s)!\n", dev_attr_tp_version.attr.name); */
	}
#endif

#ifdef TP_HAVE_PROX
	firmware_class_msg = class_create(THIS_MODULE, "mtk-tpd");/* client->name */

	if (IS_ERR(firmware_class_msg))
		DBG(&g_I2cClient->dev, "wang Failed to create class(firmware)!\n");
	firmware_cmd_dev_msg = device_create(firmware_class_msg, NULL, 0, NULL, "device");/* device */

	if (IS_ERR(firmware_cmd_dev_msg))
		DBG(&g_I2cClient->dev, "wang Failed to create device(firmware_cmd_dev_msg)!\n");

	if (device_create_file(firmware_cmd_dev_msg, &dev_attr_proximity) < 0) {
		/*  /sys/class/mtk-tpd/device/proximity */
		DBG(&g_I2cClient->dev, "wang Failed to create device file(%s)!\n", dev_attr_proximity.attr.name);
	}
#endif
#ifdef WAKE_LOCK_LPSENSOR
	wake_lock_init(&pls_delayed_work_wake_lock_msg, WAKE_LOCK_SUSPEND, "prox_delayed_work");
#endif

#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
	nRetVal = device_create_file(&g_I2cClient->dev, &dev_attr_gestureswitch);
	if (nRetVal < 0)
		DBG(&client->dev,
