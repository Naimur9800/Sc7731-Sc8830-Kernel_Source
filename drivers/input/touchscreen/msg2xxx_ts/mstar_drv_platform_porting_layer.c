...
#include <linux/wakelock.h> /* luojy 20150914 */
/*=============================================================*/
...
struct wake_lock tp_wakelock; /*luojy 20150914 */
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
...
static int MS_TS_MSG_IC_GPIO_RST; /*  Must set a value other than 1 */
...
#else
static struct notifier_block _gAdfNotifier;
#endif
...
int g_TpVirtualKey[] = {TOUCH_KEY_APPSELECT, TOUCH_KEY_HOME, TOUCH_KEY_BACK};

#ifdef CONFIG_ENABLE_REPORT_KEY_WITH_COORDINATE
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
struct kobject *g_PropertiesKObj;
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
#define BUTTON_W (100)
#define BUTTON_H (100)
...
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
#ifdef CONFIG_ENABLE_PROXIMITY_DETECTION
struct input_dev *g_ProximityInputDevice;
#endif /* CONFIG_ENABLE_PROXIMITY_DETECTION */
#endif /* CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM || CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM */
...
struct workqueue_struct *g_EsdCheckWorkqueue;
void DrvPlatformLyrEsdCheck(struct work_struct *pWork);
#endif /* CONFIG_ENABLE_ESD_PROTECTION */

struct input_dev *g_InputDevice;
...
{
	struct msg2xxx_ts_platform_data *pdata; /*KEY_MENU */
	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
	pdata = g_I2cClient->dev.platform_data;
...
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
	wake_lock_timeout(&tp_wakelock, msecs_to_jiffies(200)); /* luojy 20150914 */
#endif
...
s32 DrvPlatformLyrInputDeviceInitialize(struct i2c_client *pClient)
{
	...
	__set_bit(KEY_HOMEPAGE,  g_InputDevice->keybit);
	__set_bit(KEY_APPSELECT,  g_InputDevice->keybit);
/*
#ifdef CONFIG_TP_HAVE_KEY
	//  Method 2.
...
*/
	...
	input_set_capability(g_InputDevice, EV_KEY, KEY_F13);
	input_set_capability(g_InputDevice, EV_KEY, KEY_F14);
	input_set_capability(g_InputDevice, EV_KEY, KEY_F15);
	input_set_capability(g_InputDevice, EV_KEY, KEY_F16);
	input_set_capability(g_InputDevice, EV_KEY, KEY_F18);
	input_set_capability(g_InputDevice, EV_KEY, KEY_F19);
	input_set_capability(g_InputDevice, EV_KEY, KEY_F20);
	input_set_capability(g_InputDevice, EV_KEY, KEY_GESTURE_DC);
#endif /* CONFIG_ENABLE_GESTURE_WAKEUP */
...
*/
	...
	wake_lock_init(&tp_wakelock, WAKE_LOCK_SUSPEND, "tp_input_wakelock"); /* luojy 20150914 */

#ifdef TP_HAVE_PROX
	/* setup prox input device */
	proximity_input_dev_msg = input_allocate_device();

	/* p_tp_param = &p_tp_cfg->tp; */
	proximity_input_dev_msg->name = PROXIMITY_INPUT_DEV; /* p_tp_cfg->name; */
	proximity_input_dev_msg->phys = PROXIMITY_INPUT_DEV;
	proximity_input_dev_msg->id.bustype = BUS_I2C;
	proximity_input_dev_msg->dev.parent = &pClient->dev;
	__set_bit(EV_ABS, proximity_input_dev_msg->evbit);
	input_set_abs_params(proximity_input_dev_msg, ABS_DISTANCE, 0, 1, 0, 0);

	if (!proximity_input_dev_msg) {
		nRetVal = -ENOMEM;
		/* DBG("prox%s: failed to allocate input device\n", __func__); */
		return nRetVal;
	}
	nRetVal = input_register_device(proximity_input_dev_msg);
	if (nRetVal) {
		input_put_device(proximity_input_dev_msg);
		/* DBG("prox Unable to register prox device %d!", nRetVal); */
		return nRetVal;
	}
#endif
...
/* 	pm_runtime_forbid(&g_I2cClient->dev); */
	_gAdfNotifier.notifier_call = MsDrvInterfaceTouchDeviceAdfNotifierCallback;
	adf_register_client(&_gAdfNotifier);
#endif
#endif /* CONFIG_ENABLE_NOTIFIER_FB */
#endif /* CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM || CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM */
}
...
*/
	...
	wake_lock_destroy(&tp_wakelock); /* luojy 20150914 */

#ifdef TP_HAVE_PROX
	/* DBG("prox %s() ***\n", __func__); */
	input_unregister_device(proximity_input_dev_msg);
#endif
...
void DrvPlatformLyrEsdCheck(struct work_struct *pWork)
{
	...
	if (_gInterruptFlag == 0) {
		/*  Skip ESD check while finger touch */
...
#ifdef CONFIG_ENABLE_ITO_MP_TEST
	if (g_IsInMpTest == 1) {
		/*  Check whether mp test is proceeding */
