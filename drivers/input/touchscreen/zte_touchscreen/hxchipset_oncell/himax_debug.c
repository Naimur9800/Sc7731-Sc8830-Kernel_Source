...
static ssize_t himax_CRC_test_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	ssize_t ret = 0;
	uint8_t result = 0;
	char *temp_buf;
...
static ssize_t himax_vendor_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (IC_TYPE < 8)
			ret +=
			    snprintf(temp_buf + ret, len - ret, "CONFIG_VER = 0x%2.2X\n", ic_data->vendor_config_ver);
		else {
			ret +=
			    snprintf(temp_buf + ret, len - ret, "TOUCH_VER = 0x%2.2X\n",
				     ic_data->vendor_touch_cfg_ver);
			ret +=
			    snprintf(temp_buf + ret, len - ret, "DISPLAY_VER = 0x%2.2X\n",
		...
		if (ic_data->vendor_cid_maj_ver < 0 && ic_data->vendor_cid_min_ver < 0)
			...
			ret +=
			    snprintf(temp_buf + ret, len - ret, "CID_VER = 0x%2.2X\n",
...
static ssize_t himax_attn_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	ssize_t ret = 0;
...
static ssize_t himax_int_en_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
...
static ssize_t himax_int_en_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
	struct himax_ts_data *ts = private_ts;
	char buf_tmp[12] = { 0 };
	int value, ret = 0;
...
static ssize_t himax_layout_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	struct himax_ts_data *ts = private_ts;
	size_t ret = 0;
	char *temp_buf;
...
static ssize_t himax_layout_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
	struct himax_ts_data *ts = private_ts;
	char buf_tmp[5];
	int i = 0, j = 0, k = 0, ret;
...
static ssize_t himax_debug_level_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	struct himax_ts_data *ts_data;
	size_t ret = 0;
	char *temp_buf;
...
static ssize_t himax_debug_level_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
...
static ssize_t himax_proc_register_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	int ret = 0;
	uint16_t loop_i;
	uint8_t data[128];
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		ret +=
		    snprintf(temp_buf + ret, len - ret, "command:  %02X,%02X,%02X,%02X\n", register_command[3],
	...
	if ((buf[0] == 'r' || buf[0] == 'w') && buf[1] == ':' && buf[2] == 'x') {
		...
		if (buf[0] == 'r') {
			...
			if (count < 3) {
				...
				himax_register_write(private_ts->client, register_command, byte_length, w_data,
						     cfg_flag);
			} else {
				...
				byte_length = count - 1;
				himax_register_write(private_ts->client, register_command, byte_length, &w_data[0],
						     cfg_flag);
			}
		} else
			return len;
...
void himax_log_touch_int_devation(int touched)
{
	...
	if (touched == HX_FINGER_ON) {
		...
		timeDelta.tv_nsec =
		    (timeEnd.tv_sec * 1000000000 + timeEnd.tv_nsec) - (timeStart.tv_sec * 1000000000 +
								       timeStart.tv_nsec);
...
void himax_log_touch_event_detail(struct himax_ts_data *ts, int x, int y, int w, int loop_i, uint8_t EN_NoiseFilter,
		...
		if (old_finger >> loop_i == 1) {
			if (ts->useScreenRes) {
				I("status: Screen:F:%02d Up, X:%d, Y:%d, N:%d\n", loop_i + 1,
					...
					ts->pre_finger_data[loop_i][1] * ts->heightFactor >> SHIFTBITS, EN_NoiseFilter);
			} else {
				I("status: Raw:F:%02d Up, X:%d, Y:%d, N:%d\n", loop_i + 1,
					ts->pre_finger_data[loop_i][0], ts->pre_finger_data[loop_i][1], EN_NoiseFilter);
			}
		}
	} else {
		I("%s : wrong input!\n", __func__);
...
/* #endif */
static ssize_t himax_diag_arrange_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
	char buf[80] = { 0 };
...
void himax_ts_diag_func(void)
{
	...
	if (dsram_type == 3) {
		memcpy(mutual_data_old, mutual_data_new, x_channel * y_channel * sizeof(int16_t));
	}
	diag_max_cnt++;
	if (dsram_type >= 1 && dsram_type <= 3) {
		...
		if (!IS_ERR(diag_sram_fn)) {
			...
			if (write_counter < write_max_count)
				queue_delayed_work(private_ts->himax_diag_wq, &private_ts->himax_diag_delay_wrok,
						   1 / 10 * HZ);
			else {
...
static ssize_t himax_reset_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
	char buf_tmp[12];
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (debug_level_cmd == 't') {
			...
			if (handshaking_result == 0) {
				ret +=
				    snprintf(temp_buf + ret, len - ret, "Handshaking Result = %d (MCU Running)\n",
					     handshaking_result);
			} else if (handshaking_result == 1) {
				ret +=
				    snprintf(temp_buf + ret, len - ret, "Handshaking Result = %d (MCU Stop)\n",
					     handshaking_result);
			} else if (handshaking_result == 2) {
				ret +=
				    snprintf(temp_buf + ret, len - ret, "Handshaking Result = %d (I2C Error)\n",
					     handshaking_result);
			} else {
			...
			if (ic_data->vendor_panel_ver < 0)
				...
				ret +=
				    snprintf(temp_buf + ret, len - ret, "PANEL_VER = 0x%2.2X\n",
					     ic_data->vendor_panel_ver);
			...
			if (ic_data->HX_INT_IS_EDGE) {
				ret +=
				    snprintf(temp_buf + ret, len - ret, "Driver register Interrupt : EDGE TIRGGER\n");
			} else {
				ret +=
				    snprintf(temp_buf + ret, len - ret, "Driver register Interrupt : LEVEL TRIGGER\n");
			}
			...
			ret +=
			    snprintf(temp_buf + ret, len - ret, "chip_monitor :%d\n",
...
static ssize_t himax_proc_FW_debug_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	int ret = 0;
	uint8_t loop_i = 0;
	uint8_t tmp_data[64];
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (himax_read_FW_status(cmd_set, tmp_data) == NO_ERR) {
			ret +=
			    snprintf(temp_buf + ret, len - ret, "0x%02X%02X%02X%02X :\t", cmd_set[5], cmd_set[4],
				     cmd_set[3], cmd_set[2]);
			...
			for (loop_i = 0; loop_i < cmd_set[1]; loop_i = loop_i + 2) {
				if ((loop_i % 16) == 0)
					ret += snprintf(temp_buf + ret, len - ret, "0x%02X%02X%02X%02X :\t",
							cmd_set[5], cmd_set[4],
							cmd_set[3] + (((cmd_set[2] + loop_i) >> 8) & 0xFF),
...
static ssize_t himax_proc_DD_debug_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	int ret = 0;
	uint8_t tmp_data[64];
	uint8_t loop_i = 0;
...
static ssize_t himax_proc_flash_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	ssize_t ret = 0;
	int loop_i;
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		for (loop_i = 0; loop_i < 1024; loop_i++) {
			ret +=
			    snprintf(temp_buf + ret, len - ret, "x%2.2X",
...
/*
static ssize_t himax_chip_self_test_store(struct device *dev,struct device_attribute *attr,
...
static ssize_t himax_sense_on_off_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
	char buf[80] = { 0 };
...
#ifdef HX_HIGH_SENSE
static ssize_t himax_HSEN_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	struct himax_ts_data *ts = private_ts;
	size_t ret = 0;
	char *temp_buf;
...
static ssize_t himax_HSEN_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
	struct himax_ts_data *ts = private_ts;
	char buf[80] = { 0 };
...
static ssize_t himax_SMWP_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	size_t ret = 0;
	struct himax_ts_data *ts = private_ts;
	char *temp_buf;
...
static ssize_t himax_SMWP_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
	struct himax_ts_data *ts = private_ts;
	char buf[80] = { 0 };
...
static ssize_t himax_GESTURE_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
	struct himax_ts_data *ts = private_ts;
	int i = 0;
	char buf[80] = { 0 };
...
static ssize_t himax_esd_cnt_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		ret +=
		    snprintf(temp_buf + ret, len - ret, "EB_cnt = %d, EC_cnt = %d, ED_cnt = %d\n", hx_EB_event_flag,
			     hx_EC_event_flag, hx_ED_event_flag);
		if (copy_to_user(buf, temp_buf, len))
			I("%s,here:%d\n", __func__, __LINE__);
		kfree(temp_buf);
		HX_PROC_SEND_FLAG = 1;
...
static ssize_t himax_esd_cnt_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
	int i = 0;
	char buf[12] = { 0 };
...
static ssize_t himax_proc_guest_info_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (himax_guest_info_get_status()) {
			ret += snprintf(temp_buf + ret,  sizeof(temp_buf) - ret, "Not Ready\n");
			if (copy_to_user(buf, temp_buf, len))
				I("%s,here:%d\n", __func__, __LINE__);
			kfree(temp_buf);
			HX_PROC_SEND_FLAG = 1;
		...
		for (j = 0; j < 4; j++) {
			ret += snprintf(temp_buf + ret,  sizeof(temp_buf) - ret, "g_guest_str%d:\n", j);
			for (i = 0; i < 128; i++) {
				if (i % 16 == 0 && i > 0)
					ret += snprintf(temp_buf + ret,  sizeof(temp_buf) - ret, "\n");
				ret += snprintf(temp_buf + ret,  sizeof(temp_buf) - ret, "0x%02X\t", g_guest_str[j][i]);

			}
			ret += snprintf(temp_buf + ret,  sizeof(temp_buf) - ret, "\n");
			I("str[%d] %s\n", j, g_guest_str[j]);
		}
		ret += snprintf(temp_buf + ret,  sizeof(temp_buf) - ret, "g_guest_str8:\n");
		for (i = 0; i < 128; i++) {
			if (i % 16 == 0 && i > 0)
				ret += snprintf(temp_buf + ret,  sizeof(temp_buf) - ret, "\n");
			ret += snprintf(temp_buf + ret,  sizeof(temp_buf) - ret, "0x%02X\t", g_guest_str[8][i]);
		...
		ret += snprintf(temp_buf + ret,  sizeof(temp_buf) - ret, "g_guest_str9:\n");
		for (i = 0; i < 128; i++) {
			if (i % 16 == 0 && i > 0)
				ret += snprintf(temp_buf + ret,  sizeof(temp_buf) - ret, "\n");
			ret += snprintf(temp_buf + ret,  sizeof(temp_buf) - ret, "0x%02X\t", g_guest_str[9][i]);
		}
		ret += snprintf(temp_buf + ret,  sizeof(temp_buf) - ret, "\n");
...
int himax_touch_proc_init(void)
{
	...
	himax_proc_attn_file = proc_create(HIMAX_PROC_ATTN_FILE, 0444, himax_touch_proc_dir, &himax_proc_attn_ops);
...
#ifdef HX_TP_PROC_SELF_TEST
	himax_proc_self_test_file = proc_create(HIMAX_PROC_SELF_TEST_FILE, 0644,
...
#ifdef HX_HIGH_SENSE
	himax_proc_HSEN_file = proc_create(HIMAX_PROC_HSEN_FILE, 0644,
...
#ifdef HX_SMART_WAKEUP
	himax_proc_SMWP_file = proc_create(HIMAX_PROC_SMWP_FILE, 0644,
					   himax_touch_proc_dir, &himax_proc_SMWP_ops);
	if (himax_proc_SMWP_file == NULL) {
		E(" %s: proc SMWP file create failed!\n", __func__);
		goto fail_14;
	}
	himax_proc_GESTURE_file = proc_create(HIMAX_PROC_GESTURE_FILE, 0644,
...
#ifdef HX_TP_PROC_SENSE_ON_OFF
	himax_proc_SENSE_ON_OFF_file = proc_create(HIMAX_PROC_SENSE_ON_OFF_FILE, 0644,
...
#endif
#ifdef HX_ESD_RECOVERY
	himax_proc_ESD_cnt_file = proc_create(HIMAX_PROC_ESD_CNT_FILE, 0644,
...
#endif
	himax_proc_CRC_test_file = proc_create(HIMAX_PROC_CRC_TEST_FILE, 0644,
...
#ifdef HX_TP_PROC_GUEST_INFO
	himax_proc_guest_info_file = proc_create(HIMAX_PROC_GUEST_INFO_FILE, 0644,
