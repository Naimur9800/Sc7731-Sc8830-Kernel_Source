...
void himax_flash_dump_func(struct i2c_client *client, uint8_t local_flash_command, int Flash_Size,
	...
	for (i = 0; i < 8; i++) {
		for (j = 0; j < 32; j++) {
			...
			for (k = 0; k < 32; k++) {
				...
				x44_command[2] = j;
				x44_command[3] = i;
				if (i2c_himax_write(client, x44_command[0], &x44_command[1],
...
int hiamx_parse_str2int(char *str)
{
	...
	for (i = 0; i < strlen(str); i++) {
		...
		/* str's the lowest char is the number's the highest number  So we should reverse
		this number before using the power function -1: starting number is from 0 ex:10^0 = 1,10^1=10 */
	}
	if (negtive_flag == 1) {
		result = 0 - result;
	}
...
uint32_t himax_selftest_print(uint8_t *output_buffer, uint16_t *mutual_bank, uint16_t *self_bank, uint32_t len,
			      int i, int j, int transpose, uint32_t ret)
{
...
uint32_t himax_selftest_inloop(uint8_t *output_buffer, uint16_t *mutual_bank, uint16_t *self_bank, uint32_t len,
			       int in_init, int out_init, bool transpose, int j, uint32_t ret)
{
	int i;
	int in_max = 0;
	uint8_t x_channel = getXChannel();
...
uint32_t himax_selftest_outloop(uint8_t *output_buffer, uint16_t *mutual_bank, uint16_t *self_bank, uint32_t len,
				int transpose, int out_init, int in_init, uint32_t ret)
{
	int j;
	int out_max = 0;
	int self_cnt = 0;
	...
	if (out_init > 0) {	/* bit1 = 1 */
		...
		for (j = out_init - 1; j >= 0; j--) {
			...
			ret =
			    himax_selftest_inloop(output_buffer, mutual_bank, self_bank, len, in_init, out_init,
						  transpose, j, ret);
			if (!transpose) {
				ret +=
				    snprintf(output_buffer + ret, len - ret, " %5d\n",
...
uint32_t himax_selftest_arrange(uint8_t *output_buffer, uint16_t *mutual_bank, uint16_t *self_bank, uint32_t len,
				uint32_t ret)
{
	int bit2, bit1, bit0;
	int i;
	...
	ret =
	    himax_selftest_outloop(output_buffer, mutual_bank, self_bank, len, bit2, bit1 * y_channel, bit0 * x_channel,
				   ret);
...
{
	uint32_t ret = 0;
	uint16_t mutual_num = 0, self_num, width = 0;
	uint8_t x_channel = getXChannel();
	uint8_t y_channel = getYChannel();
	int i = 0;
	int max_self = 0;
...
static uint8_t Self_Test_Bank(uint8_t *RB1H)
{
	...
	uint32_t bank_sum = 0, m = 0;
	uint8_t bank_avg = 0;
	uint8_t bank_ulmt[mutual_num];
	uint8_t bank_dlmt[mutual_num];
	uint8_t bank_min = 0, bank_max = 0;
	uint8_t slf_tx_fail_cnt = 0, slf_rx_fail_cnt = 0;
	uint16_t mut_fail_cnt = 0;
	int fail_flag = 0;
	uint8_t bank_val_tmp = 0;

	uint8_t set_bnk_ulmt[mutual_num];
	uint8_t set_bnk_dlmt[mutual_num];
	uint8_t set_avg_bnk_ulmt = 0;
	uint8_t set_avg_bnk_dlmt = 0;
...
void himax_register_write(struct i2c_client *client, uint8_t *write_addr, int write_length, uint8_t *write_data,
			  bool cfg_flag)
{
	uint8_t outData[4] = {0};
...
int himax_interface_on(struct i2c_client *client)
{
	return 0;
}
...
static void himax_changeIref(struct i2c_client *client, int selected_iref)
{
	...
	I("%s:cmd[0]=%d,cmd[1]=%d,temp_iref_1=%d,temp_iref_2=%d\n", __func__, cmd[0], cmd[1], temp_iref[iref_number][0],
	  temp_iref[iref_number][1]);
...
void himax_touch_information(struct i2c_client *client)
{
	...
	I("%s:HX_RX_NUM =%d,HX_TX_NUM =%d,HX_MAX_PT=%d,HX_X_RES =%d,HX_Y_RES =%d,HX_INT_IS_EDGE=%d\n", __func__,
	  ic_data->HX_RX_NUM, ic_data->HX_TX_NUM, ic_data->HX_MAX_PT, ic_data->HX_X_RES, ic_data->HX_Y_RES,
...
bool himax_ic_package_check(struct i2c_client *client)
{
	...
	if (cmd[0] == 0x05 && cmd[1] == 0x85 && (cmd[2] == 0x25 || cmd[2] == 0x26
...
void diag_parse_raw_data(struct himax_report_data *hx_touch_data, int mul_num, int self_num, uint8_t diag_cmd,
	...
	if (hx_touch_data->hx_rawdata_buf[0] == hx_touch_data->hx_rawdata_buf[1]
		...
		for (i = 0; i < hx_touch_data->rawdata_size; i++) {
			...
			if (temp1 < mul_num) {
				...
				self_data[i + index - mul_num] = hx_touch_data->hx_rawdata_buf[i + 4];
#if defined(HX_TP_SELF_TEST_DRIVER) || defined(CONFIG_TOUCHSCREEN_HIMAX_ITO_TEST)
				if (Selftest_flag == 1) {
		...
		if (Selftest_flag == 1) {
			...
			if (test_counter == TEST_DATA_TIMES) {
				...
				i2c_himax_write(private_ts->client, command_F1h_bank[0], &command_F1h_bank[1], 1,
						DEFAULT_RETRY_CNT);
				usleep_range(10000, 12000);
			}
		}
