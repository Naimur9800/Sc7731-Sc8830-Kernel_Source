...
static void sprd_bltc_rgb_init(struct sprd_leds_bltc_rgb *brgb)
{
        ...
        sci_adi_set(brgb->sprd_arm_module_en_addr + 0x0c, (0x1));//SW POWERDOWN DISABLE
        sci_adi_clr(brgb->sprd_arm_module_en_addr + 0x0c, (0x1));//CURRENT CONTROL DEFAULT
	sci_adi_clr(brgb->sprd_arm_module_en_addr + 0xa0, (0x1<<2));//WHTLED_SERIES_EN=1
	sci_adi_set(brgb->sprd_arm_module_en_addr + 0xa0, (0x1<<8));//WHTLED POWERDOWN ENABLE
...
{
        sci_adi_set(brgb->sprd_arm_module_en_addr + 0xec, (0x800)); 
...
//        PRINT_INFO("base_addr1:0x%1lx\n",base_addr1);	/***0x40038800***/
        ...
        for (i = 0; i < SPRD_LED_TYPE_TOTAL; i++) {
			...
			if((g_brgb_r == NULL) && (brgb != NULL)&& (i ==0))
			    g_brgb_r = brgb;
			if((g_brgb_g == NULL) && (brgb != NULL)&& (i ==1))
			    g_brgb_g = brgb;
                ...
                brgb->enable = 0;
#if CONFIG_OF
                brgb->sprd_bltc_base_addr = SPRD_ADI_BASE + 0x83c0;	/***0x40038440***/
