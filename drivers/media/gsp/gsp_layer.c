...
int gsp_layer_iommu_map(struct gsp_layer *layer, struct device *dev)
{
	...
	ret = gsp_layer_dmabuf_map(layer, dev);
	if (ret) {
		GSP_ERR("layer[%d] dma buf map failed\n",
			gsp_layer_to_type(layer));
		goto done;
	}
	...
	iommu_data.table = map->table;
	iommu_data.ch_type = SPRD_IOMMU_FM_CH_RW;

	GSP_DEBUG("gsp layer[%d] map iova size: %zu\n",
		  gsp_layer_to_type(layer), buf->size);
	ret = sprd_iommu_get_kaddr(dev, &iommu_data);
	if (ret) {
			 ...
			 struct device *dev)
{
	...
	if (buf->size != 0 && addr->addr_y != 0) {
		...
		iommu_data.ch_type = SPRD_IOMMU_FM_CH_RW;
		if (sprd_iommu_free_kaddr(dev, &iommu_data))
