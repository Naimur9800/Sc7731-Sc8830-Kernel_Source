...
*/
static struct COEF_ENTRY_T *gsp_r3p0_coef_cache_hit_check(
		struct gsp_r3p0_core *core,
...
static inline void gsp_r3p0_coef_cache_move_to_head(struct gsp_r3p0_core *core,
					   ...
					   struct GSC_MEM_POOL *pool_ptr)
{
	...
	if (NULL == pool_ptr) {
		GSP_ERR("GSP_Allocate:%d _Allocate error!\n", __LINE__);
		return NULL;
	}
...
static uint32_t CalYmodelCoef(uint32_t coef_length,
					  ...
					  struct GSC_MEM_POOL *pool_ptr)
{
	...
	mid_i		   = coef_length >> 1;
	d_filter[mid_i] = div64_s64_s64((int64_t) ((int64_t) N << GSC_FIX),
				(int64_t) MAX(M, N));
	for (i = 0; i < mid_i; i++) {
		int64_t angle_x =
			div64_s64_s64((int64_t) ARC_32_COEF *
		...
		int32_t value_x = r3p0_sin_32((int32_t) angle_x);
		int32_t value_y = r3p0_sin_32((int32_t) angle_y);
	...
	for (i = -1; i < mid_i; i++) {
		...
		int64_t t = a - ((b * r3p0_cos_32(angle_32)) >> 30);

		d_filter[mid_i + i + 1] = (t * d_filter[mid_i + i + 1]) >>
				GSC_FIX;
		d_filter[mid_i - (i + 1)] = d_filter[mid_i + i + 1];
	}
	for (i = 0; i < 8; i++) {
				 ...
				 struct GSC_MEM_POOL *pool_ptr)
{
	...
	if (1 == dir) {
		coef_length = (int16_t) (tap * 8);
		memset(y_coef_data_ptr, 0, coef_length*sizeof(int16_t));
		CalYmodelCoef(coef_length, y_coef_data_ptr, I, D, pool_ptr);
	} else {
		if (D > I) {
			coef_length = (int16_t) (tap * 8);
			memset(y_coef_data_ptr, 0, coef_length*sizeof(int16_t));
			CalYmodelCoef(coef_length, y_coef_data_ptr, I,
					D, pool_ptr);
		} else {
			coef_length = (int16_t) (4 * 8);
			memset(y_coef_data_ptr, 0, coef_length*sizeof(int16_t));
			CalYmodelCoef(coef_length, y_coef_data_ptr,
					I, D, pool_ptr);
		}
	}
...
static void GetFilter(int16_t pos_start,
			   int16_t *coef_data_ptr,
			   int16_t *out_filter,
			   int16_t iI_hor,
			   int16_t coef_len,
...
static void WriteScalarCoef(int16_t *scale_coef,
					 int16_t *coef_ptr,
					 int16_t len)
{
	int i, j;
	...
	for (i = 0; i < 8; i++) {
		for (j = 0; j < len; j++) {
			*(scale_coef + i * COEF_ARR_COL_MAX + j) =
...
static void _rearrang_coeff(void *src, void *dst, int32_t tap)
{
	uint32_t i, j;
	...
	memset((void *)dst_ptr, 0x00,
		COEF_ARR_ROWS*COEF_ARR_COL_MAX*sizeof(int16_t));
	...
	case 2: {
		for (i = 0; i < 8; i++) {
			for (j = 0; j < tap; j++)
				*(dst_ptr+i*COEF_ARR_COL_MAX + j) =
					*(src_ptr+i*COEF_ARR_COL_MAX+j);
		}
	}
	break;
...
static void ConfigRegister_BlockScalingCoef(int32_t *reg_scaling_hor,
				 int32_t iHorBufLen,
...
static void CheckCoefRange(int16_t *coef_ptr, int16_t rows,
		int16_t columns, int16_t pitch)
{
	int16_t i, j;
				 ...
				 uint32_t ver_tap)
{
	...
	if (core->cache_coef_init_flag == 1) {
		entry = gsp_r3p0_coef_cache_hit_check(core,
			i_w, i_h, o_w, o_h, hor_tap, ver_tap);
		if (entry) {
			gsp_r3p0_coef_cache_move_to_head(core, entry);
			return entry->coef;
		}
	}
	...
	if (NULL == cong_Ycom_hor
		|| NULL == cong_Ycom_ver
		|| NULL == coeff_array_hor
		|| NULL == coeff_array_ver) {
		return 0;
	}
	...
	GetFilter(pos_start, y_coef_data_x, Y_hor_filter, 8,
			coef_len, Y_hor_filter_len);
	...
	GetFilter(pos_start, y_coef_data_x, Y_ver_filter, 8,
			coef_len, Y_ver_filter_len);
	...
	scaling_reg_buf_hor = &core->coef_calc_buf[0];
	scaling_reg_buf_ver = &core->coef_calc_buf[SCALER_COEF_TAB_LEN_HOR];
	...
	if (core->cache_coef_init_flag == 1) {
		...
		if ((ulong)scaling_reg_buf_ver & MEM_OPS_ADDR_ALIGN_MASK
			|| (ulong)&entry->coef[32] & MEM_OPS_ADDR_ALIGN_MASK) {
			GSP_INFO("memcpy use none 8B alignment address!");
		}
		memcpy((void *)&entry->coef[32],
				(void *)scaling_reg_buf_ver, 16 * 4);
		gsp_r3p0_coef_cache_move_to_head(core, entry);
