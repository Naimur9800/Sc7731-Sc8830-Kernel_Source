...
int FMR_ASSERT(unsigned short *a)
{
	if ((a) == NULL) {
...
{
	FMR_ASSERT((unsigned short *)flags);
	*flags |= flag_mask;
	return 0;
}
...
void rds_get_rt(unsigned char *buf, unsigned char grp_type)
{
	...
	pr_info("RT Text A/B Flag is %d", text_flag);
	...
	pr_info("RT is %s", fmdev->rds_data.rt_data.textdata[3]);
}
...
void rds_get_ps(unsigned char *buf)
{
	...
	pr_info("blk2 =%d, blk4=%d", *blk_2, *blk_4);
/*	if ((*blk_2) == 1) { */
		...
		pr_info("The event is %x", fmdev->rds_data.event_status);
/*		} */
	pr_info("The PS is %s", fmdev->rds_data.ps_data.PS[3]);
	pr_info("blk4+1=%x", *(blk_4 + 1));
	pr_info("blk4+2=%x", *(blk_4 + 2));
...
void rds_get_af_method(unsigned char AFH, unsigned char AFL)
{
	...
	pr_info("af code is %d and %d", AFH, AFL);
	if (AFH >= RDS_AF_NUM_1 && AFH <= RDS_AF_NUM_25) {
		...
		/* Put AF freq fm buffer and check if AF freq is repeat again */
		for (indx = 1; indx < (num + 1); indx++) {
			if ((AFH == (fmdev->rds_data.af_data.AF[0][2*num-1]))
				...
				pr_info("AF same as");
				break;
			} else if (!(fmdev->rds_data.af_data.AF[0][2 *
				indx-1])) {
				/*convert to 100KHz */
			...
			for (indx2 = indx + 1; indx2 < (num + 1); indx2++) {
				...
				if (AFH > (fmdev->rds_data.af_data.AF[0][2*indx2
					...
					*indx2-1];
					...
					fmdev->rds_data.af_data.AF[0][2*indx2]
						= AFL;
				} else if (AFH == (fmdev->rds_data
				.af_data.AF[0][2 * indx2-1])) {
...
void rds_get_tp_ta(unsigned char *buf, unsigned char grp_type)
{
	...
	unsigned int *flag = &(fmdev->rds_data.RDSFLAG.flag_status);
	...
	if (grp_type == 0x0a || grp_type == 0x0B || grp_type == 0xFB) {
		...
		rds_flag_set(flag, RDS_FLAG_IS_TA);
	}
...
/* pi_code for version A, pi_code_b for version B */
	...
	if (pi_code == 0 && pi_code_b != 0)
		pi_code = pi_code_b;
	/* send pi_code value to global and copy to user space
	in read rds interface */
...
void dump_rx_data(unsigned char *buffer, unsigned char len)
{
	unsigned int i;
...
*/
void string_to_hex(unsigned char *buf_src, unsigned char *buf_des)
{
	...
	if (ret != 8)
		pr_info("sscanf converts into data error %d", ret);
	pr_info("new decimal data is :%d,%d,%d,%d,%d,%d,%d,%d",
		c1, b1, c2, b2, c3, b3, c4, b4);
	buf_des[0] = c1;
	buf_des[1] = b1 >> 8;
	buf_des[2] = b1 & 0xFF;
...
int rds_parser(unsigned char *buffer1, unsigned char len)
{
	...
	pr_info("group type is : 0x%x", grp_type);
	...
	/*ODA (Open Data Applications) group availability signaled in
	type 3A groups*/
	case 0x3B:
	case 0x4B:
	case 0x6A:
	...
	default:
		pr_info("rds group type[0x%x] not to be processed", grp_type);
		break;
	}
	return 0;
}
