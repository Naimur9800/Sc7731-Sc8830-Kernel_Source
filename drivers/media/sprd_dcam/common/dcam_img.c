...
LOCAL int sprd_img_setflash(uint32_t flash_mode, uint32_t flash_index)
{
	...
	case FLASH_CLOSE:
		...
		break;
	default:
		DCAM_TRACE("sprd_img_setflash unknow mode:flash_mode 0x%x\n",
			flash_mode);
		break;
	}
...
int dcam_read_flash_light_status(struct dcam_dev *dev)
{
	struct dcam_info *info = &dev->dcam_cxt;
	int ret = 0, nread = 0;
	unsigned char buff[20];
	unsigned char flash_light_status[4];
	mm_segment_t old_fs;
	char *flash_path = "/sys/class/flash_test/flash_test/flash_value";

	old_fs = get_fs();
	set_fs(KERNEL_DS);
	struct file *fp = filp_open(flash_path, O_RDONLY, 0);
	if (IS_ERR(fp)) {
		printk("open flash attribute file failed\n");
		set_fs(old_fs);
		return -1;
	} else {
		nread = vfs_read(fp, (char __user *)buff, sizeof(buff), &fp->f_pos);
		strncpy(flash_light_status, strpbrk(buff, ":") + 1, sizeof(flash_light_status));
		info->flash_light_status = simple_strtoull(flash_light_status, NULL, 16) & 0xff;
		DCAM_TRACE("flash_light_status: 0x%x\n", info->flash_light_status);
		ret = 0;
	}
	filp_close (fp, NULL);
	set_fs(old_fs);
	...
	while (1) {
		if (0 == down_interruptible(&dev->flash_thread_sem)) {
			if (dev->is_flash_thread_stop) {
				dcam_read_flash_light_status(dev);
				if ((dev->dcam_cxt.flash_light_status & FLASH_OPEN) != FLASH_OPEN)
					sprd_img_setflash(FLASH_CLOSE, 0);
				sprd_img_setflash(FLASH_CLOSE, 1);
...
LOCAL int sprd_img_tx_done(struct dcam_frame *frame, void* param)
{
	...
	ts = ktime_to_timespec(boot_time);
	...
	node.time.tv_usec = ts.tv_nsec / NSEC_PER_USEC;
	DCAM_TRACE("boottime, %ld %ld \n", node.time.tv_sec, node.time.tv_usec);
