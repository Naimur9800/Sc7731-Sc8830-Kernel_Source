...
#endif
static volatile ulong gsp_coef_force_calc = 0;
...
#endif
static volatile ulong s_suspend_resume_flag = 0;// 0 : resume, in ON state; 1: suspend, in OFF state
...
static int32_t GSP_Scaling_Coef_Gen_And_Config(volatile ulong* force_calc)
{
    uint8_t     h_tap = 8;
    uint8_t     v_tap = 8;
...
static int GSP_Map(void)
{
	...
	if(s_gsp_cfg.layer0_info.src_addr.addr_y == 0
		&&s_gsp_cfg.layer0_info.mem_info.share_fd){
	...
	if(s_gsp_cfg.layer1_info.src_addr.addr_y == 0
		&&s_gsp_cfg.layer1_info.mem_info.share_fd){
	...
	if(s_gsp_cfg.layer_des_info.src_addr.addr_y == 0
		&&s_gsp_cfg.layer_des_info.mem_info.share_fd){
                          ...
                          unsigned long arg)
{
    ...
    GSP_TRACE("%s:pid:0x%08x,cmd:0x%08x, io number 0x%x, param_size %zu \n",
                ...
                _IOC_NR(cmd),
                param_size);
    //printk("%s:GSP_IO_GET_CAPABILITY:0x%lx\n",__func__,GSP_IO_GET_CAPABILITY);
...
int32_t gsp_drv_probe(struct platform_device *pdev)
{
    ...
    gsp_base_addr = (unsigned long)ioremap_nocache(r.start, resource_size(&r));
    if(!gsp_base_addr)
	BUG();

#ifndef GSP_IOMMU_WORKAROUND1
#if defined(CONFIG_ARCH_SCX15) || defined(CONFIG_ARCH_SCX30G) || defined(CONFIG_ARCH_SCX35L)
	ret = of_property_read_u32(gsp_of_dev->of_node, "gsp_mmu_ctrl_base", (u32*)&gsp_mmu_ctrl_addr);

	if(0 != ret){
		printk("%s: read gsp_mmu_ctrl_addr fail (%d)\n", __func__, ret);
		return ret;
	}
       gsp_mmu_ctrl_addr = (ulong)ioremap_nocache(gsp_mmu_ctrl_addr,sizeof(uint32_t));
       if(!gsp_mmu_ctrl_addr)
		BUG();
#endif
#endif

    printk("gsp: irq = %d, gsp_base_addr = 0x%lx,\n", gsp_irq_num, gsp_base_addr);
#else
