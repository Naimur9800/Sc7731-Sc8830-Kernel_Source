...
int32_t isp_get_int_num(struct isp_node *node)
{
	...
	isr_mask = node->irq_val1 & (~BIT_7);
	REG_WR(ISP_INT_CLR0, isr_mask);
	isr_mask = node->irq_val2;
	REG_WR(ISP_INT_CLR1, isr_mask);
	isr_mask = node->irq_val3;
	REG_WR(ISP_INT_CLR2, isr_mask);
