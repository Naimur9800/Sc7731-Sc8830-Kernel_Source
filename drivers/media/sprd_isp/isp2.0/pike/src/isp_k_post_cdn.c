...
static int32_t isp_k_post_cdn_slice_size(struct isp_io_param *param)
{
	...
	ret = copy_from_user((void *)&start_row_mod4, param->property_param, sizeof(start_row_mod4));
	if (0 != ret) {
		printk("isp_k_post_cdn_slice_size: copy_from_user error, ret = 0x%x\n", (uint32_t)ret);
		return -1;
	}

	REG_WR(ISP_POSTCDN_START_ROW_MOD4, (start_row_mod4 & 0x3));
