...
/* [Kernel_13/23] */
	} else if (node->irq_val1 & BIT_15) {
		ret = isp_k_rawafm_switch_bq_buf(isp_private);
		if (ret){
			//node->irq_val1 &= ~BIT_15;
			printk("[3A Porting]enter isp_k_switch_bq_buf: [BIT_15] ret 0x%x \n", ret);
		}
	}
		printk("[3A Porting]enter isp_k_switch_bq_buf: ret 0x%x, irq_val1 0x%x \n", ret, node->irq_val1);
...
int isp_k_af_create_bq_thread(struct isp_k_private *isp_private)
{
	...
	sema_init(&isp_private->rawaf_type1_thread_sem, 0);
	isp_private->rawaf_type1_thread = kthread_run(isp_k_rawafm_type1_thread_loop, isp_private, "isp_rawafm_type1_thread");
	printk("[3A Porting]isp_k_af_create_bq_thread!\n");

	if (IS_ERR(isp_private->rawaf_type1_thread)) {
		printk("isp_create_rawaf_thread error!\n");
		return -1;
	}
...
int isp_k_af_stop_bq_thread(struct isp_k_private *isp_private)
{
	...
	if (isp_private->rawaf_type1_thread) {
		isp_private->is_rawaf_type1_thread_stop = 1;
		up(&isp_private->rawaf_type1_thread_sem);
		if (0 != isp_private->is_rawaf_type1_thread_stop) {
			while (cnt < 500) {
				cnt++;
				if (0 == isp_private->is_rawaf_type1_thread_stop)
					break;
		...
		isp_private->rawaf_type1_thread = NULL;
	    printk("[3A Porting]isp_k_af_stop_bq_thread: rawaf_type1_thread\n");
...
static int32_t isp_k_bq_enqueue_buf(struct isp_io_param *param, struct isp_k_private *isp_private)
{
	...
	case ISP_NODE_TYPE_RAWAEM:
		...
		break;
/* [Kernel_16/23] */
	case ISP_NODE_TYPE_RAWAFM:
		ret = isp_k_rawafm_enqueue_buf(isp_private, &bq_node);
		printk("[3A Porting]isp_k_bq_enqueue_buf: ISP_NODE_TYPE_RAWAFM cmd id:%d.\n", bq_node.type);
		break;
	default:
...
/* [Kernel_17/23] */
	case ISP_NODE_TYPE_RAWAFM: 
		ret = isp_k_rawafm_dequeue_buf(isp_private, &bq_node);
		printk("[3A Porting]isp_k_bq_dequeue_buf: ISP_NODE_TYPE_RAWAFM cmd id:%d.\n", bq_node.type);
		break;
	default:
...
/* [Kernel_18/23] */
	isp_buf_queue_init(&isp_private->af_type1_irq_queue);
	isp_buf_queue_init(&isp_private->af_type1_user_queue);
	isp_private->is_af_reserved_buf = 0;
	isp_private->is_rawaf_type1_schedule = 0;
	sema_init(&isp_private->rawaf_type1_schedule_sem, 0);
	printk("[3A Porting]isp_k_bq_init.\n");
	return 0;
}
