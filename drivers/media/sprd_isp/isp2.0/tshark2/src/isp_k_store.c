...
static int32_t isp_k_store_block(struct isp_io_param *param)
{
	...
	val = ((store_info.border.right_border & 0xFF) << 24) | ((store_info.border.left_border & 0xFF) << 16)
		| ((store_info.border.down_border & 0xFF) << 8) | ((store_info.border.up_border & 0xFF));
	REG_WR(ISP_STORE_BORDER, val);
