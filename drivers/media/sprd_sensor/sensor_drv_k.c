...
LOCAL int sensor_k_register_subdevs(struct platform_device *pdev)
{
	...
	printk("[kernel][%s %d]sensor register sub device E\n",__func__,__LINE__);
...
//+add by wdm
int set_mclk(uint32_t mclk , struct sensor_file_tag* p_file)
{
    int ret;
	//struct sensor_file_tag         *p_file;
	struct sensor_module_tab_tag   *p_mod = s_p_sensor_mod_tab;// platform_get_drvdata(_sensor_k_get_platform_device());
    ...
    if(mclk)
    {
        if (atomic_inc_return(&p_mod->total_users) == 1) {
            ret = clk_mm_i_eb(dn,1);
            printk("[kernel][%s %d]sensor clk_mm_i_eb ok %d\n", __FUNCTION__,__LINE__,ret);
        }
        mdelay(50);
        ret = _sensor_k_set_mclk( p_file, p_mod->of_node, mclk);
        printk("[kernel][%s %d]sensor set_mclk ok %d\n", __FUNCTION__,__LINE__,ret);
    }else
    {
        ret = _sensor_k_set_mclk( p_file, p_mod->of_node, 0);
        mdelay(50);
        printk("[kernel][%s %d]sensor set_mclk ok %d\n", __FUNCTION__,__LINE__,ret);
        ret = clk_mm_i_eb(dn,0);
        printk("[kernel][%s %d]sensor clk_mm_i_eb ok %d\n", __FUNCTION__,__LINE__,ret);
...
EXPORT_SYMBOL(set_mclk);

int set_clk_mm_i_eb(uint32_t enable)
{
    int ret;

    ret = clk_mm_i_eb( s_p_sensor_mod_tab->of_node, enable);
    return ret;
}
EXPORT_SYMBOL(set_clk_mm_i_eb);

int set_i2c_clk(uint32_t clock)
{
    int ret;
    struct sensor_file_tag *fd_handle ;

    ret = _sensor_k_set_i2c_clk(fd_handle,clock);

    return ret;
}
EXPORT_SYMBOL(set_i2c_clk);
//-add by wdm
int sensor_k_probe(struct platform_device *pdev)
{
    ...
    for (i = 0; i < SENSOR_DEV_MAX; i++) {
        ...
        ret = gpio_request(gpio_tab.pwn, "camera_pwd");
        if (ret) {
            tmp = 1;
            printk("sensor: gpio already request pwn %d %d.\n", gpio_tab.pwn, i);
        }
        ret = gpio_request(gpio_tab.reset, "camera_rest");
        if (ret) {
            tmp = 1;
