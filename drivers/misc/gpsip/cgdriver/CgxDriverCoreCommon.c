...
unsigned int executedebug0 = 0;
unsigned int executedebug1 = 0;
unsigned int executedebugrcv = 0;
unsigned int executestep = 0;
unsigned int cgblockNumber = 0;
unsigned int cgreceived = 0;

void cgxprint(void)
{
   static unsigned int rcvold = 0;
   if (executedebugrcv != rcvold) {
       printk("%s step %d , cnt %d", __FUNCTION__, executestep,
            executedebugrcv);
       rcvold = executedebugrcv;
   }
}
									   ...
									   TCgDriverStatus *pResults)
{
	...
	pResults->rc = ECgOk;
 #if 0   
    if((aRequest != 0x21) && (aRequest != 0x20))
        printk("CgxDriverExecute 0x%x, 0x%x\n", aRequest, exdebug);
    if((aRequest == 0x21) || (aRequest == 0x20))
        exdebug++;
#endif
	switch (aRequest)
	...
	case CGX_IOCTL_WAIT_RCV:
		...
		if ( !isBlockCanceled(pState, pControl->wait.blockNumber + 1))
		{
			...
			{
                executedebug0 +=1;
                cgblockNumber = pControl->wait.blockNumber;
                cgreceived = pState->transfer.blocks.received;
...
unsigned int interruptdebugcount = 0;
...
unsigned int intcodedebug = 0;
unsigned int intcodedebugcount = 0;
