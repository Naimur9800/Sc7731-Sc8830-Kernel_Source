...
TCgReturnCode CgxDriverDataReadyInterruptHandlerStart(void *pDriver)
{
	...
	{
        printk("cgx InterruptHandlerStart \n");
	...
	else
	{
		...
		#ifdef CGX_DRIVE_DMA_INT_GLOBAL_SYNC_OBJECT
                printk("cgx InterruptHandlerStart 1\n");
...
extern unsigned int executedebug0;
extern unsigned int executedebug1;
extern unsigned int dmastartdebug;
extern unsigned int dmastopdebug;
extern unsigned int executedebugrcv;
extern unsigned int intcodedebug;
extern unsigned int intcodedebugcount;
extern unsigned int cgblockNumber;
extern unsigned int cgreceived;
extern unsigned int interruptdebugcount;
...
for(;;){
	...
	//DBGMSG1( "Waiting for completion... Semaphore=%u", pDriverInfo->xferCompleted.done );
		...
		rv = wait_for_completion_timeout( &pDriverInfo->xferCompleted,
			timeout);
		if (rv != -ERESTARTSYS) {
			break;
		}
		if (rv == -ERESTARTSYS) {
    		printk( "CgxEndWait ERESTARTSYS=%d, 0x%x, 0x%x, 0x%x,0x%x dma:0x%x,0x%x\n", timeout, debugcount1, 
    		debugcount2, executedebug0, executedebug1, dmastartdebug, dmastopdebug);               
		}
}
	...
	if ((rv == 0) && (timeout > 0))
	{
		...
		printk( "CgxEndWait timeout=%d, 0x%x, 0x%x, 0x%x,0x%x \n", timeout, debugcount1, 
    		debugcount2, executedebug0, executedebug1);
		printk( "CgxEndWait timeout v 0x%x, cnt 0x%x:0x%x, rcv 0x%x, bl 0x%x:0x%x\n",
        		intcodedebug, intcodedebugcount,interruptdebugcount,
        		executedebugrcv, cgblockNumber, cgreceived);
	}
	else
	{
          if (timeout != 0) {
            debugcount2 += 1;
#if 0
    		printk( "CgxEndWait ok time=%d, 0x%x, 0x%x, 0x%x,0x%x dma:0x%x,0x%x \n", timeout, 
    		debugcount1,debugcount2,executedebug0, executedebug1, dmastartdebug, dmastopdebug);
            printk( "CgxEndWait ok time=%d, 0x%x, 0x%x, 0x%x,0x%x \n", timeout, 
        		debugcount1,debugcount2,executedebug0, executedebug1);
            printk( "CgxEndWait ok value 0x%x, cnt 0x%x:0x%x,rcv 0x%x\n",
        		intcodedebug, intcodedebugcount,interruptdebugcount, executedebugrcv);
#endif
          }
	}
...
#if 0
TCgReturnCode CgxDriverTransferEndWait_Ext(void *pDriver, U32 aTimeoutMS,
       TCgDriverControl *pControl, TCgxDriverState *pState)
{
	DRVCONTEXT	 *pDriverInfo = (DRVCONTEXT *)pDriver;
	TCgReturnCode   rc = ECgOk;
	unsigned long   timeout = aTimeoutMS * HZ / 1000;
    ...
    for(;;) {
        if (pDriverInfo->state.flags.resume == 1) {
    		...
    		if (rv == -ERESTARTSYS) {
        		printk( "CgxEndWait ERESTARTSYS=%d, 0x%x, 0x%x, 0x%x,0x%x dma:0x%x,0x%x\n",
                        timeout, debugcount1, debugcount2, executedebug0,
                        executedebug1, dmastartdebug, dmastopdebug);
    		}
    }
	...
	if ((rv == 0) && (timeout > 0))
	{
		...
		printk( "CgxEndWait timeout v 0x%x, cnt 0x%x:0x%x, rcv 0x%x, bl 0x%x:0x%x\n",
        		intcodedebug, intcodedebugcount,interruptdebugcount,
        		executedebugrcv, cgblockNumber, cgreceived);
		printk( "CgxEndWait timeout, blockN 0x%x:0x%x \n",
              pControl->wait.blockNumber, pState->transfer.blocks.received);
	}
	else
	{
          if (timeout != 0) {
