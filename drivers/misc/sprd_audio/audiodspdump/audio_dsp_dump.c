...
struct dsp_log_init_data {
	...
	char *dump_content;
	uint32_t dump_type;
	uint32_t usedmem_type;
	uint32_t usedmem_size;
	uint32_t dump_mem_addr;
	void *dump_mem_addr_v;
	void *dump1_v;
	uint32_t dump1_p;
	uint32_t dump1_bytes;
	void *dump2_v;
	uint32_t dump2_p;
	uint32_t dump2_bytes;
	void *dump3_v;
	uint32_t dump3_p;
	uint32_t dump3_bytes;
	uint8_t dst;
	uint8_t channel;
	uint32_t txblocknum;
...
struct dsp_log_sbuf {
	...
	void *dump_mem_addr_v;
	uint32_t usedmem_size;
	uint8_t header_buf[PACKET_HEADER_SIZE];
	int need_packed;
	int sn_number;
 ...
 */
static void audio_dsp_add_headers(uint8_t *buf, int len, uint8_t type,
				  uint32_t m_sn)
{
	int pkt_len = 0;
	uint32_t n = 0;
				  ...
				  uint32_t is_timeout)
{
	...
	if (!is_timeout || init->timeout_dump) {
		if ((init->dump1_v > 0) && init->dump1_bytes
			&& agdsp_can_access()) {
	...
	if ((init->dump2_v > 0) && init->dump2_bytes) {
		unalign_memcpy((char *)buf + bytes, init->dump2_v,
			       init->dump2_bytes);
		bytes += init->dump2_bytes;
	}

	if ((init->dump3_v > 0) && init->dump3_bytes && agdsp_can_access()) {
...
static int audio_dsp_audio_info_dump(void *private, uint32_t is_timeout)
{
	uint32_t bytes = 0;
		...
		char __user *buf, size_t count, loff_t *ppos)
{
	...
	if (audio_buf->need_packed) {
		...
		buf = (uint8_t *) buf + PACKET_HEADER_SIZE;
	} else {
		read_count = count;
	}
	...
	unsigned int cmd, unsigned long arg)
{
	...
	pr_info("audio_dsp_ioctl enter into");
	switch (cmd) {
	case DSPLOG_CMD_LOG_ENABLE:
		...
		ret = aud_send_cmd_no_wait(audio_buf->channel,
			SBLOCK_TYPE_TPLOG, (uint32_t)arg, 0, 0, 0);
		if (ret < 0) {
	...
	case DSPLOG_CMD_PCM_ENABLE:
		...
		ret = aud_send_cmd_no_wait(audio_buf->channel,
			SBLOCK_TYPE_DUMP, (uint32_t)arg, 0, 0, 0);
		if (ret < 0) {
	...
	case DSPLOG_CMD_DSPDUMP_ENABLE:
		audio_buf->dev_res->init->dump_enable = (uint32_t)arg;
		break;
	case DSPLOG_CMD_TIMEOUTDUMP_ENABLE:
		audio_buf->dev_res->init->timeout_dump = (uint32_t)arg;
		break;
	default:
		return -EINVAL;
	}
	...
	struct device *dev)
{
	...
	if (ret) {
		pr_info("%s no sprd-name\n", __func__);
		goto error;
	}

	ret = of_property_read_string(np, "sprd-dump-content",
		(const char **)&pdata->dump_content);
	if (ret) {
		pr_info("%s no sprd-dump-content\n", __func__);
		goto error;
	}
...
static int audio_dsp_probe(struct platform_device *pdev)
{
	...
	pr_info("peter1: audio_dsp_probe in\n");
	/* parse dt
...
late_initcall_sync(audio_dsp_init);
