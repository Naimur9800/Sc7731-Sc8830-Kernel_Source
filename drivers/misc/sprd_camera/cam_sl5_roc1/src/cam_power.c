...
#ifdef TEST_ON_HAPS
static void IO_REG_OWR(unsigned int addr, unsigned int mask)
{
	unsigned long  base_address = 0;
	unsigned int val;

	base_address = (unsigned long)ioremap_nocache(addr, 0x4);
	val = readl_relaxed((void *)(base_address));
	pr_info("%s addr:%x val = 0x%x\n", __func__, addr, val);
	val = val | mask;
	writel_relaxed(val, (void *)(base_address));
	val = readl_relaxed((void *)(base_address));
	pr_info("%s addr:%x val = 0x%x\n", __func__, addr, val);
...
static void  IO_REG_CLR(unsigned int addr, unsigned int mask)
{
	unsigned long  base_address = 0;
	unsigned int val;
...
static unsigned int IO_REG_RD(unsigned int addr)
{
	unsigned long  base_address = 0;
	unsigned int val;
...
#else
	pr_info("todo here parse power dts.\n");
#endif
	/* todo: power domain init from DTS */
	pr_info("%s: cam_pw_domain init ok", __func__);
	return ret;
}
EXPORT_SYMBOL(sprd_cam_pw_domain_init);
...
int sprd_cam_domain_eb(void)
{
	...
	/* todo: cam domain enable */
#ifdef TEST_ON_HAPS
	IO_REG_OWR(0x327d0000, BIT_9);
	IO_REG_CLR(0x327e0024, BIT_24|BIT_25);

	/* failed here */
	while (0) {
		if (!(IO_REG_RD(0x327E010c) & 0x7C00))
			break;
	}

	IO_REG_OWR(0x62200000, BIT_2|BIT_3|BIT_4|BIT_5|BIT_6|BIT_7|BIT_8);
	IO_REG_OWR(0x62200008, BIT_3|BIT_4|BIT_5|BIT_6);

	pr_info("\n[jack] %s done\n", __func__);
	return ret;
#else
	pr_info("todo here power enable camera.\n");
#endif
