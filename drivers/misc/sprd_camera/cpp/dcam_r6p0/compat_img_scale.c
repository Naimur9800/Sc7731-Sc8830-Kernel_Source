...
#define COMPAT_SCALE_IO_START
	_IOW(SCALE_IO_MAGIC, 0, struct compat_scale_frame_param_t)
			...
			struct compat_scale_frame_param_t __user *up)
{
	...
	if (!access_ok(VERIFY_READ, up,
		sizeof(struct compat_scale_frame_param_t)) ||
		copy_from_user(&kp->input_size, &up->input_size,
			sizeof(struct scale_size_t)) ||
		copy_from_user(&kp->input_rect, &up->input_rect,
			sizeof(struct scale_rect_t)) ||
		get_user(kp->input_format, &up->input_format) ||
		copy_from_user(&kp->input_addr, &up->input_addr,
			sizeof(struct scale_addr_t)) ||
		copy_from_user(&kp->input_endian, &up->input_endian,
			sizeof(struct scale_endian_sel_t)) ||
		copy_from_user(&kp->output_size, &up->output_size,
			sizeof(struct scale_size_t)) ||
		get_user(kp->output_format, &up->output_format) ||
		copy_from_user(&kp->output_addr, &up->output_addr,
			sizeof(struct scale_addr_t)) ||
		copy_from_user(&kp->output_endian, &up->output_endian,
			sizeof(struct scale_endian_sel_t)) ||
...
long compat_scale_k_ioctl(struct file *filp,
