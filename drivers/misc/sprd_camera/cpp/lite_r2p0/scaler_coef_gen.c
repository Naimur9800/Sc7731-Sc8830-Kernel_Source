		       ...
		       struct gsc_mem_pool *pool_ptr)
{
	...
	temp_addr = ((temp_addr + (1 << align_shift) - 1) >>
			      ...
			      struct gsc_mem_pool *pool_ptr)
{
	...
	if (buffer_ptr == (void *)((pool_ptr->used_size - buffer_size) +
			pool_ptr->begin_addr))
	...
	for (i = 0; i < i_ver_bak; i++) {
		...
		for (j = 1 - luma_ver_tap / 2; j <= luma_ver_tap / 2; j++) {
			...
			if (l <= 0) {
				...
				cong_ycom_ver[phase][j + luma_ver_tap / 2 - 1];
			} else {
				if (l >= d_ver_bak - 1)
...
int64_t div64_s64_s64(int64_t dividend, int64_t divisor)
{
	signed char sign = 1;
			      ...
			      short n, short m, struct gsc_mem_pool *pool_ptr)
{
	...
	for (i = -1; i < mid_i; i++) {
		int index0 =
			(int)div64_s64_s64(((i + 1) * out_size * 4096 * 128),
				(4 * MAX(in_size, out_size)));
		int index1 =
			(int)div64_s64_s64(((i + 1) * 4096 * 128),
				(4 * in_size));
		...
		d_filter_fix[mid_i + i + 1] =
			(int)div64_s64_s64(out_size * (54 * 1024 * 1024 -
				46 * (long long)cos_value),
				(MAX(in_size, out_size) * 100));
		else
		d_filter_fix[mid_i + i + 1] =
			(int)div64_s64_s64((long long)sin_value0 *
				(54 * 1024 * 1024 - 46 * (long long)cos_value),
				(in_size * sin_value1 * 100));
				  ...
				  unsigned int output_format)
{
	...
	pr_info("height_in %d wight_in %d hor_deci %d ver_deci %d\n",
			row_in, col_in, hor_deci, ver_deci);
	pr_info("height_deci, width_deci, h_out, w_out: %d, %d, %d, %d\n",
		row_deci, col_deci, row_out, col_out);

	/* yuv420 */
	if (input_format == SCALE_YUV420) {
		/* 420 to 420 */
		if (output_format == SCALE_YUV420) {
			/* down scaling */
			if (2 * row_out <= row_deci) {
				...
				ylabel = DOWN;
				uvlabel = DOWN;
				pr_info("cpp yuv420->yuv420 down\n");
			} else {
				y_ver_tap = 4;
				uv_ver_tap = 4;
				ylabel = UP;
				uvlabel = UP;
				pr_info("cpp yuv420->yuv420 up\n");
			}
		} else if (output_format == SCALE_YUV422) {
			/* 420 to 422 */
			if (2 * row_out <= row_deci) {
				...
				y_ver_tap = tap1;
				ylabel = DOWN;
				pr_info("cpp yuv420->yuv422 y down\n");
			} else {
				/* y channel up scaling  */
				y_ver_tap = 4;
				ylabel = UP;
				pr_info("cpp yuv420->yuv422 y up\n");
			}
			...
			if (2 * row_out_uv <= row_deci_uv) {
				...
				uv_ver_tap = tap2;
				uvlabel = DOWN;
				pr_info("cpp yuv420->yuv422 uv down\n");
			} else {
				/* uv channel up scaling */
				uv_ver_tap = 4;
				uvlabel = UP;
				pr_info("cpp yuv420->yuv422 uv up\n");
			}
		}
	} else if (input_format == SCALE_YUV422) {
		/* yuv422 */
		if (output_format == SCALE_YUV420) {
			/* 422 to 420 */
			...
			if (2 * row_out <= row_deci) {
				...
				y_ver_tap = tap1;
				ylabel = DOWN;
				pr_info("cpp yuv422->yuv420 y down\n");
			} else {
				/* y channel up scaling */
				y_ver_tap = 4;
				ylabel = UP;
				pr_info("cpp yuv422->yuv420 y up\n");
			}
			...
			if (2 * row_out_uv <= row_deci_uv) {
				...
				uv_ver_tap = tap2;
				uvlabel = DOWN;
				pr_info("cpp yuv422->yuv420 uv down\n");
			} else {
				/* uv channel up scaling */
				uv_ver_tap = 4;
				uvlabel = UP;
				pr_info("cpp yuv422->yuv420 uv up\n");
			}
		} else if (output_format == SCALE_YUV422) {
			/* 422 to 422 */
			if (2 * row_out <= row_deci) {
				...
				ylabel = DOWN;
				uvlabel = DOWN;
				pr_info("cpp yuv422->yuv422 down\n");
			} else {
				y_ver_tap = 4;
				uv_ver_tap = 4;
				ylabel = UP;
				uvlabel = UP;
				pr_info("cpp yuv422->yuv422 up\n");
			}
		}
	}
