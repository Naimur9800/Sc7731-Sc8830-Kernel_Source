...
static void scale_k_set_output_size(struct scale_drv_private *p)
{
	...
	if (cfg_parm->output_size.w > SCALE_FRAME_WIDTH_MAX ||
		...
		pr_info("invalid ouput size %d %d\n",
...
static int scale_k_calc_sc_size(struct scale_drv_private *p)
{
	...
	pr_info("sc_input_size %d %d, deci %d input_rect %d %d",
		  ...
		  struct scale_drv_private *p)
{
	...
	pr_info("in_size %d %d in_rect %d %d %d %d out_size %d %d sc_deci_val %d\n",
		...
		p->sc_deci_val);
	pr_info("in_addr 0x%x 0x%x out_addr 0x%x 0x%x\n",
