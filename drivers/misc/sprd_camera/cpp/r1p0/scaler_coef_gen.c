				...
				short dir, struct gsc_mem_pool *pool_ptr)
{
	...
	/* x direction */
	if (dir == 1) {
		coef_lenght = (short)(tap * phase);
		cal_y_model_coef(coef_lenght, y_coef_data_ptr, i, d, pool_ptr);
	} else {
		if (d > i) {
			/* down */
			coef_lenght = (short)(tap * phase);
			cal_y_model_coef(coef_lenght, y_coef_data_ptr, i, d,
					 pool_ptr);
		} else {
			/* up */
			coef_lenght = (short)(4 * phase);
			cal_y_model_coef(coef_lenght, y_coef_data_ptr, i, d,
					 pool_ptr);
		}
	}
...
static short cal_uv_scaling_coef(short tap,
				 ...
				 short dir, struct gsc_mem_pool *pool_ptr)
{
	short uv_coef_lenght;

	if (dir == 1) {
		/* x direction */
		uv_coef_lenght = (short)(tap * 8);
		cal_y_model_coef(uv_coef_lenght, uv_coef_data_ptr, i, d,
				 pool_ptr);
	} else {
		/* y direction */
		if (d > i)
				 ...
				 int flag, struct gsc_mem_pool *pool_ptr)
{
	...
	/* ch/char *coef_txt4 = alloc_mem(COUNT * sizeof(char), 0, pool_ptr); */
	/* ch/char *coef_txt5 = alloc_mem(COUNT * sizeof(char), 0, pool_ptr); */
				  ...
				  unsigned int temp_buf_size, unsigned char dec)
{
	...
	pr_info("row_in %d col_in %d hor_deci %d ver_deci %d\n",
			row_in, col_in, hor_deci, ver_deci);
	pr_info("row_deci, col_deci, row_out, col_out: %x, %x, %x, %x\n",
