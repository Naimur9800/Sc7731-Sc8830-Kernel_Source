...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "CPP_PATH2_PATH3: %d: %d " fmt, current->pid, __LINE__
	...
	(struct cpp_path2_path3_drv_private *p)
{
	...
	if (cfg_parm->input_rect.x > SCALE_FRAME_WIDTH_MAX ||
		...
		pr_debug("input size %d %d\n",
			cfg_parm->input_size.w, cfg_parm->input_size.h);
		pr_debug("input path2 rect %d %d %d %d\n",
			cfg_parm->input_rect.x, cfg_parm->input_rect.y,
			cfg_parm->input_rect.w, cfg_parm->input_rect.h);
		pr_debug("input path3 rect %d %d %d %d\n",
			cfg_parm->path3_input_rect.x,
			cfg_parm->path3_input_rect.y,
			cfg_parm->path3_input_rect.w,
			cfg_parm->path3_input_rect.h);

		reg_wr(p, CPP_PATH2_CFG4, 0);   /*set source offset to 0*/
		...
		val = cfg_parm->input_size.w | (cfg_parm->input_size.h << 16);
		...
		reg_wr(p, CPP_PATH3_CFG4, 0);   /*set source offset to 0*/
	...
	(struct cpp_path2_path3_drv_private *p)
{
	...
	if (cfg_parm->input_addr.mfd[0] == 0
		...
		pr_debug("path23 set src addr, direct_addr=%d\n",
		...
		reg_wr(p, CPP_PATH2_SRC_UV_BASE_ADDR,
		       p->iommu_src.iova[1]);
		reg_wr(p, CPP_PATH2_SRC_V_BASE_ADDR,
		       p->iommu_src.iova[2]);
			...
			(struct cpp_path2_path3_drv_private *p)
{
	...
	pr_info("path2_path3 set input y,u,v=(0x%x,0x%x,0x%x)\n",
	...
	(struct cpp_path2_path3_drv_private *p)
{
	...
	if (cfg_parm->path2_output_addr.mfd[0] == 0 ||
		...
		memcpy(p->iommu_dst_path2.mfd, cfg_parm->path2_output_addr.mfd,
		       3 * sizeof(unsigned int));
		pr_debug("path23 set dst addr2, direct_addr=%d\n",
				cfg_parm->isDirectVirAddr);
		...
		if (!cfg_parm->isDirectVirAddr) {
		    ...
		    cpp_get_addr(&p->iommu_dst_path2, SPRD_IOMMU_PF_CH_WRITE);
		} else {
		    p->iommu_dst_path2.iova[0] = cfg_parm->path2_output_addr.y;
		    p->iommu_dst_path2.iova[1] = cfg_parm->path2_output_addr.u;
		    p->iommu_dst_path2.iova[2] = cfg_parm->path2_output_addr.v;
		}
		...
		reg_wr(p, CPP_PATH2_DES_V_BASE_ADDR,
		       p->iommu_dst_path2.iova[2]);
	}
	...
	if (cfg_parm->path3_output_addr.mfd[0] == 0 ||
		...
		memcpy(p->iommu_dst_path3.mfd, cfg_parm->path3_output_addr.mfd,
		       3 * sizeof(unsigned int));
		pr_debug("path23 set dst addr3, direct_addr=%d\n",
				cfg_parm->isDirectVirAddr);
...
static int cpp_path3_k_calc_sc_size(struct cpp_path2_path3_drv_private *p)
{
	...
	if (cfg_parm->path3_input_rect.w >
	    ...
	    cfg_parm->path3_output_size.h) {
		pr_err("path3 invalid input rect %d %d, output size %d %d\n",
			cfg_parm->path3_input_rect.w,
			cfg_parm->path3_input_rect.h,
			cfg_parm->path3_output_size.w,
			cfg_parm->path3_output_size.h);
		  ...
		  struct cpp_path2_path3_drv_private *p)
{
	...
	/*reg_owr(p, CPP_PATH2_CFG0, CPP_PATH2_WORK_WITH_EPF);*/
	...
	pr_debug("in_size %d %d\n", p->cfg_parm.input_size.w,
		p->cfg_parm.input_size.h);
	pr_debug("path2 in_rect %d %d %d %d\n",
		p->cfg_parm.input_rect.x, p->cfg_parm.input_rect.y,
		p->cfg_parm.input_rect.w, p->cfg_parm.input_rect.h);
	pr_debug("path3_in_rect %d %d %d %d\n",
		p->cfg_parm.path3_input_rect.x, p->cfg_parm.path3_input_rect.y,
		p->cfg_parm.path3_input_rect.w, p->cfg_parm.path3_input_rect.h);
	pr_debug("out_size path2=%dx%d,path3=%dx%d,sc_deci_val %d\n",
...
void cpp_path2_path3_stop(struct cpp_path2_path3_drv_private *p)
{
	...
	reg_awr(p, CPP_PATH3_CFG0, ~(CPP_PATH3_WORK_WITH_PATH2|
		CPP_PATH3_WORK_WITH_PATH0));
	cpp_free_addr(&p->iommu_src, SPRD_IOMMU_PF_CH_READ);
	cpp_free_addr(&p->iommu_dst_path2, SPRD_IOMMU_PF_CH_WRITE);
