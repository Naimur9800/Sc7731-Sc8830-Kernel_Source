...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "CPP_PATH3: %d: %d " fmt, current->pid, __LINE__
...
static void cpp_path3_k_ahb_reset(struct cpp_path3_drv_private *p)
{
	reg_awr(p, CPP_PATH_START, (~CPP_PATH3_START_BIT));
}

static void cpp_path3_k_set_input_size(struct cpp_path3_drv_private *p)
{
	struct sprd_cpp_path3_cfg_parm *cfg_parm = &p->cfg_parm;
...
static void cpp_path3_k_set_input_rect(struct cpp_path3_drv_private *p)
{
	struct sprd_cpp_path3_cfg_parm *cfg_parm = &p->cfg_parm;
...
static void cpp_path3_k_set_input_format(struct cpp_path3_drv_private *p)
{
	struct sprd_cpp_path3_cfg_parm *cfg_parm = &p->cfg_parm;
...
static void cpp_path3_k_set_src_addr(struct cpp_path3_drv_private *p)
{
	struct sprd_cpp_path3_cfg_parm *cfg_parm = &p->cfg_parm;
	...
	if (cfg_parm->input_addr.mfd[0] == 0
		...
		pr_debug("path3 set src addr, direct_addr=%d\n",
...
#else
static void cpp_path3_k_set_src_addr_t32_load_image
			(struct cpp_path3_drv_private *p)
{
	struct sprd_cpp_path3_cfg_parm *cfg_parm = &p->cfg_parm;

	pr_info("path3 set input y,u,v=(0x%x,0x%x,0x%x)\n",
...
static void cpp_path3_k_set_input_endian(struct cpp_path3_drv_private *p)
{
	struct sprd_cpp_path3_cfg_parm *cfg_parm = &p->cfg_parm;
	unsigned int y_endian = 0;
	unsigned int uv_endian = 0;

	if (cfg_parm->input_endian.y_endian >= PATH3_ENDIAN_MAX ||
...
static void cpp_path3_k_set_output_size(struct cpp_path3_drv_private *p)
{
	struct sprd_cpp_path3_cfg_parm *cfg_parm = &p->cfg_parm;
	unsigned int val = 0x0;
...
static void cpp_path3_k_set_output_format(struct cpp_path3_drv_private *p)
{
	struct sprd_cpp_path3_cfg_parm *cfg_parm = &p->cfg_parm;
...
#if !defined(CPP_TEST_DRIVER)
static void cpp_path3_k_set_dst_addr(struct cpp_path3_drv_private *p)
{
	struct sprd_cpp_path3_cfg_parm *cfg_parm = &p->cfg_parm;
	...
	if (cfg_parm->output_addr.mfd[0] == 0 ||
		...
		memcpy(p->iommu_dst.mfd, cfg_parm->output_addr.mfd,
		       3 * sizeof(unsigned int));
		pr_debug("path3 set dst addr, direct_addr=%d\n",
				cfg_parm->isDirectVirAddr);
		...
		if (!cfg_parm->isDirectVirAddr) {
		    ...
		    cpp_get_addr(&p->iommu_dst, SPRD_IOMMU_PF_CH_WRITE);
		} else {
		    p->iommu_dst.iova[0] = cfg_parm->output_addr.y;
		    p->iommu_dst.iova[1] = cfg_parm->output_addr.u;
		    p->iommu_dst.iova[2] = cfg_parm->output_addr.v;
		}
...
#else
static void cpp_path3_k_set_dst_addr_t32_load_image
				(struct cpp_path3_drv_private *p)
{
	struct sprd_cpp_path3_cfg_parm *cfg_parm = &p->cfg_parm;
...
static void cpp_path3_k_set_output_endian(struct cpp_path3_drv_private *p)
{
	struct sprd_cpp_path3_cfg_parm *cfg_parm = &p->cfg_parm;
	unsigned int y_endian = 0;
	unsigned int uv_endian = 0;

	if (cfg_parm->output_endian.y_endian >= PATH3_ENDIAN_MAX ||
	    cfg_parm->output_endian.uv_endian >= PATH3_ENDIAN_MAX) {
...
static void cpp_path3_k_enable(struct cpp_path3_drv_private *p)
{
	reg_owr(p, CPP_PATH_EB, CPP_PATH3_EB_BIT);
}

static void cpp_path3_k_disable(struct cpp_path3_drv_private *p)
{
	reg_awr(p, CPP_PATH_EB, (~CPP_PATH3_EB_BIT));
}

static int cpp_path3_k_calc_sc_size(struct cpp_path3_drv_private *p)
{
	int i = 0;
...
static int cpp_path3_k_set_sc_coeff(struct cpp_path3_drv_private *p)
{
...
static int cpp_path3_k_cfg_scaler(struct cpp_path3_drv_private *p)
{
	int ret = 0;
...
#if 1
static void cpp_reg_trace(struct cpp_path3_drv_private *p)
{
#if 0 /*def SCALE_DRV_DEBUG*/
	unsigned long addr = 0;

	pr_info("CPP path3: Register list");
		  ...
		  struct cpp_path3_drv_private *p)
{
	...
	cpp_path3_k_ahb_reset(p);
	cpp_path3_k_enable(p);
	cpp_path3_k_set_input_size(p);
	cpp_path3_k_set_input_format(p);
#ifdef CPP_TEST_DRIVER
	cpp_path3_k_set_src_addr_t32_load_image(p);
#else
	cpp_path3_k_set_src_addr(p);
#endif
	cpp_path3_k_set_input_endian(p);
	cpp_path3_k_set_output_size(p);
	cpp_path3_k_set_output_format(p);
#ifdef CPP_TEST_DRIVER
	cpp_path3_k_set_dst_addr_t32_load_image(p);
#else
	cpp_path3_k_set_dst_addr(p);
#endif
	cpp_path3_k_set_output_endian(p);
	...
	ret = cpp_path3_k_cfg_scaler(p);
	if (ret) {
		pr_err("failed to start path3\n");
		return ret;
	}
	cpp_path3_k_set_input_rect(p);
	...
	reg_owr(p, CPP_PATH_START, CPP_PATH3_START_BIT);
	cpp_reg_trace(p);
	udelay(1);
...
{
	reg_awr(p, CPP_PATH_START, (~CPP_PATH3_START_BIT));
	cpp_path3_k_disable(p);
	cpp_free_addr(&p->iommu_src, SPRD_IOMMU_PF_CH_READ);
	cpp_free_addr(&p->iommu_dst, SPRD_IOMMU_PF_CH_WRITE);
}
