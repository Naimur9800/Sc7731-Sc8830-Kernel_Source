...
static void scale_k_set_src_addr(struct scale_drv_private *p)
{
	struct sprd_cpp_scale_cfg_parm *cfg_parm = &p->cfg_parm;
	...
	if (cfg_parm->input_addr.mfd[0] == 0
		...
		pr_debug("scale set input y,u,v=(0x%lx,0x%lx,0x%lx) direct_addr=%d\n",
			p->iommu_src.iova[0], p->iommu_src.iova[1],
			p->iommu_src.iova[2], cfg_parm->isDirectVirAddr);
...
#if !defined(CPP_TEST_DRIVER)
static void scale_k_set_dst_addr(struct scale_drv_private *p)
{
	struct sprd_cpp_scale_cfg_parm *cfg_parm = &p->cfg_parm;
	...
	if (cfg_parm->output_addr.mfd[0] == 0 ||
		...
		pr_debug("scale set dst y,u,v=(0x%lx,0x%lx,0x%lx) direct_addr=%d\n",
			p->iommu_dst.iova[0],
			p->iommu_dst.iova[1],
			p->iommu_dst.iova[2],
...
static void cpp_reg_trace(struct scale_drv_private *p)
{
	...
	pr_info("CPP scale: Register list");
