...
#include "isp_reg.h"
#include "../dcam_if_r3p0/dcam_drv.h"
#include "cam_iommu.h"
...
struct isp_module {
	...
	struct isp_sc_array *scl_array[ISP_MAX_COUNT];
};
...
struct isp_pipe_dev {
	...
	unsigned int is_offline_thread_stop;
	unsigned int is_wait_fmcu;
	unsigned int is_ynr_process;
	struct pfiommu_info pfinfo_3dnr[3];
	struct pfiommu_info pfinfo_ynr[2];
};
