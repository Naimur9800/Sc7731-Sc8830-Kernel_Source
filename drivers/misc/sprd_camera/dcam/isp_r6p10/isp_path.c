...
static int isp_path_offline_cowork_cap(struct isp_path_desc *scl0,
			    ...
			    struct isp_store_cce_desc *store_cce)
{
	...
	if (scl0->valid != 1) {
		...
		if (path->in_rect.w > ISP_SCL0_MAX_WIDTH) {
			scl0->dst.w = ISP_SCL0_MAX_WIDTH;
			      ...
			      struct isp_store_cce_desc *store_cce)
{
	...
	if (store_cce->valid) {
		...
		if (rtn) {
			pr_err("fail to get store cce param error\n");
			return rtn;
		}
	}
...
int isp_start_pre_proc(struct isp_path_desc *scl0,
		       struct isp_path_desc *pre,
		       struct isp_path_desc *vid,
		       struct isp_path_desc *cap,
		       struct isp_store_cce_desc *store_cce)
{
	...
	if (pre->valid) {
		if (pre->path_mode == ISP_PRE_ONLINE) {
			rtn = isp_path_online(pre, scl0);
			if (rtn) {
				pr_err("fail to config Preview online\n");
				return rtn;
			}
		} else if (pre->path_mode == ISP_PRE_OFFLINE) {
			rtn = isp_path_offline(pre, scl0, store_cce);
			if (rtn) {
				pr_err("fail to config Preview offline\n");
				return rtn;
			}
		} else {
			pr_err("fail to config Isp preview path mode\n");
			return -EINVAL;
		}
	}
	...
	if (vid->valid) {
		if (vid->path_mode == ISP_VID_ONLINE) {
			...
			if (pre->valid && pre->path_mode == ISP_PRE_ONLINE) {
				rtn = isp_path_online_cowork(pre, vid, scl0);
				if (rtn) {
					pr_err("fail to config Onlie cowork\n");
					return rtn;
				}
			}
		} else if (vid->path_mode == ISP_VID_OFFLINE) {
			rtn = isp_path_offline(vid, scl0, store_cce);
			if (rtn) {
				pr_err("Video offline config error\n");
				return rtn;
			}
		} else if (vid->path_mode == ISP_VID_ONLINE_CPP) {
			/* add isp video with cpp here */
		} else {
			pr_err("fail to config Isp video path mode\n");
			return -EINVAL;
		}
	}

	if (cap->valid) {
		rtn = isp_path_offline(cap, scl0, store_cce);
		if (rtn) {
		...
		if (pre->valid && vid->valid &&
			...
			rtn = isp_path_offline_cowork_cap(scl0, pre, vid, cap,
							  store_cce);
			if (rtn) {
...
int isp_storecce_set_next_frm(struct isp_module *module)
{
	...
	if (isp_buf_queue_read(p_buf_queue, &frame) == 0 &&
		...
		pr_err("fail to read isp store cce buf\n");
		return -EFAULT;
	}
	...
	pr_debug("store cce y_addr %x, uv_addr %x\n", yuv_addr[0], yuv_addr[1]);
	ISP_REG_WR(idx, yuv_reg[0], yuv_addr[0]);
