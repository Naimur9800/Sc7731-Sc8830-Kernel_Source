	...
	if (in_ptr->cap_slice_need) {
		if (input->width == output->width &&
			...
			*w = (input->width > SLICE_WIDTH_MAX) ?
			...
			if (output->width > SLICE_SCL_WIDTH_MAX) {
				tempw = (output->width + SLICE_SCL_WIDTH_MAX
					- 1) / SLICE_SCL_WIDTH_MAX;
				*w = input->width / tempw;
				*w = (*w > SLICE_WIDTH_MAX) ?
				SLICE_WIDTH_MAX : *w;
			} else {
	...
	struct slice_base_info *base_info)
{
	...
	for (i = 0; i < slice_total_row; i++) {
		for (j = 0; j < slice_total_col; j++) {
			...
			if (i != 0) {
				temp_win.start_row -= base_info->yuv_overlap_up;
				temp_overlap.overlap_up =
	...
	struct slice_context_info *cxt, unsigned int *fmcu_num)
{
	...
	pr_debug("isp is raw capture %d\n", in_ptr->is_raw_capture);
