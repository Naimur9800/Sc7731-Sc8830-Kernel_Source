...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "ISP_SLW: %d %d %s : "\
				...
				struct camera_frame *res_frame)
{
	...
	while (!slowmotion_frame_dequeue(p_queue, &slw_handle->p_from_queue)) {
		if (slw_handle->p_from_queue.is_reserved == 0) {
			rtn = isp_frame_dequeue(&slw_handle->p_from_queue
						.slw_queue,
				&frame);
...
int set_isp_fmcu_cmd_reg(enum isp_scl_id path_id, void *isp_handle)
{
	...
	if (rtn) {
		...
		if (slw_handle->p_from_embed.is_reserved == 0) {
			pr_err("fail to get p_from_embed\n");
			return rtn;
		}
	}
	rtn = set_pingpang_reg(idx, &slw_handle->p_from_embed);
	if (rtn) {
		pr_err("fail to set pingpang reg\n");
		return rtn;
	}
	...
	rtn = slowmotion_frame_enqueue(p_insert_queue,
				       &slw_handle->p_from_embed);
	if (rtn) {
		pr_err("fail to enqueue frame slowmotion queue\n");
		return rtn;
	}
	...
	enum isp_scl_id path_id, unsigned int buf_reserved)
{
	...
	if (unlikely(buf_reserved)) {
		...
		if (slowmotion_frame_dequeue(p_empty_queue,
			...
			pr_err("fail to dequeue frame slowmotion queue\n");
	...
	for (i = 0; i < ISP_SLW_FRM_NUM; i++) {
		...
		if (rtn) {
			pr_err("fail to fmcu slw frame addr\n");
			return rtn;
		}
...
int isp_fmcu_slw_queue_init(void *isp_handle)
{
	...
	p_reserved->fmcu_addr_phy = fmcu_slw->fmcu_addr_phy;
...
int isp_fmcu_slw_start(enum isp_scl_id path_id, void *isp_handle)
{
	...
	if (rtn) {
		pr_err("fail to set fmcu int reg\n");
		return rtn;
	}
...
int set_fmcu_slw_cfg(void *handle)
{
	...
	if (!fmcu_slice || !fmcu_slw) {
		pr_err("fail to get valid input ptr\n");
		return -ISP_RTN_PARA_ERR;
	}
	fmcu_slw->status = ISP_ST_STOP;
	fmcu_slw->fmcu_addr_vir = fmcu_slice->iommu_addr_vir;
	fmcu_slw->fmcu_addr_phy = fmcu_slice->iommu_addr_phy;

	rtn = isp_fmcu_slw_int_init(handle);
	if (rtn) {
		pr_err("fail to set fmcu slw int reg\n");
		return rtn;
	}

	rtn = isp_fmcu_slw_queue_init(handle);
	if (rtn) {
		pr_err("fail to set fmcu slw queue init\n");
		return rtn;
	}
...
exit:
	return rtn;
vzalloc_exit:
	pr_err("fail to alloc memory for fmcu slw\n");
	return rtn;
