		...
		struct isp_k_block *isp_k_param, enum isp_id idx)
{
	...
	for (i = 0, j = 0; i < ISP_PINGPANG_YUV_YGAMMA_NUM; i++, j += 4) {
		...
		ISP_REG_WR(idx, ybuf_addr + j + ISP_CH1_ADDR_OFFSET,
	...
	if (isp_k_param->yuv_ygamma_buf_id) {
		...
		ISP_REG_OWR(idx,
			ISP_YGAMMA_PARAM + ISP_CH1_ADDR_OFFSET, BIT_1);
	} else {
		ISP_REG_MWR(idx, ISP_YGAMMA_PARAM, BIT_1, 0);
		ISP_REG_MWR(idx,
			ISP_YGAMMA_PARAM + ISP_CH1_ADDR_OFFSET, BIT_1, 0);
	}
	...
	struct isp_k_block *isp_k_param, enum isp_id idx)
{
	...
	ISP_REG_MWR(idx, ISP_YGAMMA_PARAM + ISP_CH1_ADDR_OFFSET,
			BIT_0, ygamma_info.bypass);
