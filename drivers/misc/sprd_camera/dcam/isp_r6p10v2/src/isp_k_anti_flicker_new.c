...
static int isp_k_afl_new_bypass(struct isp_io_param *param, enum isp_id idx)
{
	...
	if (bypass)
		ISP_REG_OWR(idx, ISP_ANTI_FLICKER_NEW_PARAM0, BIT_0);
	else
		ISP_REG_MWR(idx, ISP_ANTI_FLICKER_NEW_PARAM0, BIT_0, 0);
					...
					enum isp_id idx)
{
	...
	ret = copy_from_user((void *)&afl_info, param->property_param,
			sizeof(afl_info));
	if (ret != 0) {
		pr_err("anti_flicker_new: copy error, ret=0x%x\n",
						(unsigned int)ret);
		return -EPERM;
	}
	...
	/* AFL new or old */
	ISP_REG_MWR(idx, ISP_COMMON_3A_CTRL0, BIT_1, 1 << 1);

	ISP_REG_MWR(idx, ISP_ANTI_FLICKER_NEW_PARAM0,
				BIT_1, afl_info.mode << 1);

	ISP_REG_MWR(idx, ISP_ANTI_FLICKER_NEW_PARAM0, 0xF << 4,
		afl_info.skip_frame_num << 4);

	ISP_REG_WR(idx, ISP_ANTI_FLICKER_NEW_PARAM1,
				(afl_info.afl_stepx & 0xFFFFFF));
	ISP_REG_MWR(idx, ISP_ANTI_FLICKER_NEW_PARAM2,
				0xFFFFFF, afl_info.afl_stepy);

	ISP_REG_MWR(idx, ISP_ANTI_FLICKER_NEW_PARAM2, 0xFF << 24,
		afl_info.frame_num << 24);

	ISP_REG_MWR(idx, ISP_ANTI_FLICKER_NEW_COL_POS,
				0xFFFF, afl_info.start_col);

	ISP_REG_MWR(idx, ISP_ANTI_FLICKER_NEW_COL_POS, 0xFFFF << 16,
		afl_info.end_col << 16);

	ISP_REG_WR(idx, ISP_ANTI_FLICKER_NEW_REGION0, afl_info.step_x_region);
	ISP_REG_WR(idx, ISP_ANTI_FLICKER_NEW_REGION1, afl_info.step_y_region);
	...
	ISP_REG_WR(idx, ISP_ANTI_FLICKER_NEW_SUM1, afl_glb_total_num);

	afl_region_total_num = get_afl_region_data_num(afl_info.step_y_region,
				afl_info.frame_num, afl_info.img_size.height);

	ISP_REG_WR(idx, ISP_ANTI_FLICKER_NEW_SUM2, afl_region_total_num);
...
int isp_k_anti_flicker_new_cfg_addr(unsigned long phy_addr, enum isp_id idx)
{
	int ret = 0;
	...
	ISP_REG_WR(idx, ISP_ANTI_FLICKER_NEW_REGION3, phy_addr);
...
int isp_k_cfg_anti_flicker_new(struct isp_io_param *param, enum isp_id idx)
{
	...
	if (!param) {
		pr_err("isp_k_cfg_anti_flicker_new: param is null error.\n");
		return -EPERM;
	}
