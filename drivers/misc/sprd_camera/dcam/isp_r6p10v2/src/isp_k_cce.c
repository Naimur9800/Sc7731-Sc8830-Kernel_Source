	...
	(struct isp_io_param *param, enum isp_id idx)
{
	...
	ret = copy_from_user((void *)&cce_info, param->property_param,
			sizeof(cce_info));
	if (ret != 0) {
		pr_err("cce: copy error, ret=0x%x\n", (unsigned int)ret);
		return -EPERM;
	}
	...
	if ((val == 0) || (val1 == 0)) {
		pr_err("cce_block: copy error, 0x%x, 0x%x, 0x%x, 0x%x\n",
...
int isp_k_cfg_cce(struct isp_io_param *param, enum isp_id idx)
{
	...
	if (!param) {
		pr_err("isp_k_cfg_cce: param is null error.\n");
		return -EPERM;
	}

	if (param->property_param == NULL) {
		pr_err("isp_k_cfg_cce: property_param is null error.\n");
		return -EPERM;
	}
	...
	case ISP_PRO_CCE_BLOCK_UV:
		break;
	default:
		pr_err("cce:cmd id:%d, not supported.\n", param->property);
		break;
	}
