...
static int isp_k_uvd_chk_sum_clr(struct isp_io_param *param, enum isp_id idx)
{
	int ret = 0;
	unsigned int val = 0;
	...
	if (ret != 0) {
		pr_err("isp_k_uvd_chk_sum_clr: copy error, ret = 0x%x\n",
			(unsigned int)ret);
		return -1;
	}
...
int isp_k_cfg_uvd(struct isp_io_param *param, enum isp_id idx)
{
	...
	case ISP_PRO_UVD_CHK_SUM_CLR:
		ret = isp_k_uvd_chk_sum_clr(param, idx);
		break;
	default:
