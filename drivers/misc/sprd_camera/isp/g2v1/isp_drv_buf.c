...
{
	if (queue == NULL) {
		pr_err("_isp_img_buf_queue_init: queue is null error.\n");
		return -EINVAL;
	}
			...
			struct isp_img_buf *buf)
{
	...
	if (queue == NULL || buf == NULL) {
		pr_err("isp_img_queue_write: queue is null error.\n");
		return -EINVAL;
	}
	...
	*queue->write++ = *buf;
	...
	if (queue->write == queue->read) {
		queue->write = ori_buf;
		queue->num--;
		pr_err("isp_img_queue_write:Warning,isp_img_queue is full\n");
	}
			...
			struct isp_img_buf *buf)
{
	...
	if (queue == NULL || buf == NULL) {
		pr_err("isp_img_queue_read: queue is null error\n");
		return -EAGAIN;
	}
	...
	if (queue->read != queue->write) {
		*buf = *queue->read++;
		...
		pr_err("_isp_img_buf_queue_read: Warning,isp_img_queue is empty\n");
		spin_unlock_irqrestore(&queue->lock, flag);
		return -EAGAIN;
	}
	spin_unlock_irqrestore(&queue->lock, flag);
...
uint32_t _isp_img_buf_queue_nodenum(struct isp_img_buf_queue *queue)
{
	...
	if (queue == NULL) {
		pr_err("_isp_img_buf_queue_nodenum: queue is null error\n");
		return -EAGAIN;
	}
...
{
	if (queue == NULL) {
		pr_err("_isp_frame_queue_init: queue is null error.\n");
		return -EINVAL;
	}
			...
			struct isp_img_frame *buf)
{
	...
	if (queue == NULL || buf == NULL) {
		pr_err("_isp_frame_queue_write: queue is null error.\n");
		return -EINVAL;
	}
	...
	*queue->write++ = *buf;
	...
	if (queue->write == queue->read) {
		queue->write = ori_buf;
		pr_err("_isp_frame_queue_write:Warning,image frame queue is full\n");
	}
			...
			struct isp_img_frame *buf)
{
	...
	if (queue == NULL || buf == NULL) {
		pr_err("_isp_frame_queue_read: queue is null error\n");
		return -EINVAL;
	}
			...
			struct img_statis_frame *buf)
{
	...
	*queue->write++ = *buf;
	...
	if (queue->write == queue->read) {
		queue->write = ori_buf;
		pr_err("isp_statis_queue_write:Warning,isp_img_queue is full\n");
	}
...
{
	if (queue == NULL) {
		pr_err("isp_irq_queue_init: queue is null error.\n");
		return -EINVAL;
	}
...
int32_t _isp_irq_queue_write(struct isp_queue *queue, struct isp_irq_info *node)
{
	...
	if (queue == NULL || node == NULL) {
		pr_err("isp_irq_queue_write: queue or node is null error %p %p\n",
				queue, node);
...
int32_t _isp_irq_queue_read(struct isp_queue *queue, struct isp_irq_info *node)
{
	...
	if (queue == NULL || node == NULL) {
		pr_err("isp_irq_queue_read: queue or node is null error %p %p\n",
				queue, node);
