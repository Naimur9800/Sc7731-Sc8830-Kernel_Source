				 ...
				 struct isp_pipe_dev *isp_pipeline)
{
	...
	ret =
	    copy_to_user(param->property_param, (void *)&buf_size,
			 sizeof(uint32_t));
	if (ret != 0) {
...
int32_t isp_capability(void *param, struct isp_pipe_dev *isp_pipeline)
{
	int32_t ret = 0;
	...
	ret =
	    copy_from_user((void *)&cap_param, (void *)param,
			   sizeof(cap_param));
	if (ret != 0) {
