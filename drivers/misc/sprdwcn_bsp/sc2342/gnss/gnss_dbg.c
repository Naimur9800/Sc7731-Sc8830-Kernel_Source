...
struct gnss_ring_t *gnss_rx_ring;
...
static bool gnss_ring_over_loop(struct gnss_ring_t *pring, u_long len, int rw)
{
	if (rw == GNSS_RING_R)
...
int gnss_memcpy_rd(char *dest, char *src, size_t count)
{
	return copy_to_user((void __user *)dest, src, count);
}

int gnss_memcpy_wr(char *dest, char *src, size_t count)
{
	return copy_from_user(dest, (void __user *)src, count);
}
...
static ssize_t gnss_dbg_read(struct file *filp, char __user *buf, size_t count,
			     loff_t *pos)
{
	return 0;
}
