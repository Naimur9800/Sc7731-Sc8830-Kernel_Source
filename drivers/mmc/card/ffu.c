...
int mmc_ffu_install(struct mmc_card *card, u8 *ext_csd)
{
	...
	pr_err("FFU check mode operation \n");
	if ( FFU_FEATURES(ext_csd[EXT_CSD_FFU_FEATURES])) {
	...
	/* read ext_csd */
	pr_err("FFU read ext_csd\n");
	err = mmc_send_ext_csd(card, ext_csd);
	pr_err("FFU read ext_csd reversion =%d\n",ext_csd[EXT_CSD_REV]);
	if (err) {
	...
	pr_err("FFU read EXT_CSD_FFU_STATUS %d\n",err);
	if (!err) {
	...
	u8 *data, int buf_bytes)
{
	...
	pr_err("FFU: time=%d\n", card->ext_csd.generic_cmd6_time);
	if (err) {
		pr_err("FFU: %s: error %d FFU mode is not supported\n",
			mmc_hostname(card->host), err);
		err = -EINVAL;
		goto exit;
	}
	...
	pr_err("FFU perform write, cmd->arg =%0x\n",cmd->arg);
