...
#endif
long
lte_concur_proc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    int ret = 0;
	...
	case LTE_CONCUR_REQ:
		...
		len = val->size;
		ret = wlan_cmd_req_lte_concur(0, (unsigned char *)val +
