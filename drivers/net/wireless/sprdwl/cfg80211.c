				   ...
				   struct cfg80211_connect_params *sme)
{
	...
	if (sme->crypto.n_akm_suites) {
		if (priv->roaming_offload) {
			if (sme->crypto.akm_suites[0] == WLAN_AKM_SUITE_PSK)
				key_mgmt = SPRDWL_AKM_SUITE_PSK;
			else if (sme->crypto.akm_suites[0] == WLAN_AKM_SUITE_8021X)
				key_mgmt = SPRDWL_AKM_SUITE_8021X;
			else if (sme->crypto.akm_suites[0] == WLAN_AKM_SUITE_FT_PSK)
				key_mgmt = SPRDWL_AKM_SUITE_FT_PSK;
			else if (sme->crypto.akm_suites[0] == WLAN_AKM_SUITE_FT_8021X)
				key_mgmt = SPRDWL_AKM_SUITE_FT_8021X;
			else if (sme->crypto.akm_suites[0] == WLAN_AKM_SUITE_WAPI_CERT)
				key_mgmt = SPRDWL_AKM_SUITE_WAPI_CERT;
			else if (sme->crypto.akm_suites[0] == WLAN_AKM_SUITE_WAPI_PSK)
				key_mgmt = SPRDWL_AKM_SUITE_WAPI;
		} else {
			if (sme->crypto.akm_suites[0] == WLAN_AKM_SUITE_PSK)
				   ...
				   struct key_params *params)
{
	...
	if (priv->roaming_offload) {
		if (params->cipher == WLAN_CIPHER_SUITE_PMK) {
			ret = sprdwl_set_roam_offload_cmd(vif->priv->sipc,
							  SPRDWL_ROAM_OFFLOAD_SET_PMK,
							  params->key, params->key_len);
			if (ret < 0)
				wiphy_err(wiphy, "%s Failed to add cipher key!\n",
					  __func__);
			return ret;
		}
	}
...
void sprdwl_event_connect_result_11R(struct sprdwl_vif *vif)
{
...
void sprdwl_event_connect_result(struct sprdwl_vif *vif)
{
	...
	priv = vif->priv;
	if (priv->roaming_offload)
		return sprdwl_event_connect_result_11R(vif);
