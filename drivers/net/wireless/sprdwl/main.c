...
#define CMD_ROAM_OFFLOAD		"OFFLOAD_ROAMING"
...
int sprdwl_priv_cmd(struct net_device *ndev, struct ifreq *ifr)
{
	...
	if (strnicmp(command, CMD_BLACKLIST_ENABLE,
		...
		int skip = strlen(CMD_ROAM_OFFLOAD) + 1;

		ret = kstrtoint(command + skip, 0, &value);
		if (ret)
			goto exit;
		netdev_info(ndev,
			    "%s, Received roaming offload command, value : %d\n",
			    __func__, value);
		if (value) {
			priv->roaming_offload = value;
			bytes_written =
			sprdwl_set_roam_offload_cmd(priv->sipc,
...
static int sprdwl_get_power_control_reason(const char *buf)
{
	...
	if (strstr(buf, "videostreaming") != NULL)
		ret = ONLINE_VIDEO;
	else if (strstr(buf, "thermal") != NULL)
		ret = THERMAL;
	else if (strstr(buf, "wifivoice") != NULL)
		ret = WIFIVOICE;
	return ret;
}
				  ...
				  const char *buf, size_t count)
{
	...
	if (reason == WIFIVOICE) {
		wiphy_info(priv->wiphy, "wifi voice state : %d\n", state);
		ret = sprdwl_set_wifi_voice_cmd(priv->sipc, state);
		if (ret)
			wiphy_err(priv->wiphy, "set wifi voice error\n");
		return count;
	}
	if (state) {
		if (priv->sprd_power_ctl == 0)
			do_work = true;
		priv->sprd_power_ctl = priv->sprd_power_ctl | (1<<reason);
	} else {
		priv->sprd_power_ctl = priv->sprd_power_ctl & ~(1<<reason);
		if (priv->sprd_power_ctl == 0)
			do_work = true;
	...
	if (!do_work) {
		wiphy_info(priv->wiphy, "do nothing,reason:%s\n", state ?
			   "power control is already on" :
			   "other scene need power control");
		return count;
	}
