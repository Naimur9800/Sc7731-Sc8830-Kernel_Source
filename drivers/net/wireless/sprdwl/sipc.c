...
int wlan_sipc_cmd_send(struct wlan_sipc *sipc, u16 len, u8 type, u8 id)
{
	...
	kt = ktime_get();
	send_num = (u16)(div_u64(kt.tv64, NSEC_PER_MSEC));
	send_buf->u.cmd.seq_no = cpu_to_le16(send_num);

	pr_debug(SIPC_PRI "send cmd [%d %d seq_no:%d]\n", id, type, send_num);
	/* sipc-sbuf ops */
...
int wlan_sipc_cmd_receive(struct wlan_sipc *sipc, u16 len, u8 id)
{
	...
	pr_debug(SIPC_PRI "recv cmd id %d rsp seq:%d\n", cmd_id, seq_no);
...
int sprdwl_set_roam_offload_cmd(struct wlan_sipc *sipc,  u8 sub_type,
				u8 *data, u8 len)
{
	struct wlan_sipc_data *send_buf = sipc->send_buf;
	struct wlan_sipc_roam_offload  *roam_data =
	    (struct wlan_sipc_roam_offload *)send_buf->u.cmd.variable;
	int ret;

	mutex_lock(&sipc->cmd_lock);

	roam_data->type = sub_type;
	roam_data->len = len;
	memcpy(roam_data->value, data, len);

	sipc->send_len = SPRDWL_CMD_HDR_SIZE +
			 sizeof(struct wlan_sipc_roam_offload) + len;
	sipc->recv_len = SPRDWL_CMD_RESP_HDR_SIZE;
	ret = sprdwl_cmd_send_recv(sipc, CMD_TYPE_SET,
				   WIFI_CMD_SET_ROAM_OFFLOAD);
...
int sprdwl_set_wifi_voice_cmd(struct wlan_sipc *sipc, u8 value)
{
	struct wlan_sipc_data *send_buf = sipc->send_buf;
	struct wlan_sipc_wifi_voice wifi_voice;
	int ret;

	mutex_lock(&sipc->cmd_lock);

	wifi_voice.state = value;
	memcpy(send_buf->u.cmd.variable, &wifi_voice, sizeof(wifi_voice));
	sipc->send_len = SPRDWL_CMD_HDR_SIZE + sizeof(wifi_voice);
	sipc->recv_len = SPRDWL_CMD_RESP_HDR_SIZE;
	ret = sprdwl_cmd_send_recv(sipc,
				   CMD_TYPE_SET, WIFI_CMD_SET_WIFIVOICE);
...
#endif
	if (get_gwifi_nvm_ap_filter_support())
		open->mode |= 0x40;

	if (get_gwifi_rssi_switch())
		open->mode |= 0x20;
...
static void wlan_sipc_event_rx_handler(struct sprdwl_priv *priv)
{
	...
	if (ret) {
		pr_err("%s Failed to receive sblock (%d)\n", __func__, ret);
		return;
	}
	...
	seq_no = le16_to_cpu(recv_buf->u.event.seq_no);
	...
	pr_debug(SIPC_PRI "%s event %d seq:%d\n", __func__, event_id, seq_no);
