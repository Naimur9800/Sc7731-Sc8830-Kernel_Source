...
#define SPRDWL_AKM_SUITE_WAPI		    4
...
struct wlan_sipc_roam_offload {
#define SPRDWL_ROAM_OFFLOAD_SET_PMK    1
#define SPRDWL_ROAM_OFFLOAD_SET_FLAG   2
	u8 type;
	u8 len;
	u8 value[0];
} __packed;
...
#define WLAN_EVENT_SBLOCK_NUM	2
...
extern int sprdwl_set_power_control_cmd(struct wlan_sipc *sipc,
					u8 value, u8 reason);
extern int sprdwl_set_wifi_voice_cmd(struct wlan_sipc *sipc, u8 value);
#endif
...
extern int get_gwifi_nvm_ap_filter_support(void);
extern int get_gwifi_rssi_switch(void);
extern int sprdwl_set_roam_offload_cmd(struct wlan_sipc *sipc,
				       u8 sub_type, u8 *value, u8 len);
#ifdef CONFIG_OF
