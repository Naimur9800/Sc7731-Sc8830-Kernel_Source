...
#define WIFI_AP_FILTER_SUPPORT   "wifi_ap_filter_support"
#define WIFI_RSSI_SWITCH   "wifi_rssi_switch"
...
static int g_wifi_ap_filter_support = 0;
static int g_wifi_rssi_switch = 0;
...
int get_gwifi_nvm_ap_filter_support(void)
{
	return g_wifi_ap_filter_support;
}

int get_gwifi_rssi_switch(void)
{
	return g_wifi_rssi_switch;
}
	...
	for (i = 0, p = 0; i < file_len; i++) {
		if (('\n' == *(p_buf + i)) || ('\r' == *(p_buf + i))
			...
			if (5 <= (i - p)) {
				...
				else if ((0 == strcmp(WIFI_AP_FILTER_SUPPORT, cmd.sprdwl))){
					printk(KERN_ERR "%s: wifi_filter support %d\n", __func__, cmd.par[0]);
					g_wifi_ap_filter_support = cmd.par[0];
				} else if ((0 == strcmp(WIFI_RSSI_SWITCH, cmd.sprdwl))){
					printk(KERN_ERR "%s: wifi_rssi_switch  %d\n", __func__, cmd.par[0]);
					g_wifi_rssi_switch = cmd.par[0];
				}
			}
			p = i + 1;
		}
