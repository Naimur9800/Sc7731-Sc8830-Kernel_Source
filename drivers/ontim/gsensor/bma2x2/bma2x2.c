...
static char gsensor_version[]="bma2x2_1.0";
static char gsensor_vendor_name[20]="bma2x2";
...
//+add by hzb
	int Range = BMA2X2_RANGE_2G;
	int RangeG = 2*980665;
	int valMax[3] = {128,128,128};
	s32 val[3];
	bma2x2_set_range(client,Range);
		switch (Range) {
		case BMA2X2_RANGE_2G:
			RangeG = 2*980665;
			break;
...
//-add by hzb
	...
	case 0:
		...
		acc->z = acc->z >> (sizeof(short)*8-(BMA2X2_ACC_Z12_LSB__LEN +
					BMA2X2_ACC_Z_MSB__LEN));
//+add by hzb
 		valMax[0]=1<<(BMA2X2_ACC_X12_LSB__LEN + BMA2X2_ACC_X_MSB__LEN-1);
		valMax[1]=1<<(BMA2X2_ACC_Y12_LSB__LEN + BMA2X2_ACC_Y_MSB__LEN-1);
 		valMax[2]=1<<(BMA2X2_ACC_Z12_LSB__LEN + BMA2X2_ACC_Z_MSB__LEN-1);
//-add by hzb
		break;
	case 1:
		comres = bma2x2_smbus_read_byte_block(client,
		...
		acc->z = acc->z >> (sizeof(short)*8-(BMA2X2_ACC_Z10_LSB__LEN +
					BMA2X2_ACC_Z_MSB__LEN));
//+add by hzb
 		valMax[0]=1<<(BMA2X2_ACC_X10_LSB__LEN + BMA2X2_ACC_X_MSB__LEN-1);
 		valMax[1]=1<<(BMA2X2_ACC_Y10_LSB__LEN + BMA2X2_ACC_Y_MSB__LEN-1);
 		valMax[2]=1<<(BMA2X2_ACC_Z10_LSB__LEN + BMA2X2_ACC_Z_MSB__LEN-1);
//-add by hzb
		break;
	case 2:
		comres = bma2x2_smbus_read_byte_block(client,
		...
		acc->z = acc->z >> (sizeof(short)*8-(BMA2X2_ACC_Z8_LSB__LEN +
					BMA2X2_ACC_Z_MSB__LEN));
//+add by hzb
 		valMax[0]=1<<(BMA2X2_ACC_X8_LSB__LEN + BMA2X2_ACC_X_MSB__LEN-1);
 		valMax[1]=1<<(BMA2X2_ACC_Y8_LSB__LEN + BMA2X2_ACC_Y_MSB__LEN-1);
 		valMax[2]=1<<(BMA2X2_ACC_Z8_LSB__LEN + BMA2X2_ACC_Z_MSB__LEN-1);
//-add by hzb
		break;
	case 3:
		comres = bma2x2_smbus_read_byte_block(client,
		...
		acc->z = acc->z >> (sizeof(short)*8-(BMA2X2_ACC_Z14_LSB__LEN +
					BMA2X2_ACC_Z_MSB__LEN));
//+add by hzb
 		valMax[0]=1<<(BMA2X2_ACC_X14_LSB__LEN + BMA2X2_ACC_X_MSB__LEN-1);
 		valMax[1]=1<<(BMA2X2_ACC_Y14_LSB__LEN + BMA2X2_ACC_Y_MSB__LEN-1);
 		valMax[2]=1<<(BMA2X2_ACC_Z14_LSB__LEN + BMA2X2_ACC_Z_MSB__LEN-1);
//-add by hzb
		break;
	default:
		break;
...
#endif
//+add by hzb
	//printk(KERN_INFO "sensor_type=%d,acc->x=%d,RangeG=%d,valMax[0]=%d\n",sensor_type,acc->x,RangeG,valMax[0]);
	val[0]=acc->x<<16;
	val[1]=acc->y<<16;
	val[2]=acc->z<<16;

	val[0]=(val[0]>>16)*(RangeG/valMax[0]) /1000 ;
	val[1]=(val[1]>>16)*(RangeG/valMax[1]) /1000 ;
	val[2]=(val[2]>>16)*(RangeG/valMax[2]) /1000 ;
...
#ifdef CONFIG_SENSORS_BMI058
	if (2 == data)
		data = C_BMI058_One_U8X;
	else if (1 == data)
		data = C_BMI058_Two_U8X;
#endif
...
static DEVICE_ATTR(fifo_mode, 0664,
		bma2x2_fifo_mode_show, bma2x2_fifo_mode_store);
static DEVICE_ATTR(fifo_framecount, 0664,
		bma2x2_fifo_framecount_show, NULL);
static DEVICE_ATTR(fifo_trig, 0664,
		bma2x2_fifo_trig_show, bma2x2_fifo_trig_store);
static DEVICE_ATTR(fifo_trig_src, 0664,
...
static DEVICE_ATTR(offset_x, 0664,
		bma2x2_offset_x_show,
		bma2x2_offset_x_store);
static DEVICE_ATTR(offset_y, 0664,
		bma2x2_offset_y_show,
		bma2x2_offset_y_store);
static DEVICE_ATTR(offset_z, 0664,
...
static DEVICE_ATTR(slope_no_mot_duration, 0664,
		bma2x2_slope_no_mot_duration_show,
			bma2x2_slope_no_mot_duration_store);
static DEVICE_ATTR(slope_no_mot_threshold, 0664,
...
static DEVICE_ATTR(softreset, 0664,
		...
		const struct i2c_device_id *id)
{
	...
	if (pdata) {
		if (pdata->irq_gpio_cfg && (pdata->irq_gpio_cfg(pdata,1) < 0)) {
			dev_err(&client->dev,
				"IRQ GPIO conf. error\n");
		}
	...
	if (pdata)
	{
		if ((pdata->irq_gpio_fst >=0)|| (pdata->irq_gpio_sec>=0))
		{
			...
			if ((pdata->irq_use_num ==1) ||(pdata->irq_use_num ==2))
			{
	...
	{
		err = request_irq(data->IRQ, bma2x2_irq_handler, IRQF_TRIGGER_RISING | IRQF_ONESHOT,
				"bma2x2", data);
		if (err)
