...
static char gsensor_version[]="lis3dh_1.0";
static char gsensor_vendor_name[20]="lis3dh";
...
#endif
	struct mutex enable_mutex;  //add by hzb
...
static ssize_t lis3dh_vendor_name_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	 return sprintf(buf, "lis3dh\n");
}
		...
		const char *buf, size_t count)
{
	...
	if (error)
		return error;
	acc->pdata->poll_interval = max(data,
						(unsigned long )acc->pdata->min_interval);
			    ...
			    const struct i2c_device_id *id)
{
	...
	*/
	...
	mutex_init(&acc->lock);
	mutex_lock(&acc->lock);
	mutex_init(&acc->enable_mutex); //add by hzb
	...
	if (GSENSOR_GINT1_GPI>=0) gpio_request(GSENSOR_GINT1_GPI, "GSENSOR_INT1");
	if (GSENSOR_GINT2_GPI>=0) gpio_request(GSENSOR_GINT2_GPI, "GSENSOR_INT2");
...
/*		 Create workqueue for IRQ.*/
	...
	atomic_set(&acc->enabled, 0);
//+add by hzb
	err = sysfs_create_group(&acc->input_dev->dev.kobj,&lis3dh_attribute_group); 
	if (err < 0){
...
err_free_irq2:
	if (acc->irq2) {
		...
		if (GSENSOR_GINT2_GPI>=0) gpio_free(GSENSOR_GINT2_GPI);
	}
...
err_free_irq1:
	if (acc->irq1) {
		...
		if (GSENSOR_GINT1_GPI>=0) gpio_free(GSENSOR_GINT1_GPI);
	}
err_mutexunlockfreedata:
//+modify by hzb
	mutex_destroy(&acc->enable_mutex); //add byhzb
	mutex_unlock(&acc->lock);
...
//+add by hzb		
		...
		lis3dh_acc_misc_data = NULL;
		mutex_unlock(&acc->lock);
		mutex_destroy(&acc->enable_mutex);
//-add byhzb
		kfree(acc->pdata);
		kfree(acc);
	}
