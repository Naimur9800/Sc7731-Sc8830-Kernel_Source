...
static char gsensor_version[]="mir3da_1.0";
static char gsensor_vendor_name[20]="mir3da";
...
#define MI_DATA(format, ...)            if(1){printk(KERN_ERR MI_TAG format "\n", ## __VA_ARGS__);}
#define MI_MSG(format, ...)             if(1){printk(KERN_ERR MI_TAG format "\n", ## __VA_ARGS__);}
#define MI_ERR(format, ...)             if(1){printk(KERN_ERR MI_TAG format "\n", ## __VA_ARGS__);}
#define MI_FUN                          if(1){printk(KERN_ERR MI_TAG "%s is called, line: %d\n", __FUNCTION__,__LINE__);}
...
#if  0//DEVICE_CREATE_MODE == DEVICE_CREATE_BYSELF
#define I2C_STATIC_BUS_NUM              (2) //define which I2C bus to connect
...
struct mir3da_data {
    ...
    //    struct mir3da_platform_data *pdata ;
#ifdef CONFIG_HAS_EARLYSUSPEND
...
//static unsigned char mir3da_direction_placement=MIR3DA_DIRECTION_NUM; // current soldered placement

static struct work_info m_work_info = {{0}};
static int    direction_remap = MIR3DA_DIRECTION_NUM;
...
// printk("MIR3DA wwwwwwww          %s,%d,direction_remap:%d\n",__func__,__LINE__,direction_remap);
...
// printk("MIR3DA wwwwwwww          %s,%d,ABS_X:%d,ABS_Y:%d,ABS_Z:%d\n",__func__,__LINE__,ABS_X,ABS_Y,ABS_Z);
...
static int  mir3da_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    INIT_DELAYED_WORK( &m_work_info.write_work, sensor_write_work );
        ...
        direction_remap = pdata->direction_placement ;
        mir3da->power_on = pdata->power_on;
        if (mir3da->power_on) mir3da->power_on(1);
