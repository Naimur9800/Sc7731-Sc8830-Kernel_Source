...
#define DRIVER_VERSION  "3.8.1_0205"
...
static char stk3311_prox_version[]="stk3311_1.0";
static char stk3311_prox_vendor_name[20]="stk_ps";
u16 stk3311_prox_min = 0;
u16 stk3311_prox_max = 0;
u16 stk3311_prox_test = 0;
    DEV_ATTR_DECLARE(als_prox)
    DEV_ATTR_DEFINE("version",stk3311_prox_version)
    DEV_ATTR_DEFINE("vendor",stk3311_prox_vendor_name)
    DEV_ATTR_VAL_DEFINE("prox_min",&stk3311_prox_min,ONTIM_DEV_ARTTR_TYPE_VAL_16BIT)
    DEV_ATTR_VAL_DEFINE("prox_max",&stk3311_prox_max,ONTIM_DEV_ARTTR_TYPE_VAL_16BIT)
    DEV_ATTR_VAL_DEFINE("prox_test",&stk3311_prox_test,ONTIM_DEV_ARTTR_TYPE_VAL_16BIT)
    DEV_ATTR_DECLARE_END;
    ONTIM_DEBUG_DECLARE_AND_INIT(als_prox,als_prox,8);


#if 0//def SPREADTRUM_PLATFORM
...
void stk_als_set_new_thd(struct stk3x1x_data *ps_data, uint16_t alscode)
{
    ...
    high_thd = alscode*110/100;// + stk_lux2alscode(ps_data, STK_ALS_CHANGE_THD);
    low_thd = alscode *90/100;//- stk_lux2alscode(ps_data, STK_ALS_CHANGE_THD);
    if (high_thd >= (1<<16))
        high_thd = (1<<16) -1;
    if (low_thd <0)
        low_thd = 0;
...
static void stk3x1x_proc_plat_data(struct stk3x1x_data *ps_data, struct stk3x1x_platform_data *plat_data)
{
	...
	ps_data->wait_reg = plat_data->wait_reg;	
    	printk(KERN_INFO "%s: ps_data->wait_reg :%d", __func__,ps_data->wait_reg);
...
//#endif	
	...
	w_reg = 0;
#ifndef STK_POLL_PS	
	w_reg |= 0x07;	//modify int mode STK_INT_PS_MODE
#else
	w_reg |= 0x01;		
#endif	
...
static int32_t stk3x1x_init_all_reg(struct stk3x1x_data *ps_data)
{
	...
	ps_data->psi = 0xFFFF;	
#else
         if (stk3311_prox_test)
         {
             ps_data->ps_thd_h =stk3311_prox_max;
             ps_data->ps_thd_l =stk3311_prox_min;
//printk(KERN_INFO "%s: ps_thd_h:%d,ps_thd_l:%d,line:%d\n", __func__,ps_data->ps_thd_h,ps_data->ps_thd_l,__LINE__);				
...
static int32_t stk3x1x_enable_ps(struct stk3x1x_data *ps_data, uint8_t enable, uint8_t validate_reg)
{
	...
	curr_ps_enable = ps_data->ps_enabled?1:0;	
         //printk(KERN_INFO "%s:stk3x1x_enable_ps : %d\n", __func__, enable);
    ...
    if(enable)
	{
		...
		ps_data->ps_enable_count=1;
		ps_data->ps_enabled = true;
#ifdef STK_CHK_REG		
		if(!validate_reg)		
		{
			ps_data->ps_distance_last = 10;
...
#endif /* #ifdef STK_CHK_REG */
		{
                                   ...
                                   usleep_range(4000, 5000);
                                    ...
                                    ps_data->ps_distance_last = near_far_state?10:0;
                                    //input_report_abs(ps_data->ps_input_dev, ABS_DISTANCE,  ps_data->ps_distance_last );
                                    //input_sync(ps_data->ps_input_dev);
                                    //wake_lock_timeout(&ps_data->ps_wakelock, 3*HZ);
                                    reading = stk3x1x_get_ps_reading(ps_data);
                                    stk3x1x_set_ps_thd_h(ps_data, 100);
                                    stk3x1x_set_ps_thd_l(ps_data, 100);			
		}
	}
	else
	{		
...
static int32_t stk3x1x_enable_als(struct stk3x1x_data *ps_data, uint8_t enable)
{
	...
	if(ps_data->ges_enabled)
	{
		...
		return 0;
         ...
         //printk(KERN_INFO "%s:stk3x1x_enable_als : %d\n", __func__, enable);
...
static int32_t stk3x1x_get_als_reading(struct stk3x1x_data *ps_data)
{
	...
	//printk(KERN_ERR "%s: als_data=%d,line:%d\n",__func__, als_data,__LINE__);	
	...
	if(ps_data->p_wv_r_bd_with_co & 0b010)
	{
		//printk(KERN_ERR "%s: als_data=%d,line:%d\n",__func__, als_data,__LINE__);	
            	if(als_data < 30)
            	{
            		...
            		printk(KERN_ERR "%s: als_data=%d, als_code_last=%d,ir_data=%d,line:%d\n", 
            				__func__, als_data, ps_data->als_code_last, ir_data,__LINE__);	
            		if(ir_data > 40)
            		{
	...
	//printk(KERN_ERR "%s: als_data=%d,ps_data->als_code_last:%d,line:%d\n",__func__, als_data,ps_data->als_code_last ,__LINE__);	
...
static ssize_t stk_als_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    printk(KERN_INFO "%s: Enable ALS show: %d\n", __func__, ret);
...
static ssize_t stk_ps_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    printk(KERN_INFO "%s: Enable PS  show: %d\n", __func__, ret);
	return scnprintf(buf, PAGE_SIZE, "%d\n", ret);	
}
...
static struct device_attribute ps_distance_attribute = __ATTR(distance,0666,stk_ps_distance_show, stk_ps_distance_store);
...
#endif
static ssize_t stk3311_vendor_name_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	 return sprintf(buf, "stk_ps\n");
}

static DEVICE_ATTR(vendor_name, S_IRUGO,stk3311_vendor_name_show, NULL);

static DEVICE_ATTR(als_onoff,  0664 , stk_als_enable_show, stk_als_enable_store);

static DEVICE_ATTR(proximity_onoff,  0664 ,stk_ps_enable_show, stk_ps_enable_store);

static struct attribute *stk3311_attributes[] = {
...
static const struct attribute_group stk3311_attr_group = {
	.attrs = stk3311_attributes,
};
...
static void stk_als_poll_work_func(struct work_struct *work)
{
	...
	if(abs(ps_data->als_lux_last - reading_lux) >= STK_ALS_CHANGE_THD)
	{
		...
		input_sync(ps_data->als_input_dev);
#ifdef STK_DEBUG_PRINTF				
printk(KERN_INFO "%s: als input event %d lux,line:%d\n",__func__, reading_lux,__LINE__);		
#endif		
	}
	return;
}
...
static void stk_ps_poll_work_func(struct work_struct *work)
{
	...
	if(ps_data->ps_enabled)
	{
		...
		near_far_state = (org_flag_reg & STK_FLG_NF_MASK)?10:0;	
		reading = stk3x1x_get_ps_reading(ps_data);
		if(ps_data->ps_distance_last != near_far_state)
		{
			...
			wake_lock_timeout(&ps_data->ps_wakelock, 3*HZ);		
#ifdef STK_DEBUG_PRINTF		
printk(KERN_INFO "%s: ps input event %d cm, ps code = %d,line:%d\n",__func__, near_far_state, reading,__LINE__);		
#endif		
		}
...
static void stk_work_func(struct work_struct *work)
{
	...
	reading = stk3x1x_get_ps_reading(ps_data);
#ifdef STK_DEBUG_PRINTF	
printk(KERN_INFO "%s: ps input event %d cm, ps code = %d,line:%d\n",__func__, ps_data->ps_distance_last, reading,__LINE__);			
#endif	
#else
	/* mode 0x01 or 0x04 */	
	org_flag_reg = stk3x1x_get_flag(ps_data);
	if(org_flag_reg < 0)
		goto err_i2c_rw;	
//printk(KERN_ERR "%s: org_flag_reg= 0x%x,line:%d\n",__func__,org_flag_reg,__LINE__);			
...
printk(KERN_ERR  "%s: reading=%d, line:%d", __func__, reading,__LINE__);

		reading = reading * ps_data->als_correct_factor / 1000;
		ps_data->als_lux_last = stk_alscode2lux(ps_data, reading)/5;
		input_report_abs(ps_data->als_input_dev, ABS_MISC, ps_data->als_lux_last);
		input_sync(ps_data->als_input_dev);
        
printk(KERN_ERR "%s: reading=%d, lux : %d ,line:%d\n",__func__,reading, ps_data->als_lux_last,__LINE__);
    ...
    if (org_flag_reg & STK_FLG_PSINT_MASK)
    {
		...
		reading = stk3x1x_get_ps_reading(ps_data);
                    ...
                    disable_flag |= STK_FLG_PSINT_MASK;
                    reading = stk3x1x_get_ps_reading(ps_data);
                    if (stk3311_prox_test >1)
                    {
                        input_report_abs(ps_data->ps_input_dev, ABS_DISTANCE, reading);
                        //printk(KERN_INFO "%s:stk3311_prox_test 33333333 near_far_state=%d,line:%d,reading = %d\n",__func__, near_far_state,__LINE__, reading);
                        input_sync(ps_data->ps_input_dev);
                        wake_lock_timeout(&ps_data->ps_wakelock, 3*HZ);			
                    }
                    else
                    {
                            if(ps_data->ps_enable_count<=2)
                            {		
                                    near_far_state = 6 + ps_data->ps_enable_count;
                                    ps_data->ps_enable_count++;
                                //printk(KERN_INFO "%s: qqqqqqqqqqqq near_far_state=%d,line:%d,reading = %d\n",__func__, near_far_state,__LINE__, reading);
                            }
                            else if(reading >= ps_data->ps_thd_h)
                            {		
                                    near_far_state = 0;
                                    stk3x1x_set_ps_thd_h(ps_data, 0xFFFF);
                                    stk3x1x_set_ps_thd_l(ps_data, ps_data->ps_thd_l);			

                                //printk(KERN_INFO "%s: NNNNNNNNNNNNN near_far_state=%d,line:%d,reading = %d\n",__func__, near_far_state,__LINE__, reading);
                            }
                            else if(reading <= ps_data->ps_thd_l)
                            {
                                    near_far_state = 10;
                                    stk3x1x_set_ps_thd_h(ps_data, ps_data->ps_thd_h);
                                    stk3x1x_set_ps_thd_l(ps_data, 0);			

                                //printk(KERN_INFO "%s: FFFFFFFFFFFFFFFFF near_far_state=%d,line:%d,reading = %d\n",__func__, near_far_state,__LINE__, reading);
                            }
                            ps_data->ps_distance_last = near_far_state;
                            input_report_abs(ps_data->ps_input_dev, ABS_DISTANCE, near_far_state);
                            input_sync(ps_data->ps_input_dev);
 ...
 #endif
//printk(KERN_INFO "%s: xxxxxxxxxxxxx disable_flag=%d,line:%d,reading = %d\n",__func__, near_far_state,__LINE__, reading);
...
static int stk3x1x_set_input_devices(struct stk3x1x_data *ps_data)
{
    ...
    #else
    	ps_data->ps_input_dev = ps_data->als_input_dev;
    #endif
	...
	input_set_abs_params(ps_data->ps_input_dev, ABS_DISTANCE, 0,1, 0, 0);
    	...
    	ps_data->als_input_dev->name = ALS_NAME;//als_prox
                        ...
                        const struct i2c_device_id *id)
{
	...
	printk(KERN_INFO "%s: ps_data->als_transmittance :%d", __func__,ps_data->als_transmittance);
	...
	ps_data->power_on = plat_data->power_on;  //wg
	if (ps_data->power_on) ps_data->power_on(1);
	...
	if (sysfs_create_group(&ps_data->als_input_dev->dev.kobj, &stk3311_attr_group))
	{
