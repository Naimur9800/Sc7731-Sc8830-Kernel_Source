...
//+modify by hzb
                          if (dev_attr_array[i].dev_attr_type == ONTIM_DEV_ARTTR_TYPE_VAL_8BIT)
                              ...
                              ((void (*)(int ))(dev_attr_array[i].dev_attr_context))(-1);
...
static ssize_t ontim_core_debug_dev_attr_debug_trace_level_store(struct kobject *kobj, struct kobj_attribute *attr,
        const char *buf, size_t count)
{
    unsigned long  dbg_trace_level; 
    int res;   
    char *s_name ;
    struct ontim_debug *ontim_debug;
    mutex_lock(&sysfs_lock);
    res = strict_strtoul(buf, 10, &dbg_trace_level);
...
int ontim_dev_debug_file_exist(struct ontim_debug *ontim_debug)
{
    ...
    ontim_debug_sd = sysfs_get_dirent(ontim_core_dbg_kobj->sd, NULL, ontim_debug->dev_name);
    if (!ontim_debug_sd) {
        retval = -ENODEV;
...
//    ontim_debug->dbg_trace_level=0;
    ontim_debug_sd = sysfs_get_dirent(ontim_core_dbg_kobj->sd, NULL, ontim_core_dbg_item->name);
    if (ontim_debug_sd) {
	retval = -ENODEV;
			  ...
			  const char *buf, size_t count)
{
    ...
    mutex_lock(&sysfs_lock);
    if(strnstr(buf,"0x",2))
        res = strict_strtol(buf, 16, &gpio_force_num);
    else
	res = strict_strtol(buf, 10, &gpio_force_num);
			  ...
			  const char *buf, size_t count)
{
    ...
    mutex_lock(&sysfs_lock);
    if(strnstr(buf,"0x",2))
        res = strict_strtol(buf, 16, &gpio_value);
    else
	res = strict_strtol(buf, 10, &gpio_value);
