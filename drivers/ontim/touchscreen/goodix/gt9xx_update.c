...
#define GUP_SEARCH_FILE_TIMES       2   /* modify by hzb */
...
static u8 gup_burn_dsp_isp(struct i2c_client *client, struct goodix_ts_data *ts) //modify by hzb
{
    s32 ret = 0;
    u8* fw_dsp_isp = NULL;
    u8  retry = 0;
//+add by hzb
    ...
    if (ts)
    {
        if (ts->panel_parm)
        {
            if ((ts->panel_parm->panel_fw) && (ts->panel_parm->panel_fw_size)) 
            {
                from_file=0;
            }
        }
...
//+add by hzb
    if (from_file == 0)
    {
        memcpy(fw_dsp_isp,&(ts->panel_parm->panel_fw[FW_HEAD_LENGTH + (4*FW_SECTION_LENGTH+FW_DSP_LENGTH+FW_BOOT_LENGTH)]), FW_DSP_ISP_LENGTH); 
    }
    else
//-add by hzb
    {
...
static u8 gup_burn_fw_ss51(struct i2c_client *client, struct goodix_ts_data *ts) //modify by hzb
{
    u8* fw_ss51 = NULL;
    u8  retry = 0;
    s32 ret = 0;
...
//+add by hzb
    if (from_file == 0)
    {
        memcpy(fw_ss51,&(ts->panel_parm->panel_fw[FW_HEAD_LENGTH]), FW_SECTION_LENGTH); 
    }
    else
//-add by hzb
    {
...
//+add by hzb
    if (from_file == 0)
    {
        memcpy(fw_ss51,&(ts->panel_parm->panel_fw[FW_HEAD_LENGTH + FW_SECTION_LENGTH]), FW_SECTION_LENGTH); 
    }
    else
//-add by hzb
    {
...
//+add by hzb
    if (from_file == 0)
    {
        memcpy(fw_ss51,&(ts->panel_parm->panel_fw[FW_HEAD_LENGTH + 2*FW_SECTION_LENGTH]), FW_SECTION_LENGTH); 
    }
    else
//-add by hzb
    {
...
//+add by hzb
    if (from_file == 0)
    {
        memcpy(fw_ss51,&(ts->panel_parm->panel_fw[FW_HEAD_LENGTH + 3*FW_SECTION_LENGTH]), FW_SECTION_LENGTH); 
    }
    else
//-add by hzb
    {
...
static u8 gup_burn_fw_dsp(struct i2c_client *client, struct goodix_ts_data *ts) //modify by hzb
{
    s32 ret = 0;
    u8* fw_dsp = NULL;
    u8  retry = 0;
...
//+add by hzb
    if (from_file == 0)
    {
        memcpy(fw_dsp,&(ts->panel_parm->panel_fw[FW_HEAD_LENGTH + 4*FW_SECTION_LENGTH]),FW_DSP_LENGTH); 
    }
    else
//-add by hzb
    {
...
static u8 gup_burn_fw_boot(struct i2c_client *client, struct goodix_ts_data *ts) //modify by hzb
{
    s32 ret = 0;
    u8* fw_boot = NULL;
    u8  retry = 0;
...
//+add by hzb
    if (from_file == 0)
    {
        memcpy(fw_boot,&(ts->panel_parm->panel_fw[FW_HEAD_LENGTH + (4*FW_SECTION_LENGTH+FW_DSP_LENGTH)]), FW_BOOT_LENGTH); 
    }
    else
//-add by hzb
    {
...
s32 gup_update_proc(void *dir)
{
    ...
    while(retry++ < 5)
    {
        show_len = 10;
        total_len = 100;
        ret = gup_burn_dsp_isp(guitar_client,NULL); //modify by hzb
        if(FAIL == ret)
        {
        ...
        show_len += 10;
        ret = gup_burn_fw_ss51(guitar_client,NULL);//modify by hzb
        if(FAIL == ret)
        {
        ...
        show_len += 40;
        ret = gup_burn_fw_dsp(guitar_client,NULL);
        if(FAIL == ret)
        {
        ...
        show_len += 20;
        ret = gup_burn_fw_boot(guitar_client,NULL);//modify by hzb
        if(FAIL == ret)
        {
...
static u8 gup_ontim_check_update_fw(struct goodix_ts_data *ts, st_fw_head* fw_head)
{
    s32 i = 0;
    s32 fw_checksum = 0;
    u8 *buf=NULL;
	
    if ((ts->panel_parm->panel_fw==NULL) ||(ts->panel_parm->panel_fw_size ==0))
    { 
        GTP_ERROR("Not find FW !!!");
        return FAIL;
     }
    
    buf = ts->panel_parm->panel_fw;
    memcpy(fw_head, buf, FW_HEAD_LENGTH);
    buf+=FW_HEAD_LENGTH;
...
s32 gup_ontim_update_proc(struct goodix_ts_data *ts)
{
    s32 ret = 0;
    u8  retry = 0;
    st_fw_head fw_head;
//    struct goodix_ts_data *ts = NULL;
    ...
    GTP_DEBUG("[%s]Begin update ......",__func__);
    if ( ts == NULL)
    {
    ...
    ret = gup_ontim_check_update_fw(ts, &fw_head);
    if(FAIL == ret)
    {
        GTP_ERROR("[update_proc]check update file fail.");
        goto update_fail;
    ...
    while(retry++ < 5)
    {
        show_len = 10;
        total_len = 100;
        ret = gup_burn_dsp_isp(guitar_client,ts); //modify by hzb
        if(FAIL == ret)
        {
            GTP_ERROR("[update_proc]burn dsp isp fail.");
            continue;
        ...
        show_len += 10;
        ret = gup_burn_fw_ss51(guitar_client,ts);//modify by hzb
        if(FAIL == ret)
        {
            GTP_ERROR("[update_proc]burn ss51 firmware fail.");
            continue;
        ...
        show_len += 40;
        ret = gup_burn_fw_dsp(guitar_client,ts); //modify by hzb
        if(FAIL == ret)
        {
            GTP_ERROR("[update_proc]burn dsp firmware fail.");
            continue;
        ...
        show_len += 20;
        ret = gup_burn_fw_boot(guitar_client,ts); //modify by hzb
        if(FAIL == ret)
        {
            GTP_ERROR("[update_proc]burn bootloader firmware fail.");
            continue;
