...
static const uint32_t mipi_dsih_supported_versions[] = {0x100, 0x3132302A,
						0x3132312A, 0x3133312A};
static const uint32_t mipi_dsih_no_of_versions = sizeof(mipi_dsih_supported_versions) / sizeof(uint32_t);
/*
func:mipi_dsih_enable_hs_clk
desc: switch high speed clk to lp ,or lp to hs.
*/
void mipi_dsih_enable_hs_clk(dsih_ctrl_t * instance, int enable)
{
	mipi_dsih_dphy_enable_hs_clk(&(instance->phy_instance), enable);
}
...
void mipi_dsih_allow_return_to_lp(dsih_ctrl_t * instance, int hfp, int hbp, int vactive, int vfp, int vbp, int vsync)
{
    ...
    {
        instance->log_error("[mipi_dsih_allow_return_to_lp] invalid instance");
    }
...
void mipi_dsih_dcs_cmd_lp_transmission(dsih_ctrl_t * instance, int long_write, int short_write, int short_read)
{
    ...
    if (instance->log_error != 0)
    {
        instance->log_error("[mipi_dsih_dcs_cmd_lp_transmission] invalid instance");
    }
...
void mipi_dsih_gen_cmd_lp_transmission(dsih_ctrl_t * instance, int long_write, int short_write, int short_read)
{
    ...
    if (instance->log_error != 0)
    {
        instance->log_error("[mipi_dsih_gen_cmd_lp_transmission] invalid instance");
    }
...
#ifdef CONFIG_DSI_CTRL_R1P0
	mipi_dsih_hal_dpi_sig_delay(instance,
		95*(video_params->h_total_pixels * ratio_clock_xPF)
...
void mipi_dsih_cmd_mode(dsih_ctrl_t * instance, int en)
{
    ...
    if (instance->log_error != 0)
    {
        instance->log_error("[mipi_dsih_cmd_mode] invalid instance");
    }
...
void mipi_dsih_video_mode(dsih_ctrl_t * instance, int en)
{
    ...
    if (instance->log_error != 0)
    {
        instance->log_error("[mipi_dsih_video_mode] invalid instance");
    }
...
dsih_error_t mipi_dsih_gen_wr_packet(dsih_ctrl_t * instance, uint8_t vc, uint8_t data_type, uint8_t* params, uint16_t param_length)
{
	...
	{	/* long packet - write word count to header, and the rest to payload */
		...
		word_count = param_length;//(params[1] << 8) | params[0];
		/*
		...
		*/
		...
		{
			temp = 0;
			for (j = 0; (j < 4) && ((j + i) < (param_length)); j++)
	...
	*/
	...
	for (timeout = 0; timeout < DSIH_FIFO_ACTIVE_WAIT; timeout++)
	{
		...
		if (!mipi_dsih_hal_gen_cmd_fifo_full(instance))
		{
			...
			{
				err_code |= mipi_dsih_hal_gen_packet_header(instance, vc, data_type, param_length>>8, param_length&0xff);
			}
			break;
		}
	}
