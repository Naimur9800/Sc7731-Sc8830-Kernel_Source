...
dsih_error_t mipi_dsih_dphy_configure_hop_freq(dphy_t *phy, int freq_a, int delta);
...
extern void set_pll_frequency(dphy_t *phy, uint32_t output_freq);
extern void writel_nbits(dphy_t *phy, uint32_t value,
		uint8_t nbits, uint8_t offset, unsigned long addr);
#endif
