...
static int mipi_dsih_dphy_calc_pll_param(dphy_t *phy, struct dphy_pll *pll)
{
	...
	if (pll->hop_en) {
		if ((0 == pll->delta_freq) || (0 == pll->hopping_period))
			return 0;

		if (pll->delta_freq < 0) {
			delta = -pll->delta_freq;
			pll->sign = true;
		} else {
			delta = pll->delta_freq;
			pll->sign = false;
...
#ifdef CONFIG_DSI_CTRL_R1P0
		delta = delta * (i + 1);
		pll->kstep = pll->ref_clk * pll->hopping_period;
		pll->kdelta = (1 << 20) * delta / pll->ref_clk /
				pll->kstep + pll->sign * (1 << 11);
#else
		pll->kstep = pll->ref_clk * pll->hopping_period;
		pll->kdelta = (1 << 20) * delta * pll->out_sel
				/ pll->kstep / pll->ref_clk;
#endif
#ifdef CONFIG_DSI_CTRL_R1P0
	} else {
		delta = pll->freq / khz * 2 * (i + 1) * 15 / 1000;
		pll->kstep = pll->ref_clk * mhz * pll->hopping_period / mhz;
		pll->kdelta = (((1 << 20) * delta / pll->ref_clk /
				pll->kstep) - 3) / 4;
#endif
	}
...
FAIL:
	...
	phy->log_error("sprd-adf: failed to calculate dphy pll parameters\n");
...
static int mipi_dsih_dphy_start_hopping(dphy_t *phy)
{
	/* start hopping */
	regs._04.bits.hop_trig = !regs._04.bits.hop_trig;
	mipi_dsih_dphy_write(phy, 0x04, &regs._04.val, 1);
...
static int mipi_dsih_dphy_stop_hopping(dphy_t *phy)
{
	regs._03.bits.freq_hopping = 0;
	mipi_dsih_dphy_write(phy, 0x03, &regs._03.val, 1);
...
dsih_error_t mipi_dsih_dphy_ssc(dphy_t *phy, int freq, int mipi_ssc)
{
	...
	ret = mipi_dsih_dphy_calc_pll_param(phy, &pll);
	if (ret)
		phy->log_error("dphy: failed to set dphy pll registers\n");

	regs._06.bits.nint = pll.nint;
	regs._06.bits.mod_en = pll.mod_en;

	mipi_dsih_dphy_write(phy, 0x06, &regs._06.val, 1);
		...
		int freq_a, int delta)
{
	...
	pll.hop_en = true;
	pll.mod_en = false;
	pll.delta_freq = delta;
	pll.hopping_period = 200; /* 200us */

	mipi_dsih_dphy_config_pll(phy, freq_a);
	mipi_dsih_dphy_start_hopping(phy);

	delta_sum += delta;
	pr_info("delta_sum = %d, delta = %d\n", delta_sum, delta);

	if (delta_sum == 0) {
		mipi_dsih_dphy_stop_hopping(phy);
		pll.delta_freq = 0;
		pll.hop_en = false;
		pr_info("stop hoppin\n");
	}
				...
				u32 output_freq)
{
	...
	if (phy->bsp_pre_config)
		phy->bsp_pre_config(phy, NULL);

	pll.hop_en = false;
	pll.delta_freq = 0;
#ifdef CONFIG_DSI_CTRL_R1P0
	pll.sign = false;
	pll.hopping_period = 15;
