...
static int sprd_pmu_disable_irq(unsigned int cpu)
{
	struct device_node *pmu_np = NULL;
	int core_count = 0;
	int i = 0;
	unsigned int *pmu_irq = NULL;

	pmu_np = of_find_compatible_node(NULL, NULL, "arm,cortex-a7-pmu");
	if (!pmu_np) {
		pr_err("[%s]can not find arm pmu node\n", __func__);
		return -ENODEV;
	}

	core_count = of_irq_count(pmu_np);
	pr_info("[%s]core_count=%d\n", __func__, core_count);

	pmu_irq = kmalloc(core_count * sizeof(unsigned int), GFP_KERNEL);
	if (!pmu_irq) {
		pr_err("[%s]allocate memory for pmu irq pointer error\n",
			 __func__);
		return -ENOMEM;
	}

	for (i = 0; i < core_count; i++) {
		pmu_irq[i] = irq_of_parse_and_map(pmu_np, i);
		pr_info("[%s]pmu_irq[%d]=%d\n", __func__, i, pmu_irq[i]);
	}

	pr_info("disable cpu%d pmu irq\n", cpu);
	disable_irq(pmu_irq[cpu]);
	kfree(pmu_irq);
