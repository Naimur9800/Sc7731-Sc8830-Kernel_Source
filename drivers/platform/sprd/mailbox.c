...
static irqreturn_t mbox_recv_irqhandle(int irq_num, void *dev)
{
	...
	irq_sts = __raw_readl(REGS_RECV_MBOX_BASE + MBOX_FIFO_STS);
	irq_sts = irq_sts & 0x0000ff00;
	__raw_writel(irq_sts, REGS_RECV_MBOX_BASE + MBOX_IRQ_STS);
	irq_sts = (irq_sts) >> 8;
...
int mbox_raw_sent(u8 target_id, u64 msg)
{
	...
	while (__raw_readl((void __iomem *)(REGS_SEND_MBOX_BASE + MBOX_FIFO_STS)) & 0x4);
#if 0
...
int __init mbox_init(void)
{
	...
	if (ret) {
		printk(KERN_ERR "request mailbox irq failed %d\n", ret);
		return ret;
	}
