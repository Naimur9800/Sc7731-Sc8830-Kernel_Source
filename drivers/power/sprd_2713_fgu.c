...
/*wdm ---*/
unsigned int sprdfgu_get_rint()
{
    return sprdfgu_data.cur_rint;
}
/*---*/
...
static int __init fgu_cal_start(char *str)
{
	...
	if (str) {
		...
		/*wdm---*/
		if(fgu_data[0] == fgu_data[1] || fgu_data[0] == 0xffffffff || fgu_data[1] == 0xffffffff || fgu_data[2] == 0xffffffff) //yjg
                        return 1;
		/*---*/
...
uint32_t sprdfgu_read_vbat_ocv(void)
{
	...
	{
		/*wdm ---*/
		int cur_current=sprdfgu_read_batcurrent();
                //if(cur_current >0)
                //        sprdfgu_data.cur_rint = sprdfgu_data.charging_r;
                //else
                {
                        if(sprdfgu_data.pdata->rint_tab_size)
                       sprdfgu_data.cur_rint=sprdbat_interpolate(sprdbat_get_temp(), sprdfgu_data.pdata->rint_tab_size,
                                        sprdfgu_data.pdata->rint_tab);
                        else
                                sprdfgu_data.cur_rint = sprdfgu_data.discharging_r;
                }
                return sprdfgu_read_vbat_vol() -
                    (cur_current * sprdfgu_data.cur_rint) / 1000;
		/*---*/
	}
#else
	return sprdfgu_read_vbat_vol();
...
uint32_t ocv_buf[SPRDBAT_AVERAGE_COUNT];
uint32_t ocv_ave=0;
uint32_t sprdfgu_get_ocv_ave(void)
{
        return ocv_ave;
}
uint32_t sprdfgu_cal_ocv_ave(uint32_t ocv)
{
        static uint32_t cnt = 0,ocv_buf_full=0;
        uint32_t i, sum = 0;
...
uint32_t sprdfgu_read_capacity(void)
{
	...
	if (sprdfgu_data.pdata->fgu_mode) {
		...
		/*wdm*/
		if(ocv_ave = sprdfgu_cal_ocv_ave(voltage))
                        cap = sprdfgu_vol2capacity(ocv_ave);
                else
		/*wdm+*/
...
static void sprdfgu_hw_init(void)
{
	...
	}
	{
		/*wdm---*/
		uint32_t soft_ocv ;
                if(current_raw >0x2000)
                        sprdfgu_data.cur_rint = sprdfgu_data.charging_r;
                else
                        sprdfgu_data.cur_rint = sprdfgu_data.discharging_r;

                soft_ocv = sprdfgu_read_vbat_vol() -
                    (sprdfgu_adc2cur_ma
        ...
        FGU_DEBUG("soft_ocv = %d,cap = %d;%d;\n", soft_ocv, sprdfgu_data.init_cap, sprdfgu_data.cur_rint);
		/*---*/
	}
#endif
...
int sprdfgu_init(struct sprd_battery_platform_data *pdata)
{
	...
	/*wdm---*/
	sprdfgu_data.discharging_r = sprdfgu_data.pdata->discharging_rint;
        sprdfgu_data.charging_r = sprdfgu_data.pdata->charging_rint;
	/*---*/
