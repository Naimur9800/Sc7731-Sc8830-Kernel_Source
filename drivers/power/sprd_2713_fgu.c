...
int change_rint(int rint) 
{

    if (sprdfgu_read_batcurrent() < 0) {
        rint += 30;
    } else if (sprdfgu_read_batcurrent() >= 0) {
        rint += 155;
    }
...
#define VBAT_OCV_BUFF_CNT 10
extern int ocv_buff_clean;
static int vbat_ocv_buff[VBAT_OCV_BUFF_CNT] = {0};
static int sprdbat_get_vbat_ocv_from_buff(int vbat_ocv)
{
    static int pointer = 0;
	static int frist_time = 0;
    int i = 0, delta = 0;
    FGU_DEBUG(" vbat_ocv =%d\n",vbat_ocv);

	if ((frist_time == 0)||(ocv_buff_clean == 1)){
		for(i = 0; i<VBAT_OCV_BUFF_CNT; i++)
			vbat_ocv_buff[i] = vbat_ocv;
		frist_time = 1;
		ocv_buff_clean = 0;
		FGU_DEBUG("vbat_ocv_buff clean ,long time no update\n");
		return vbat_ocv;
    } else {
	    if(pointer >= VBAT_OCV_BUFF_CNT)
        pointer = 0;
	    vbat_ocv_buff[pointer++] = vbat_ocv;
    }
    for(i = 0; i < VBAT_OCV_BUFF_CNT; i++){
        delta += vbat_ocv_buff[i];
    }
	FGU_DEBUG("after avg vbat_ocv = %d \n",(delta/VBAT_OCV_BUFF_CNT));
    return (delta / VBAT_OCV_BUFF_CNT);
}

#define SPRDFGU_TEMP_RINT
...
#endif
    rint = change_rint(rint);
//	if (sprdfgu_read_vbat_ocv_pure(&vol)) {
//		FGU_DEBUG("hwocv...\n");
//		return vol;
//	} else {

	vbat_ocv =  sprdfgu_read_vbat_vol() -
		    (sprdfgu_read_batcurrent() * rint) / 1000;
return sprdbat_get_vbat_ocv_from_buff(vbat_ocv);
#else
...
int sprdfgu_cnom_state_change(int state)
{
	...
	case END_CNOM_RECORD:
		if (cnom_start_record != 0) {
			...
			cnom_start_record = 0;
			FGU_DEBUG("cnom_state_change  cnom_end %d cnum %d\n",
					cnom_end, temp);
		}
		break;
