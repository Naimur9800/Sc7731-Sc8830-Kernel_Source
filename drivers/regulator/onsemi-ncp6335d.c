...
#define NCP6335D_VOUT_SEL_MASK		GENMASK(6, 0)
#define NCP6335D_SLEW_MASK		GENMASK(4, 3)
#define NCP6335D_SLEW_SHIFT		GENMASK(1, 0)
...
{
	struct ncp6335d_platform_data *pdata;
	struct regulator_init_data *init_data;
	int rc;
	...
	rc = of_property_read_u32(client->dev.of_node,
				"onnn,slew-rate-ns", &pdata->slew_rate_ns);
	if (rc < 0) {
