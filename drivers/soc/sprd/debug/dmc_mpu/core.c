					   ...
					   char *buf)
{
	...
	if (core->interleaved)
		cnt += sprintf(buf + cnt,
			       "%s: 0x%08X%08X -mpuid: 0x%08X userid:0x%08X\n",
			       vio->wr ? "waddr" : "raddr",
			       vio->addr_h, vio->addr + core->ddr_addr_offset,
			       vio->id, vio->userid);
	else
		cnt += sprintf(buf + cnt,
			       "%s: 0x%08X -mpuid: 0x%08X userid:0x%08X\n",
			       vio->wr ? "waddr" : "raddr",
			       vio->addr + core->ddr_addr_offset,
			       vio->id, vio->userid);
...
static DEVICE_ATTR(config, S_IWUSR | S_IRUGO, sprd_dmc_mpu_core_cfg_show,
		   sprd_dmc_mpu_core_cfg_store);
static DEVICE_ATTR(channel, S_IRUSR | S_IRGRP, sprd_dmc_mpu_core_chn_show,
		   NULL);
static DEVICE_ATTR(dump_scene, S_IRUSR | S_IRGRP, sprd_dmc_mpu_core_dump_show,
		   NULL);
static DEVICE_ATTR(panic, S_IRUSR | S_IRGRP, sprd_dmc_mpu_core_panic_show,
		   NULL);
...
static irqreturn_t sprd_dmc_mpu_core_irq(int irq_num, void *dev)
{
	...
	dev_emerg(core->dev, "pub%d: chn%d: %s\n", mpu_info->pub_id,
		  vio->port, core->channel[vio->port]);
	if (core->interleaved)
		dev_emerg(core->dev, "%s: 0x%08X%08X - mpuid: 0x%08X userid:0x%08X\n",
			  vio->wr ? "waddr" : "raddr",
			  vio->addr_h, vio->addr + core->ddr_addr_offset,
			  vio->id, vio->userid);
	else
		dev_emerg(core->dev, "%s: 0x%08X - mpuid: 0x%08X userid:0x%08X\n",
			  vio->wr ? "waddr" : "raddr",
			  vio->addr + core->ddr_addr_offset,
			  vio->id, vio->userid);
...
static int sprd_dmc_mpu_core_get_config(struct sprd_dmpu_core *core)
{
	...
	dev_info(core->dev, "sprd_mpu->chn_num = %d\n", core->chn_num);
