/* linux/fs/iodebug/iodebug_hotpoint.c
 *
...
static unsigned int point_monitor_enable;
static unsigned int point_monitor_index;
...
static char *pointIndex2help(enum iodebug_point_e index)
{
	...
	for (i = 0; i < ARRAY_SIZE(point_table); i++) {
...
static struct iodebug_point_record *pointIndex2record(enum iodebug_point_e
									index)
{
	if ((index <= IODEBUG_POINT_MIN
		|| index >= IODEBUG_POINT_MAX)
		|| (IODEBUG_MONITOR_INDEX_SET
		&& (index != IODEBUG_MONITOR_INDEX_SET)))
		return NULL;
...
#define TEXT_FORMAT ("pid:%5d,        "            \
	...
	if (count  < IODEBUG_POINT_RECORD_NUM) {
		...
		for (i = 0; i < count; i++) {
			...
			memcpy(&data, &record->buff[i],
				sizeof(struct iodebug_point_monitor));
					...
					struct file *filp)
{
	...
	if (filp && filp->f_mapping && filp->f_mapping->host) {
		...
		if (inode && inode->i_sb && inode->i_sb->s_bdev
			...
			memcpy(info->disk_name,
				inode->i_sb->s_bdev->bd_disk->disk_name, 32);
	}
					...
					struct iodebug_point_monitor *info)
{
	...
	if ((info->flag != IODEBUG_MONITOR_FLAG) || (info->self != info)) {
		/*shoul never to here!*/
		pr_info("%s0x%x(0x%x,%s), flag:0x%x,self:0x%lx, info:0x%lx.\n",
			"iodebug: pid:",
			info->pid, current->pid, current->comm, info->flag,
			(unsigned long)(info->self), (unsigned long)info);
		return -1;
	}
	...
	spin_unlock_irqrestore(&record->lock, proc_flags);
	if (ret)
		pr_err("iodebug: fail to save info to debug recorder.\n");
...
static int show_point_monitor(struct seq_file *seq_f, void *v)
{
	...
	for (i = 0; i < ARRAY_SIZE(point_debug_record); i++) {
...
module_param_named(hotpoint_enable, point_monitor_enable,
					int, S_IRUGO | S_IWUSR);
module_param_named(hotpoint_index, point_monitor_index,
					int, S_IRUGO | S_IWUSR);
module_param_named(hotpoint_threshold, point_monitor_threshold,
					int, S_IRUGO | S_IWUSR);
module_param_array_named(hotpoint_pids, select_pid, int,
					&select_pid_size, S_IRUGO | S_IWUSR);
