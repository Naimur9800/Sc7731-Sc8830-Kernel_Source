 ...
 */
static unsigned long rtcc_threshold = 0;
static bool is_rtcc = false;
...
{
	if (isdigit(*str))
		rtcc_threshold = simple_strtoul(str, &str, 0);
	return 0;
}
early_param("rtcc", setup_rtcc_threshold);

unsigned long rtcc_threshold_or_default(void)
{
	unsigned long size;
...
static int __init rtcc_init(void)
{
	...
	if (totolram_size > threshold) {
		is_rtcc = false;
		pr_info("%s: MemTotal: %8lu MB > Threshold: %8lu MB."
			" Rtcc won't work\n)", __func__,
			totolram_size, threshold);
		return 0;
	}
	is_rtcc = true;
