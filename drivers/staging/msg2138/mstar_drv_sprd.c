...
/*=============================================================*/
/* #define MSG_TP_IC_NAME "pixcir_ts" //"msg21xxA" or "msg22xx" or "msg26xxM"
 Please define the mstar touch ic name based on the mutual-capacitive ic or
 self capacitive ic that you are using  */
/*=============================================================*/
/* VARIABLE DEFINITION */
/*=============================================================*/

struct i2c_client *gi2c_client;/* = NULL;*/
...
#ifdef CONFIG_ENABLE_REGULATOR_POWER_ON
struct regulator *g_reg_vdd;/* = NULL;*/
#endif/* CONFIG_ENABLE_REGULATOR_POWER_ON */
...
static struct msg2138_ts_platform_data *pixcir_ts_parse_dt(struct device *dev)
{
	...
	if (!pdata) {
		LOGTP_ERRO("Could not allocate struct platform_data");
		return NULL;
	}
	pdata->reset_gpio_number = of_get_gpio(np, 0);
	if (pdata->reset_gpio_number < 0) {
		LOGTP_ERRO("fail to get reset_gpio_number\n");
		goto fail;
	}
	pdata->irq_gpio_number = of_get_gpio(np, 1);
	if (pdata->irq_gpio_number < 0) {
		LOGTP_ERRO("fail to get irq_gpio_number\n");
		goto fail;
	}
	ret = of_property_read_string(np, "avdd-supply", &pdata->vdd_name);
	if (ret) {
		LOGTP_ERRO("fail to get avdd-supply\n");
		goto fail;
	}
	ret = of_property_read_u32_array(np, "virtualkeys",
			(unsigned int *)&pdata->virtualkeys, 12);

	if (ret) {
		LOGTP_ERRO("fail to get virtualkeys\n");
		goto fail;
	}
	ret = of_property_read_u32(np, "touchscreen-size-x", &pdata->TP_MAX_X);
	if (ret) {
		LOGTP_ERRO("fail to get touchscreen-size-x\n");
		goto fail;
	}
	ret = of_property_read_u32(np, "touchscreen-size-y", &pdata->TP_MAX_Y);
	if (ret) {
		LOGTP_ERRO("fail to get touchscreen-size-y\n");
		goto fail;
	}
...
static int  touch_driver_probe(struct i2c_client *client,/*__devinit*/
...
#ifdef CONFIG_ENABLE_REGULATOR_POWER_ON
	g_reg_vdd = regulator_get(&gi2c_client->dev, pdata->vdd_name);
#endif/* CONFIG_ENABLE_REGULATOR_POWER_ON */

	return interface_device_probe(gi2c_client, id);
...
/* remove function is triggered when the input device is removed from input
 sub-system */
static int  touch_driver_remove(struct i2c_client *client)/*__devexit*/
{
	LOGTP_FUNC();

	return ms_interface_device_remove(client);
}
...
static int  touch_driver_init(void)/*__init*/
{
	int ret;
	...
	if (ret < 0) {
		LOGTP_ERRO("add touch device driver i2c driver failed.\n");
		return -ENODEV;
	}
	LOGTP_INFO("add touch device driver i2c driver.\n");

	return ret;
}

static void  touch_driver_exit(void)/*__exit*/
{
	LOGTP_INFO("remove touch device driver i2c driver.\n");
