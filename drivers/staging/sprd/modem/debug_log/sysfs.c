			 ...
			 struct device_attribute *attr, char *buf)
{
	...
	if (dbg->clk_src[0]) {
		...
		for (i = 0; i < CLK_SRC_MAX; i++) {
			if (dbg->clk_src[i]) {
				ret = snprintf(temp_buf, 16, " %u",
				(unsigned int)(clk_get_rate(dbg->clk_src[i]) / 1000));
			  ...
			  const char *buf, size_t count)
{
	...
	if (dbg_log_is_freq_valid(dbg, freq) < 0) {
			    ...
			    struct device_attribute *attr, char *buf)
{
	...
	if (dbg->channel == CH_DISABLE)
		...
		ret =
		    snprintf(buf, PAGE_SIZE, "%s [ " STR_CH_DISABLE "%s]\n",
			     dbg->serdes.ch_str[dbg->channel - 1], temp_sbuf);
	else
		ret =
		    snprintf(buf, PAGE_SIZE, "UNKNOWN [ " STR_CH_DISABLE "%s]\n",
			     temp_sbuf);
