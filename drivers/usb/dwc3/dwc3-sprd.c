...
static int dwc3_sprd_is_connect_host(struct dwc3_sprd *sdwc)
{
	struct dwc3 *dwc = platform_get_drvdata(sdwc->dwc3);
	struct usb_charger *uchger = dwc->gadget.charger;
...
static inline unsigned int dwc3_sprd_adi_read(unsigned long reg)
{
	unsigned int val;
...
static enum usb_charger_type dwc3_sprd_get_charger_type(struct usb_charger *uchger)
{
	enum usb_charger_type type = SDP_TYPE;
	int charger_status, cnt = 10;

	while ((!(dwc3_sprd_adi_read(ANA_REG_GLB_CHGR_STATUS) &
		BIT_CHG_DET_DONE)) && cnt--) {
		msleep(200);
	}

	charger_status = dwc3_sprd_adi_read(ANA_REG_GLB_CHGR_STATUS) &
...
static int dwc3_sprd_start(struct dwc3_sprd *sdwc, enum usb_dr_mode mode)
{
	...
	if (mode == USB_DR_MODE_HOST) {
		 ...
		 */
		if (sdwc->vbus_irq > 0)
			disable_irq_nosync(sdwc->vbus_irq);
...
static int dwc3_sprd_stop(struct dwc3_sprd *sdwc, enum usb_dr_mode mode)
{
	...
	if (mode == USB_DR_MODE_PERIPHERAL)
		usb_gadget_set_state(&dwc->gadget, USB_STATE_REMOVED);
...
static void dwc3_sprd_hot_plug(struct dwc3_sprd *sdwc)
{
	...
	if (sdwc->vbus_active) {
		 ...
		 * device connection.
		 */
		if (mode == USB_DR_MODE_HOST && sdwc->vbus_irq > 0)
...
static int dwc3_sprd_ext_notifier(struct notifier_block *nb,
				  unsigned long action, void *data)
{
	struct dwc3_sprd *sdwc = container_of(nb, struct dwc3_sprd,
					      hot_plug_nb);
	unsigned long flags;
	int ret = 0;
	...
	case CABLE_STATUS_DEV_CONN:
		spin_lock_irqsave(&sdwc->lock, flags);
		if (sdwc->vbus_active == 1) {
			spin_unlock_irqrestore(&sdwc->lock, flags);
			dev_info(sdwc->dev, "ignore device connect from TYPEC.\n");
			return 0;
		}
		...
		if (sdwc->dr_mode != USB_DR_MODE_UNKNOWN) {
			...
			queue_work(system_unbound_wq, &sdwc->work);
			spin_unlock_irqrestore(&sdwc->lock, flags);
			dev_info(sdwc->dev, "missed disconnect event when TYPEC device connect.\n");
			return 0;
		}
		...
		queue_work(system_unbound_wq, &sdwc->work);
		spin_unlock_irqrestore(&sdwc->lock, flags);
		dev_info(sdwc->dev, "device connect from TYPEC.\n");
		break;
	case CABLE_STATUS_HOST_CONN:
		spin_lock_irqsave(&sdwc->lock, flags);
		if (sdwc->vbus_active == 1) {
			spin_unlock_irqrestore(&sdwc->lock, flags);
			dev_info(sdwc->dev, "ignore host connect from TYPEC.\n");
			return 0;
		}

		if (sdwc->dr_mode != USB_DR_MODE_UNKNOWN) {
			sdwc->vbus_active = 0;
			sdwc->next_mode = USB_DR_MODE_HOST;
			queue_work(system_unbound_wq, &sdwc->work);
			spin_unlock_irqrestore(&sdwc->lock, flags);
			dev_info(sdwc->dev, "missed disconnect event when TYPEC host connect.\n");
			return 0;
		}
		...
		queue_work(system_unbound_wq, &sdwc->work);
		spin_unlock_irqrestore(&sdwc->lock, flags);
		dev_info(sdwc->dev, "host connect from TYPEC.\n");
		break;
	case CABLE_STATUS_DEV_DISCONN:
		spin_lock_irqsave(&sdwc->lock, flags);
		if (sdwc->vbus_active == 0) {
			spin_unlock_irqrestore(&sdwc->lock, flags);
			dev_info(sdwc->dev, "ignore device disconnect from TYPEC.\n");
			return 0;
		}
		...
		queue_work(system_unbound_wq, &sdwc->work);
		spin_unlock_irqrestore(&sdwc->lock, flags);
		dev_info(sdwc->dev, "device disconnect from TYPEC.\n");
		break;
	case CABLE_STATUS_HOST_DISCONN:
		spin_lock_irqsave(&sdwc->lock, flags);
		if (sdwc->vbus_active == 0) {
			spin_unlock_irqrestore(&sdwc->lock, flags);
			dev_info(sdwc->dev, "ignore host disconnect from TYPEC.\n");
			return 0;
		}
		...
		queue_work(system_unbound_wq, &sdwc->work);
		spin_unlock_irqrestore(&sdwc->lock, flags);
		dev_info(sdwc->dev, "host disconnect from TYPEC.\n");
		break;
	default:
		/* Unknown actions */
		dev_err(sdwc->dev, "Unknown action 0x%lx\n", action);
		ret = -EINVAL;
		break;
	}
...
static void dwc3_sprd_detect_cable(struct dwc3_sprd *sdwc)
{
	...
	spin_lock_irqsave(&sdwc->lock, flags);
	if (extcon_get_cable_state_(sdwc->edev, EXTCON_USB) == true) {
		if (sdwc->vbus_active == 1) {
			spin_unlock_irqrestore(&sdwc->lock, flags);
			dev_info(sdwc->dev,
				"ignore device connection detected from VBUS GPIO.\n");
		...
		mode = sdwc->dr_mode;
		queue_work(system_unbound_wq, &sdwc->work);
	} else if (extcon_get_cable_state_(sdwc->edev, EXTCON_USB_HOST) == true) {
		if (sdwc->vbus_active == 1) {
			spin_unlock_irqrestore(&sdwc->lock, flags);
			dev_info(sdwc->dev,
				"ignore host connection detected from ID GPIO.\n");
...
static int dwc3_sprd_probe(struct platform_device *pdev)
{
	...
	if (!regmap_np) {
		pr_err("%s,get device_node fail ,return sdp\n", __func__);
		return -ENODEV;
	}
	...
	if (!pdev_regmap) {
		...
		pr_err("%s,get dev fail ,return sdp\n", __func__);
		return -ENODEV;
	}

	reg_map = dev_get_regmap(pdev_regmap->dev.parent, NULL);
	if (!reg_map) {
		pr_err("%s,get regmap fail ,return sdp\n", __func__);
		return -ENODEV;
	}
	...
	if (IS_ERR(sdwc->susp_clk)) {
		...
		ret = PTR_ERR(sdwc->susp_clk);
		goto err_ref_clk;
	}
	clk_prepare_enable(sdwc->susp_clk);

	sdwc->hibernate_en = of_property_read_bool(node,
						   "sprd,hibernation-enable");
	sdwc->pwr_collapse = of_property_read_bool(node,
				"sprd,power-collapse-on-cable-disconnect");
	sdwc->pwr_collapse_por = of_property_read_bool(node,
	...
	dwc = platform_get_drvdata(sdwc->dwc3);
	dwc->gadget.charger->get_charger_type = dwc3_sprd_get_charger_type;
	...
	if (of_property_read_bool(node, "extcon")) {
		...
		ret = of_get_named_gpio(extcon_node, "vbus-gpio", 0);
		if (ret < 0) {
		...
		ret = gpio_to_irq(sdwc->vbus_gpio);
		if (ret < 0) {
	...
	if (!strcmp(cable_detect, "typec")) {
		/*
		 * Some external ICs are employed to detect USB hot-plug
		 * such as MUIC and Type-C which is introduced with USB3.1.
		 * Type-C enhances ease of use for connecting USB devices
		 * with a focus on minimizing user confusion for plug
		 * and cable orientation.
		 */
		sdwc->hot_plug_nb.notifier_call = dwc3_sprd_ext_notifier;
		sdwc->hot_plug_nb.priority = 0;
		register_ext_notifier(&sdwc->hot_plug_nb);
		...
		if (sdwc->pwr_collapse) {
			dev_err(dev,
				"no cable detection methods are supported!\n");
			goto err_susp_clk;
		}
		sdwc->vbus_active = 1;
...
static int dwc3_sprd_remove(struct platform_device *pdev)
{
	...
	unregister_ext_notifier(&sdwc->hot_plug_nb);
	if (sdwc->edev) {
		extcon_unregister_notifier(sdwc->edev, EXTCON_USB,
					   &sdwc->vbus_nb);
		extcon_unregister_notifier(sdwc->edev, EXTCON_USB_HOST,
