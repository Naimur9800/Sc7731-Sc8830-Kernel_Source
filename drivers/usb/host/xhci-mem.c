...
void xhci_free_virt_device(struct xhci_hcd *xhci, int slot_id)
{
	...
	/*
	 * Work around for Intel phy, we need to check if all slots have been
	 * disabled, if so we should reset Intel phy, in case it can not
	 * enumerate when USB device was attached again.
	 */
	...
	if (i == MAX_HC_SLOTS)
		usb_phy_reset(xhci->main_hcd->usb_phy);
}
