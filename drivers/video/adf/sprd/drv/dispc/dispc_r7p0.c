...
typedef struct {
	...
	uint32_t dbi_ctrl;
	uint32_t dbi_timing0;
	uint32_t dbi_timing1;
	uint32_t dbi_rdata;
	uint32_t dbi_cmd;
	uint32_t dbi_data;
	uint32_t dbi_queue;
	...
	uint32_t shdw_buf_thres;
	uint32_t reserved_0x0214;
...
static int dispc_init(struct dispc_context *ctx)
{
	...
	if (ctx->if_type == SPRD_DISPC_IF_DPI) {
		width_strike = WIDTH_ALIGN(panel->width, 16);
		    ...
		    struct sprd_adf_hwlayer *hwlayer)
{
	...
	if (ctx->if_type == SPRD_DISPC_IF_DPI)
		hwlayer->dst_w = WIDTH_ALIGN(hwlayer->dst_w, 16);
	...
	/* for whale2, the dispc should stop between AFBC and RGB switch*/
	reg_val = reg->osd_ctrl & 0xF0;
	shdw_val = reg->shdw_osd_ctrl & 0xF0;
	if ((!ctx->is_stopped) && (shdw_val != reg_val)) {
		if (reg_val == 0xB0) {
			pr_info("stop dispc for switch RGB to AFBC\n");
			dispc_stop(ctx);

		} else if (shdw_val == 0xB0) {
			pr_info("stop dispc for switch AFBC to RGB\n");
			dispc_stop(ctx);
		}
	}
}
...
static struct dispc_core_ops dispc_r7p0_ops = {
