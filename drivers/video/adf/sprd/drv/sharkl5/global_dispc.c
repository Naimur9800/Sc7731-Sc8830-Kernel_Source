				...
				struct device_node *np)
{
	...
	if (IS_ERR_OR_NULL(clk_ctx->clk_src_96m)) {
		pr_err("read clk_src_96m failed\n");
		clk_ctx->clk_src_96m = NULL;
		status |= -1;
	}

	if (IS_ERR_OR_NULL(clk_ctx->clk_src_128m)) {
		pr_err("read clk_src_128m failed\n");
		clk_ctx->clk_src_128m = NULL;
		status |= -1;
	}

	if (IS_ERR_OR_NULL(clk_ctx->clk_src_153m6)) {
		pr_err("read clk_src_153m6 failed\n");
		clk_ctx->clk_src_153m6 = NULL;
		status |= -1;
	}

	if (IS_ERR_OR_NULL(clk_ctx->clk_src_384m)) {
		pr_err("read clk_src_384m failed\n");
...
static int dispc_clk_disable(struct dispc_context *ctx)
{
	...
	clk_set_parent(clk_ctx->clk_dispc_dpi, clk_ctx->clk_src_96m);
	clk_set_parent(clk_ctx->clk_dispc_core, clk_ctx->clk_src_153m6);
...
MODULE_DESCRIPTION("sprd sharkl5 dispc global and clk regs config");
