...
static char *fb_mode = "1280x720-32@60\0\0\0\0\0";
...
static int hdlcd_set_par(struct fb_info *info)
{
	...
	polarities |=
	    (hdlcd->fb.var.
	     sync & FB_SYNC_HOR_HIGH_ACT) ? HDLCD_POLARITY_HSYNC : 0;
	polarities |=
	    (hdlcd->fb.var.
	     sync & FB_SYNC_VERT_HIGH_ACT) ? HDLCD_POLARITY_VSYNC : 0;
	...
	WRITE_HDLCD_REG(HDLCD_REG_RED_SELECT,
			(0x00ff0000 | (hdlcd->fb.var.red.length & 0xf) << 8)
			| hdlcd->fb.var.red.offset);
#else
	WRITE_HDLCD_REG(HDLCD_REG_RED_SELECT,
			((hdlcd->fb.var.red.length & 0xf) << 8) | hdlcd->fb.var.
			red.offset);
#endif
	WRITE_HDLCD_REG(HDLCD_REG_GREEN_SELECT,
			((hdlcd->fb.var.green.length & 0xf) << 8) | hdlcd->fb.
			var.green.offset);
	WRITE_HDLCD_REG(HDLCD_REG_BLUE_SELECT,
			((hdlcd->fb.var.blue.length & 0xf) << 8) | hdlcd->fb.
			var.blue.offset);
			     ...
			     struct fb_info *info)
{
	...
	WRITE_HDLCD_REG(HDLCD_REG_FB_BASE_HIGH,
			(hdlcd->fb.fix.smem_start >> 32));
	WRITE_HDLCD_REG(HDLCD_REG_FB_BASE,
...
static int hdlcd_setup(struct hdlcd_device *hdlcd)
{
	...
	/* Allow max number of outstanding requests
	   with the largest beat burst */
