...
//+modify by liuwei
static int32_t sprdfb_dsi_common_cmd(sprdfb_dev_dsi_cmds * dsi_cmd)
{
	dsih_error_t result;
	result = mipi_dsih_wr_common_cmd(&(dsi_ctx.dsi_inst), dsi_cmd);
	if(OK != result){
...
struct ops_mipi sprdfb_mipi_ops = {
	...
	.mipi_common_cmd = sprdfb_dsi_common_cmd,
