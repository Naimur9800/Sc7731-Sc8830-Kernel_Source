...
#include "sprdfb.h"
#include "sprdfb_dev.h"//+modify by liuwei
...
typedef int32_t (*mipi_common_cmd_t)(sprdfb_dev_dsi_cmds * dsi_cmd);//+modify by liuwei
...
struct panel_operations {
	...
	void (*lcd_set_driving_mode)(struct panel_spec *self,bool enable);//add by liuwei
};
...
struct ops_mipi{
	...
	int32_t (*mipi_common_cmd)(sprdfb_dev_dsi_cmds * dsi_cmd);//+modify by liuwei
...
struct panel_spec {
	...
	bool mipi_clk_level; //add by liuwei
};
...
struct panel_cfg {
	...
	uint32_t lcd_id;
	uint32_t lcd_voltage_max;//+modify by liuwei
	uint32_t lcd_voltage_min;//+modify by liuwei
	const char *lcd_name;
