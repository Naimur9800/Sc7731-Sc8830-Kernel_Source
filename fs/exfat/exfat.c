...
#define TIME_START()        {__t1 = get_current_msec(); }
#define TIME_END()          {__t2 = get_current_msec(); }
...
INT32 ffsMountVol(struct super_block *sb, INT32 drv)
{
	...
	if (buf[0] != 0xEB) {
		...
		if (sector_read(sb, p_fs->PBR_sector, &tmp_bh, 1)
			!= FFS_SUCCESS) {
			bdev_close(sb);
			return FFS_MEDIAERR;
		}
...
INT32 ffsCreateFile(struct inode *inode, UINT8 *path,
			UINT8 mode, FILE_ID_T *fid)
{
	INT32 ret;
	CHAIN_T dir;
	UNI_NAME_T uni_name;
...
INT32 ffsReadFile(struct inode *inode, FILE_ID_T *fid, void *buffer,
			UINT64 count, UINT64 *rcount)
{
...
INT32 ffsWriteFile(struct inode *inode, FILE_ID_T *fid, void *buffer,
			UINT64 count, UINT64 *wcount)
{
	...
	if (fid->size == 0)
		...
		num_clusters = (INT32)((fid->size-1) >> p_fs->cluster_size_bits)
					+ 1;
	...
	while (count > 0) {
		...
		if ((offset == 0) && (oneblkwrite == p_bd->sector_size)) {
			if (sector_read(sb, LogSector, &tmp_bh, 0)
				...
				goto err_out;
			lock_buffer(tmp_bh);
			MEMCPY(((INT8 *) tmp_bh->b_data),
				((INT8 *) buffer)+write_bytes,
				(INT32) oneblkwrite);
			...
			lock_buffer(tmp_bh);
			MEMCPY(((INT8 *) tmp_bh->b_data)+offset,
				((INT8 *) buffer)+write_bytes,
				(INT32) oneblkwrite);
			set_buffer_uptodate(tmp_bh);
			unlock_buffer(tmp_bh);
	...
	if (p_fs->vol_type == EXFAT) {
		es = get_entry_set_in_dir(sb,
			&(fid->dir), fid->entry, ES_ALL_ENTRIES, &ep);
		if (es == NULL)
			goto err_out;
		ep2 = ep+1;
	} else {
...
INT32 ffsTruncateFile(struct inode *inode, UINT64 old_size, UINT64 new_size)
{
	...
	if (new_size > 0) {
		num_clusters = (INT32)((new_size-1) >> p_fs->cluster_size_bits)
				+ 1;
	...
	if (fid->dir.dir != DIR_DELETED) {
		if (p_fs->vol_type == EXFAT) {
			...
			ep = get_entry_in_dir(sb,
				&(fid->dir), fid->entry, &sector);
			if (!ep)
				return FFS_MEDIAERR;
			ep2 = ep;
		}

		p_fs->fs_func->set_entry_time(ep,
			tm_current(&tm, tz_utc), TM_MODIFY);
		p_fs->fs_func->set_entry_attr(ep, fid->attr);
...
INT32 ffsMoveFile(struct inode *old_parent_inode, FILE_ID_T *fid,
		struct inode *new_parent_inode, struct dentry *new_dentry)
{
	...
	if (new_inode) {
		...
		if (new_entry_type == TYPE_DIR) {
			...
			new_clu.size = (INT32)((new_fid->size-1)
	...
	if (olddir.dir == newdir.dir)
		ret = rename_file(new_parent_inode, &olddir,
				dentry, &uni_name, fid);
	else
		ret = move_file(new_parent_inode, &olddir, dentry,
				&newdir, &uni_name, fid);
	...
	if ((ret == FFS_SUCCESS) && new_inode) {
		...
		if (num_entries < 0)
			goto out;
		p_fs->fs_func->delete_dir_entry(sb, p_dir, new_entry,
						0, num_entries+1);
		if (new_entry_type == TYPE_DIR) {
			CHAIN_T new_clu_to_free;

			new_clu_to_free.dir = new_fid->start_clu;
			new_clu_to_free.size = (INT32)((new_fid->size-1)
						>> p_fs->cluster_size_bits) + 1;
...
INT32 ffsRemoveFile(struct inode *inode, FILE_ID_T *fid)
{
	...
	clu_to_free.size = (INT32)((fid->size-1) >> p_fs->cluster_size_bits)
				+ 1;
...
INT32 ffsGetStat(struct inode *inode, DIR_ENTRY_T *info)
{
	...
	if (is_dir) {
		if ((fid->dir.dir == p_fs->root_dir) &&
			...
			MEMSET((INT8 *) &info->CreateTimestamp,
				0, sizeof(DATE_TIME_T));
			MEMSET((INT8 *) &info->ModifyTimestamp,
				0, sizeof(DATE_TIME_T));
			MEMSET((INT8 *) &info->AccessTimestamp,
				0, sizeof(DATE_TIME_T));
	...
	*(uni_name.name) = 0x0;
	p_fs->fs_func->get_uni_name_from_ext_entry(sb, &(fid->dir),
					fid->entry, uni_name.name);
	if (*(uni_name.name) == 0x0)
		get_uni_name_from_dos_entry(sb, (DOS_DENTRY_T *) ep,
					&uni_name, 0x1);
	...
	if (is_dir) {
		...
		if (info->Size == 0)
			info->Size = (UINT64)count_num_clusters(sb, &dir)
				<< p_fs->cluster_size_bits;
...
INT32 ffsMapCluster(struct inode *inode, INT32 clu_offset,
			UINT32 *clu, int create)
{
	...
	if (EXFAT_I(inode)->mmu_private == 0)
		...
		num_clusters = (INT32)((EXFAT_I(inode)->mmu_private-1)
			>> p_fs->cluster_size_bits) + 1;
	...
	*clu = last_clu = fid->start_clu;
	...
	if (!create && *clu == CLUSTER_32(~0)) {
		pr_err("[EXFAT] error: no mapping for the %u-th cluster\n",
			clu_offset);
		dump_stack();
		return FFS_MEDIAERR;
	}
	...
	if (create && *clu == CLUSTER_32(~0)) {
		...
		*clu = new_clu.dir;
		...
		if (fid->dir.dir != DIR_DELETED) {
			if (modified && p_fs->vol_type == EXFAT) {
				es = get_entry_set_in_dir(sb, &(fid->dir),
					fid->entry, ES_ALL_ENTRIES, &ep);
				if (es == NULL)
					return FFS_MEDIAERR;
				ep++;
			}
			...
			if (modified) {
				...
				if (p_fs->fs_func->get_entry_flag(ep)
					!= fid->flags) {
					modified = TRUE;
					p_fs->fs_func->set_entry_flag(ep,
								fid->flags);
				}

				if (p_fs->fs_func->get_entry_clu0(ep)
					!= fid->start_clu) {
					modified = TRUE;
		...
		inode->i_blocks += num_alloced
			<< (p_fs->cluster_size_bits - sb->s_blocksize_bits);
	}
...
INT32 ffsReadDir(struct inode *inode, DIR_ENTRY_T *dir_entry)
{
	...
	while (clu.dir != CLUSTER_32(~0)) {
		...
		for ( ; i < dentries_per_clu; i++, dentry++) {
			...
			MEMSET((INT8 *) &dir_entry->AccessTimestamp,
				0, sizeof(DATE_TIME_T));

			*(uni_name.name) = 0x0;
			p_fs->fs_func->get_uni_name_from_ext_entry(sb, &dir,
						dentry, uni_name.name);
			if (*(uni_name.name) == 0x0)
				get_uni_name_from_dos_entry(sb,
					(DOS_DENTRY_T *) ep, &uni_name, 0x1);
			...
			if (p_fs->vol_type == EXFAT) {
				...
				get_uni_name_from_dos_entry(sb,
					(DOS_DENTRY_T *) ep, &uni_name, 0x0);
				nls_uniname_to_cstring(sb,
					dir_entry->ShortName, &uni_name);
			}
...
INT32 fat_alloc_cluster(struct super_block *sb,
			INT32 num_alloc, CHAIN_T *p_chain)
{
...
INT32 exfat_alloc_cluster(struct super_block *sb,
			INT32 num_alloc, CHAIN_T *p_chain)
{
...
void exfat_free_cluster(struct super_block *sb,
			CHAIN_T *p_chain, INT32 do_relse)
{
	INT32 num_clusters = 0;
	UINT32 clu;
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
...
#else
		ret = sb_issue_discard(sb, START_SECTOR(clu),
				(1 << p_fs->sectors_per_clu_bits), GFP_NOFS, 0);
#endif
		if (ret == -EOPNOTSUPP) {
...
INT32 __load_upcase_table(struct super_block *sb, UINT32 sector,
			UINT32 num_sectors, UINT32 utbl_checksum)
{
	...
	upcase_table = p_fs->vol_utbl = (UINT16 **) MALLOC(UTBL_COL_COUNT
							* sizeof(UINT16 *));
	...
	while (sector < num_sectors) {
		...
		for (i = 0; i < p_bd->sector_size && index <= 0xFFFF; i += 2) {
			...
			checksum = ((checksum & 1) ? 0x80000000 : 0) +
				(checksum >> 1) +
				*(((UINT8 *) tmp_bh->b_data)+i);
			checksum = ((checksum & 1) ? 0x80000000 : 0) +
				(checksum >> 1) +
				*(((UINT8 *) tmp_bh->b_data) + (i + 1));
...
INT32 load_upcase_table(struct super_block *sb)
{
	...
	while (clu.dir != CLUSTER_32(~0)) {
		for (i = 0; i < p_fs->dentries_per_clu; i++) {
			ep = (CASE_DENTRY_T *) get_entry_in_dir(sb, &clu,
								i, NULL);
			if (!ep)
				return FFS_MEDIAERR;
			...
			num_sectors = ((tbl_size-1) >> p_bd->sector_size_bits)
					+ 1;
...
INT32 fat_init_dir_entry(struct super_block *sb, CHAIN_T *p_dir, INT32 entry,
			UINT32 type, UINT32 start_clu, UINT64 size)
{
	UINT32 sector;
	DOS_DENTRY_T *dos_ep;
	UINT8 tz_utc = EXFAT_SB(sb)->options.tz_utc;
...
INT32 exfat_init_dir_entry(struct super_block *sb, CHAIN_T *p_dir, INT32 entry,
			UINT32 type, UINT32 start_clu, UINT64 size)
{
	...
	strm_ep = (STRM_DENTRY_T *) get_entry_in_dir(sb, p_dir,
						entry+1, &sector1);
	if (!strm_ep)
		return FFS_MEDIAERR;
...
INT32 fat_init_ext_entry(struct super_block *sb, CHAIN_T *p_dir, INT32 entry,
			INT32 num_entries, UNI_NAME_T *p_uniname,
			DOS_NAME_T *p_dosname)
{
	...
	if ((--num_entries) > 0) {
		...
		for (i = 1; i < num_entries; i++) {
			ext_ep = (EXT_DENTRY_T *) get_entry_in_dir(sb, p_dir,
					entry-i, &sector);
			if (!ext_ep)
				return FFS_MEDIAERR;
		...
		ext_ep = (EXT_DENTRY_T *) get_entry_in_dir(sb, p_dir, entry-i,
						&sector);
		if (!ext_ep)
			return FFS_MEDIAERR;
...
INT32 exfat_init_ext_entry(struct super_block *sb, CHAIN_T *p_dir, INT32 entry,
		INT32 num_entries, UNI_NAME_T *p_uniname, DOS_NAME_T *p_dosname)
{
	INT32 i;
	UINT32 sector;
	UINT16 *uniname = p_uniname->name;
	...
	for (i = 2; i < num_entries; i++) {
		name_ep = (NAME_DENTRY_T *) get_entry_in_dir(sb, p_dir,
						entry+i, &sector);
		if (!name_ep)
			return FFS_MEDIAERR;
...
void init_dos_entry(DOS_DENTRY_T *ep, UINT32 type,
			UINT32 start_clu, UINT8 tz_utc)
{
	TIMESTAMP_T tm, *tp;
...
void init_ext_entry(EXT_DENTRY_T *ep, INT32 order,
			UINT8 chksum, UINT16 *uniname)
{
	INT32 i;
	UINT8 end = FALSE;
...
void init_strm_entry(STRM_DENTRY_T *ep, UINT8 flags,
...
void fat_delete_dir_entry(struct super_block *sb, CHAIN_T *p_dir, INT32 entry,
			INT32 order, INT32 num_entries)
{
	INT32 i;
	UINT32 sector;
	DENTRY_T *ep;
...
void exfat_delete_dir_entry(struct super_block *sb, CHAIN_T *p_dir,
			INT32 entry, INT32 order, INT32 num_entries)
{
	INT32 i;
	UINT32 sector;
	DENTRY_T *ep;
...
void update_dir_checksum(struct super_block *sb, CHAIN_T *p_dir, INT32 entry)
{
	...
	chksum = calc_checksum_2byte((void *) file_ep,
				DENTRY_SIZE, 0, CS_DIR_ENTRY);
	...
	for (i = 1; i < num_entries; i++) {
		...
		chksum = calc_checksum_2byte((void *) ep, DENTRY_SIZE,
					chksum, CS_DEFAULT);
	}
...
static INT32 _walk_fat_chain(struct super_block *sb, CHAIN_T *p_dir,
				INT32 byte_offset, UINT32 *clu)
{
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
	INT32 clu_offset;
	UINT32 cur_clu;
...
INT32 find_location(struct super_block *sb, CHAIN_T *p_dir, INT32 entry,
			UINT32 *sector, INT32 *offset)
{
	INT32 off, ret;
	UINT32 clu = 0;
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
...
DENTRY_T *get_entry_with_sector(struct super_block *sb,
				UINT32 sector, INT32 offset)
{
	UINT8 *buf;
...
DENTRY_T *get_entry_in_dir(struct super_block *sb, CHAIN_T *p_dir,
				INT32 entry, UINT32 *sector)
{
	INT32 off;
	UINT32 sec;
	UINT8 *buf;
...
ENTRY_SET_CACHE_T *get_entry_set_in_dir(struct super_block *sb, CHAIN_T *p_dir,
				INT32 entry, UINT32 type, DENTRY_T **file_ep)
{
	...
	PRINTK("trying to malloc %x bytes for %d entries\n",
		offsetof(ENTRY_SET_CACHE_T, __buf)
		+ num_entries * sizeof(DENTRY_T), num_entries);
	es = MALLOC(offsetof(ENTRY_SET_CACHE_T, __buf)
		+ num_entries * sizeof(DENTRY_T));
	if (es == NULL)
		goto err_out;
	...
	PRINTK("es sec %u offset %d flags %d, num_entries %u buf ptr %p\n",
		es->sector, es->offset, es->alloc_flag, es->num_entries,
		&(es->__buf));
...
static INT32 __write_partial_entries_in_entry_set(struct super_block *sb,
					ENTRY_SET_CACHE_T *es, UINT32 sec,
					INT32 off, UINT32 count)
{
	...
	while (num_entries) {
		...
		copy_entries = MIN(remaining_byte_in_sector >> DENTRY_SIZE_BITS,
					num_entries);
		buf = buf_getblk(sb, sec);
		if (buf == NULL)
			goto err_out;
...
INT32 search_deleted_or_unused_entry(struct super_block *sb,
				CHAIN_T *p_dir, INT32 num_entries)
{
...
INT32 find_empty_entry(struct inode *inode, CHAIN_T *p_dir, INT32 num_entries)
{
	...
	while ((dentry = search_deleted_or_unused_entry(sb, p_dir, num_entries))
		< 0) {
		if (p_fs->dev_ejected)
			break;

		if ((p_fs->vol_type == EXFAT)
			&& (p_dir->dir != p_fs->root_dir))
		...
		if (p_fs->hint_uentry.entry == -1) {
			p_fs->hint_uentry.dir = p_dir->dir;
			p_fs->hint_uentry.entry = p_dir->size
				<< (p_fs->cluster_size_bits - DENTRY_SIZE_BITS);
...
INT32 fat_find_dir_entry(struct super_block *sb, CHAIN_T *p_dir,
			UNI_NAME_T *p_uniname, INT32 num_entries,
			DOS_NAME_T *p_dosname, UINT32 type)
{
		...
		for (i = 0; i < dentries_per_clu; i++, dentry++) {
			...
			if ((entry_type == TYPE_FILE)
				...
				if ((type == TYPE_ALL) || (type == entry_type)) {
					...
					if ((!lossy) && (!nls_dosname_cmp(sb,
						p_dosname->name, dos_ep->name)))
						return dentry;
				}
...
INT32 exfat_find_dir_entry(struct super_block *sb, CHAIN_T *p_dir,
			UNI_NAME_T *p_uniname, INT32 num_entries,
			DOS_NAME_T *p_dosname, UINT32 type)
{
	...
	if (p_dir->dir == p_fs->root_dir) {
		if ((!nls_uniname_cmp(sb, p_uniname->name,
			(UINT16 *) UNI_CUR_DIR_NAME)) ||
			(!nls_uniname_cmp(sb, p_uniname->name,
				(UINT16 *) UNI_PAR_DIR_NAME)))
			return -1;
	}
...
INT32 fat_count_ext_entries(struct super_block *sb, CHAIN_T *p_dir,
				INT32 entry, DENTRY_T *p_entry)
{
	INT32 count = 0;
	UINT8 chksum;
	...
	for (entry--; entry >= 0; entry--) {
		ext_ep = (EXT_DENTRY_T *)
				get_entry_in_dir(sb, p_dir, entry, NULL);
		if (!ext_ep)
			return -1;

		if ((p_fs->fs_func->get_entry_type((DENTRY_T *) ext_ep) ==
...
INT32 exfat_count_ext_entries(struct super_block *sb, CHAIN_T *p_dir,
				INT32 entry, DENTRY_T *p_entry)
{
	INT32 i, count = 0;
	UINT32 type;
...
INT32 count_dos_name_entries(struct super_block *sb, CHAIN_T *p_dir,
				UINT32 type)
{
	INT32 i, count = 0;
	INT32 dentries_per_clu;
...
INT32 get_num_entries_and_dos_name(struct super_block *sb, CHAIN_T *p_dir,
				UNI_NAME_T *p_uniname, INT32 *entries,
				DOS_NAME_T *p_dosname)
{
...
void get_uni_name_from_dos_entry(struct super_block *sb, DOS_DENTRY_T *ep,
				UNI_NAME_T *p_uniname, UINT8 mode)
{
	DOS_NAME_T dos_name;
...
void fat_get_uni_name_from_ext_entry(struct super_block *sb, CHAIN_T *p_dir,
					INT32 entry, UINT16 *uniname)
{
	INT32 i;
	EXT_DENTRY_T *ep;
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
	...
	for (entry--, i = 1; entry >= 0; entry--, i++) {
		...
		if (p_fs->fs_func->get_entry_type((DENTRY_T *) ep)
			== TYPE_EXTEND) {
...
void exfat_get_uni_name_from_ext_entry(struct super_block *sb, CHAIN_T *p_dir,
					INT32 entry, UINT16 *uniname)
{
	INT32 i;
	DENTRY_T *ep;
	...
	for (i = 2; i < es->num_entries; i++, ep++) {
		if (p_fs->fs_func->get_entry_type(ep) == TYPE_EXTEND) {
			extract_uni_name_from_name_entry((NAME_DENTRY_T *)ep,
								uniname, i);
		} else {
			goto out;
		}
...
INT32 extract_uni_name_from_ext_entry(EXT_DENTRY_T *ep, UINT16 *uniname,
					INT32 order)
{
	INT32 i, len = 0;
...
INT32 extract_uni_name_from_name_entry(NAME_DENTRY_T *ep, UINT16 *uniname,
					INT32 order)
{
	INT32 i, len = 0;
...
INT32 fat_generate_dos_name(struct super_block *sb, CHAIN_T *p_dir,
				DOS_NAME_T *p_dosname)
{
	...
	while (clu.dir != CLUSTER_32(~0)) {
		...
		for (i = 0; i < dentries_per_clu; i++) {
			ep = (DOS_DENTRY_T *)get_entry_in_dir(sb,
							&clu, i, NULL);
			if (!ep)
				return FFS_MEDIAERR;
...
INT32 resolve_path(struct inode *inode, UINT8 *path, CHAIN_T *p_dir,
		UNI_NAME_T *p_uniname)
{
	INT32 lossy = FALSE;
	struct super_block *sb = inode->i_sb;
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
...
INT32 fat32_mount(struct super_block *sb, PBR_SECTOR_T *p_pbr)
{
	...
	p_fs->num_clusters =
		((p_fs->num_sectors-num_reserved) >> p_fs->sectors_per_clu_bits)
		 + 2;
...
INT32 create_dir(struct inode *inode, CHAIN_T *p_dir, UNI_NAME_T *p_uniname,
		FILE_ID_T *fid)
{
	INT32 ret, dentry, num_entries;
	UINT64 size;
	...
	if (p_fs->vol_type == EXFAT) {
		...
		ret = p_fs->fs_func->init_dir_entry(sb, &clu, 0, TYPE_DIR,
						clu.dir, 0);
		if (ret != FFS_SUCCESS)
			return ret;

		ret = p_fs->fs_func->init_ext_entry(sb, &clu, 0, 1, NULL,
						&dot_name);
		if (ret != FFS_SUCCESS)
			return ret;
		...
		if (p_dir->dir == p_fs->root_dir)
			ret = p_fs->fs_func->init_dir_entry(sb, &clu, 1,
					TYPE_DIR, CLUSTER_32(0), 0);
		else
			ret = p_fs->fs_func->init_dir_entry(sb, &clu, 1,
					TYPE_DIR, p_dir->dir, 0);
	...
	ret = p_fs->fs_func->init_dir_entry(sb, p_dir, dentry, TYPE_DIR,
					clu.dir, size);
	if (ret != FFS_SUCCESS)
		return ret;

	ret = p_fs->fs_func->init_ext_entry(sb, p_dir, dentry, num_entries,
					p_uniname, &dos_name);
	if (ret != FFS_SUCCESS)
		return ret;
...
INT32 create_file(struct inode *inode, CHAIN_T *p_dir, UNI_NAME_T *p_uniname,
		UINT8 mode, FILE_ID_T *fid)
{
	INT32 ret, dentry, num_entries;
	DOS_NAME_T dos_name;
	...
	ret = p_fs->fs_func->init_dir_entry(sb, p_dir, dentry, TYPE_FILE | mode,
					CLUSTER_32(0), 0);
	if (ret != FFS_SUCCESS)
		return ret;
...
INT32 rename_file(struct inode *inode, CHAIN_T *p_dir, INT32 oldentry,
		UNI_NAME_T *p_uniname, FILE_ID_T *fid)
{
	...
	num_old_entries = p_fs->fs_func->count_ext_entries(sb, p_dir, oldentry,
							epold);
	...
	if (num_old_entries < num_new_entries) {
		...
		if (p_fs->fs_func->get_entry_type(epnew) == TYPE_FILE) {
			p_fs->fs_func->set_entry_attr(epnew,
				p_fs->fs_func->get_entry_attr(epnew)
							| ATTR_ARCHIVE);
			fid->attr |= ATTR_ARCHIVE;
		}
		...
		ret = p_fs->fs_func->init_ext_entry(sb, p_dir, newentry,
					num_new_entries, p_uniname, &dos_name);
		if (ret != FFS_SUCCESS)
			return ret;

		p_fs->fs_func->delete_dir_entry(sb, p_dir, oldentry, 0,
						num_old_entries);
		fid->entry = newentry;
	} else {
		if (p_fs->fs_func->get_entry_type(epold) == TYPE_FILE) {
			p_fs->fs_func->set_entry_attr(epold,
				p_fs->fs_func->get_entry_attr(epold)
							| ATTR_ARCHIVE);
			fid->attr |= ATTR_ARCHIVE;
		}
		...
		ret = p_fs->fs_func->init_ext_entry(sb, p_dir, oldentry,
					num_new_entries, p_uniname, &dos_name);
		if (ret != FFS_SUCCESS)
			return ret;

		p_fs->fs_func->delete_dir_entry(sb, p_dir, oldentry,
					num_new_entries, num_old_entries);
	}

	return FFS_SUCCESS;
}

INT32 move_file(struct inode *inode, CHAIN_T *p_olddir, INT32 oldentry,
		CHAIN_T *p_newdir, UNI_NAME_T *p_uniname, FILE_ID_T *fid)
{
	...
	num_old_entries = p_fs->fs_func->count_ext_entries(sb, p_olddir,
							oldentry, epmov);
	if (num_old_entries < 0) {
	...
	if (p_fs->fs_func->get_entry_type(epnew) == TYPE_FILE) {
		p_fs->fs_func->set_entry_attr(epnew,
			p_fs->fs_func->get_entry_attr(epnew) | ATTR_ARCHIVE);
		fid->attr |= ATTR_ARCHIVE;
	}
	buf_modify(sb, sector_new);
	buf_unlock(sb, sector_mov);
	...
	ret = p_fs->fs_func->init_ext_entry(sb, p_newdir, newentry,
					num_new_entries, p_uniname, &dos_name);
	if (ret != FFS_SUCCESS)
		return ret;

	p_fs->fs_func->delete_dir_entry(sb, p_olddir, oldentry, 0,
					num_old_entries);
...
INT32 sector_read(struct super_block *sb, UINT32 sec,
		struct buffer_head **bh, INT32 read)
{
...
INT32 sector_write(struct super_block *sb, UINT32 sec,
			struct buffer_head *bh, INT32 sync)
{
	INT32 ret = FFS_MEDIAERR;
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
...
INT32 multi_sector_read(struct super_block *sb, UINT32 sec,
			struct buffer_head **bh, INT32 num_secs, INT32 read)
{
	INT32 ret = FFS_MEDIAERR;
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
	...
	if (((sec+num_secs) > (p_fs->PBR_sector+p_fs->num_sectors))
		&& (p_fs->num_sectors > 0)) {
		PRINT("[EXFAT] multi_sector_read: out of range error!"
		" (sec = %d, num_secs = %d)\n", sec, num_secs);
		fs_error(sb);
		return ret;
	}
...
INT32 multi_sector_write(struct super_block *sb, UINT32 sec,
			struct buffer_head *bh, INT32 num_secs, INT32 sync)
{
	INT32 ret = FFS_MEDIAERR;
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
	...
	if ((sec+num_secs) > (p_fs->PBR_sector+p_fs->num_sectors)
		&& (p_fs->num_sectors > 0)) {
		PRINT("[EXFAT] multi_sector_write: out of range error!"
			" (sec = %d, num_secs = %d)\n", sec, num_secs);
		fs_error(sb);
		return ret;
	}
