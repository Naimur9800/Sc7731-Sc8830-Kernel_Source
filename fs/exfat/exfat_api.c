...
INT32 FsReadFile(struct inode *inode, FILE_ID_T *fid,
		void *buffer, UINT64 count, UINT64 *rcount)
{
	INT32 err;
	struct super_block *sb = inode->i_sb;
...
INT32 FsWriteFile(struct inode *inode, FILE_ID_T *fid,
		void *buffer, UINT64 count, UINT64 *wcount)
{
	INT32 err;
	struct super_block *sb = inode->i_sb;
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
			...
			UINT32 *clu, int create)
{
	...
	err = ffsMapCluster(inode, clu_offset, clu, create);
...
EXPORT_SYMBOL(init_exfat_core);

void  exit_exfat_core(void)
{
	FsShutdown();
}
EXPORT_SYMBOL(exit_exfat_core);
