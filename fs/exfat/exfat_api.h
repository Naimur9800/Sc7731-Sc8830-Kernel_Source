...
#define FFS_ERROR               19
	...
	INT32 FsCreateFile(struct inode *inode, UINT8 *path,
			UINT8 mode, FILE_ID_T *fid);
	INT32 FsReadFile(struct inode *inode, FILE_ID_T *fid, void *buffer,
			UINT64 count, UINT64 *rcount);
	INT32 FsWriteFile(struct inode *inode, FILE_ID_T *fid, void *buffer,
			UINT64 count, UINT64 *wcount);
	INT32 FsTruncateFile(struct inode *inode,
			UINT64 old_size, UINT64 new_size);
	INT32 FsMoveFile(struct inode *old_parent_inode, FILE_ID_T *fid,
	...
	INT32 FsMapCluster(struct inode *inode, INT32 clu_offset,
	...
	int  init_exfat_core(void);
	void  exit_exfat_core(void);
#ifdef __cplusplus
}
#endif
#endif
