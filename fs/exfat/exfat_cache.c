...
static void buf_readahead(struct super_block *sb, UINT32 sec)
{
	UINT32  i, sec_end;
	...
	if (bh == NULL || !buffer_uptodate(bh)) {
		...
		brelse(bh);
		bh = sb_find_get_block(sb, sec + 1);
		if (bh == NULL || !buffer_uptodate(bh)) {
			i = sec+1;
		}
	}
	if (i != ~0) {
		sec_end = (i + 1 + (PAGE_SIZE>>sb->s_blocksize_bits))
			& ~((PAGE_SIZE>>sb->s_blocksize_bits)-1);
		for ( ; i < sec_end; i++)
			sb_breadahead(sb, i);
	}
	brelse(bh);
