...
static INT32  convert_uni_to_ch(struct nls_table *nls, UINT8 *ch,
				UINT16 uni, INT32 *lossy);
static INT32  convert_ch_to_uni(struct nls_table *nls, UINT16 *uni,
				UINT8 *ch, INT32 *lossy);
...
void nls_uniname_to_dosname(struct super_block *sb, DOS_NAME_T *p_dosname,
				UNI_NAME_T *p_uniname, INT32 *p_lossy)
{
...
void nls_uniname_to_cstring(struct super_block *sb,
			UINT8 *p_cstring, UNI_NAME_T *p_uniname)
{
	INT32 i, j, len;
	UINT8 buf[MAX_CHARSET_SIZE];
	UINT16 *uniname = p_uniname->name;
...
void nls_cstring_to_uniname(struct super_block *sb, UNI_NAME_T *p_uniname,
				UINT8 *p_cstring, INT32 *p_lossy)
{
	...
	*(++end_of_name) = '\0';
	...
	if (STRCMP((INT8 *) p_cstring, ".")
		&& STRCMP((INT8 *) p_cstring, "..")) {
		while (*(--end_of_name) == '.') {
			if (end_of_name < p_cstring)
				break;
		}
	...
	*uniname = (UINT16) '\0';
	...
	p_uniname->name_hash = calc_checksum_2byte((void *) upname,
					j<<1, 0, CS_DEFAULT);
