/* Some of the source code in this file came from "linux/fs/fat/file.c",
 *  "linux/fs/fat/inode.c" and "linux/fs/fat/misc.c".  */
/*
 *  linux/fs/fat/file.c
 *
...
void exfat_time_unix2fat(struct exfat_sb_info *sbi,
			struct timespec *ts, DATE_TIME_T *tp)
{
...
#endif
{
	...
	if (S_ISLNK(i_mode)) {
		...
		FsReadFile(dir, &fid, EXFAT_I(inode)->target,
			...
			struct inode *new_dir, struct dentry *new_dentry)
{
	...
	err = FsMoveFile(old_dir, &(EXFAT_I(old_inode)->fid),
			new_dir, new_dentry);
	if (err) {
		if (err == FFS_PERMISSIONERR)
			err = -EPERM;
...
static int exfat_getattr(struct vfsmount *mnt,
			...
			unsigned long *mapped_blocks, int *create)
{
	...
	*mapped_blocks = 0;
	...
	if ((p_fs->vol_type == FAT12) || (p_fs->vol_type == FAT16)) {
		if (inode->i_ino == EXFAT_ROOT_INO) {
			if (sector < (p_fs->dentries_in_root
	...
	err = FsMapCluster(inode, clu_offset, &cluster, *create);
				...
				struct buffer_head *bh_result, int create)
{
	...
	if (phys) {
		...
		pr_err("[EXFAT] %s error: no mapping for block\n", __func__);
	}
...
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 00)
		...
		loff_t offset)
{
	 ...
	 */
	ret = blockdev_direct_IO(iocb, inode,
				iter, offset, exfat_get_block);
...
static int exfat_fill_inode(struct inode *inode, FILE_ID_T *fid)
{
	...
	if (info.Attr & ATTR_SUBDIR) {
		...
		inode->i_generation |= 1;
		inode->i_mode = exfat_make_mode(sbi, info.Attr, S_IRWXUGO);
		inode->i_link = (char *)(EXFAT_I(inode)->target);
...
static int exfat_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	if (p_fs->used_clusters == (UINT32) ~0) {
		...
		if (p_fs->dev_ejected)
			pr_err("[EXFAT] called statfs with previous I/O error.\n");
	}
...
static int exfat_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (!sbi->nls_io) {
		pr_err("[EXFAT] IO charset %s not found\n",
