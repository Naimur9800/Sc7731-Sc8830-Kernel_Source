...
module_param_named(compressor, zcache_compressor, charp, 0644);
...
static int zcache_debug_show_printk(enum e_show_mem_type type)
{
	pr_err("Total used : %llu kB\n", zcache_pages() * 4);
	return 0;
}

static int zcache_e_show_mem_handler(struct notifier_block *nb,
			unsigned long val, void *data)
{
	enum e_show_mem_type type = val;

	pr_err("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	pr_err("Enhanced Mem-info : Zcache\n");
	return zcache_debug_show_printk(type);
}

static struct notifier_block zcache_e_show_mem_notifier = {
	.notifier_call = zcache_e_show_mem_handler,
};
#endif
/*
...
#ifdef CONFIG_E_SHOW_MEM
	register_e_show_mem_notifier(&zcache_e_show_mem_notifier);
#endif
	return 0;
pcpufail:
	zcache_comp_exit();
