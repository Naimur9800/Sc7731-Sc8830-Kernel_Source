...
static struct snd_kcontrol_new __mixerroutedata = {
	.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
...

{
	...
	err = snd_ctl_add(saudio->card, kctl);
	printk(KERN_INFO "yaye saudio.c:kcontrol create entry4!!!!saudio:%x, kctl:%x, card:%x, add_err:%d \n",
	saudio,kctl,saudio->card,err);
}

{
	...
	kctl = snd_ctl_new1(&__mixerroutedata, saudio);
	err = snd_ctl_add(saudio->card, kctl);
	printk(KERN_INFO "yaye saudio.c:kcontrol create entry5!!!!saudio:%x, kctl:%x, card:%x, add_err:%d \n",
	saudio,kctl,saudio->card,err);
}
...
static int snd_pcm_mixer_route_control_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	printk(KERN_INFO "yaye snd_pcm_mixer_route_control_get entry!!!kcontrol:%x!\n",kcontrol);
	if(kcontrol){
		int i=0;
		...
		if(kcontrol->private_data){
			...
			printk(KERN_INFO "yaye snd_pcm_mixer_route_control_get!!!! get value %d!\n", ucontrol->value.integer.value[0]);
		}
	}
	return 0;
}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0] !=  saudio->kcon_mixer_route){
		printk(KERN_INFO "yaye snd_pcm_mixer_route_control_put entry set value:%d!!!\n", ucontrol->value.integer.value[0]);
		saudio->kcon_mixer_route = ucontrol->value.integer.value[0];
		changed = 1;

		int mixer_route = saudio->kcon_mixer_route;
		printk(KERN_INFO "yaye snd_pcm_mixer_route_control_put mixer_route %d!!!!\n", mixer_route);
		result =
			saudio_send_common_cmd_ex(saudio->dst, saudio->channel,
				SAUDIO_CMD_SET_MIXER_ROUTE, SAUDIO_CMD_SET_MIXER_ROUTE, mixer_route, 0,
				0);
		if (result) {
			ETRACE("yaye snd_pcm_mixer_route_control_put: RESUME, send_common_cmd result is %d!\n",result);
			if(result != (-ERESTARTSYS))
			saudio_snd_card_free(saudio);
			return result;
		}
