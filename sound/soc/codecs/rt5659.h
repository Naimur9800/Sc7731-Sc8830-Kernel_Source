...
/* Jack Detect Control 3 (0x00f6) */
#define RT5659_SEL_GPIO_JD1_MASK		(0x7)
#define RT5659_SEL_GPIO_JD1_SFT			9

/* Jack Detect Control 3 (0x00f7) */
#define RT5659_SEL_GPIO_JD2_MASK		(0x7)
#define RT5659_SEL_GPIO_JD2_SFT			9
...
/* if dai driver id is 0, then a index value will be used for the
 * dai id when register codec driver.*/
#define DAI_ID_TO_INDEX(id) ((id) - 1)
#define INDEX_TO_DAI_ID(index) ((index) + 1)
...
#define EQ_REG_NUM 40
#define BP_SUPPORTING_COUNT 4
#define DRC_REG_NUM2 50
...
struct compose_codec_register {
	/* 0:general register 1:private register 2:dsp register */
	WORD	uc_reg_type;
	WORD	index; /* register index */
	WORD	data; /* register data */
};

struct driver_eq_parameter {
	WORD eq_ctrl_reg_num;
	struct compose_codec_register eq_ctrl_reg[EQ_REG_NUM];
};

struct hw_eq_lowpass_properties {
	BOOL	enable;
	float	gain;
	LONG	bandwidth;
	BOOL	shelving_filter;
};

struct hw_eq_bandpass_properties {
	BOOL	enable;
	float	gain;
...
struct hw_eq_hipass_properties {
	BOOL	enable;
	float	gain;
	LONG	bandwidth;
	BOOL	shelving_filter;
...
struct hw_eq_hipass2_properties {
	BOOL	enable;
	LONG	bandwidth;
	BOOL	shelving_filter;
	BOOL	third_filter;
...
struct hw_eq_properties2 { /* for 5658 */
	struct hw_eq_lowpass_properties	lp0;
	struct hw_eq_bandpass_properties	bp[BP_SUPPORTING_COUNT];
	struct hw_eq_hipass_properties	hp1;
	struct hw_eq_hipass2_properties	hp2;
	WORD ui_bp1_source;
	float eq_input_vol;
	float eq_output_vol;
	LONG sample_rate;
...
struct ui_eq_parameter2 {/* for 5658 */
	struct hw_eq_properties2 hweq[END_eq_source_5658];
	enum eq_channel_5658 eq_ui_channel_dac;
	enum eq_channel_5658 eq_ui_channel_adc;
	enum eq_source_5658 eq_ui_source;
	BOOL dac_r_as_l;
	BOOL adc_r_as_l;
};

/* for 5659 EQ */
struct eq_dat_format3 {
	DWORD dw_eq_tag;
	struct driver_eq_parameter eq_driver_parameter[END_eq_source_5658];
	struct ui_eq_parameter2 eq_ui_parameter;
};

struct driver_alc_parameter2 {
	WORD drc_ctrl_reg_num;
	struct compose_codec_register drc_ctrl_reg[DRC_REG_NUM2];

};

struct ui_alc_band_parameter3 {
	int limit_level_max1;
	int limit_level_max2;
	int limit_level_min;
	int attack_time;
	int recovery_time;
	int post_boost_gain_left;
	int post_boost_gain_right;
	int pre_boost_gain;
	int noise_gate_ena;
	int noise_gate_hold_gain_ena;
	int noise_gate_dropmode;
	int noise_gate_holdtime;
	int noise_gate_compensation_gain;
	int noise_gate_threhold;
	int noise_gate_threhold_ratio;
	int compression_func_ena;
	int compression_ratio1;
	int compression_ratio2;
	int alc_drc_agc_change_rate;

	int drc_thmin_fast_recover;
};

struct ui_alc_parameter3 {
	int alc_func_sel;
	int alc_band_sel;
	int alc_band_cutoff_freq;
	int alc_high_low_band_radio_sel;
	struct ui_alc_band_parameter3 alc_high_low_band[BAND_NUM];
};

/* for 5659 DRC */
struct drc_dat_format3 {
	DWORD dw_drc_tag;
	struct driver_alc_parameter2 alc_driver_parameter;
	struct ui_alc_parameter3 alc_ui_parameter;

};

struct eq_drc_dat_format {
	DWORD eq_switch;
	DWORD drc_switch;
	struct eq_dat_format3 eq;
	struct drc_dat_format3 drc;
};
/* ===============for DRC and EQ firmware end===============*/
...
struct realtek_extend_cell {
	struct realtek_extend_header header;
	struct realtek_extend_reg reg[0];
};

struct realtek_extend {
	uint16_t size;
	uint16_t count;
	struct realtek_extend_cell data[0];
};

struct rt5659_priv {
	/* Must be the 1st element of rt5659_priv. */
	...
	/* for DRC and EQ firmware */
	int eq_drc_num_mode;
	struct eq_drc_dat_format *eq_drc;

	/* For extend parameters */
	uint16_t extend_params_count;
	struct realtek_extend_cell *extend_params;
	uint16_t regs_num;
	struct realtek_extend_reg *default_settings;
	int extend_params_ready;

	int mode;
	bool eq_adc_en;
	bool eq_dac_en;
	int iis_width[RT5659_AIFS];/*0: 16bits; 1: 24bits*/
};
#endif /* __RT5659_H__ */
