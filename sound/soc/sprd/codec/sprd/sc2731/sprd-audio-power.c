/*
 * sound/soc/sprd/codec/sprd/sc2731/sprd-audio-power.c
 *
...
/* Todo: there is 2 bits in sc2731 while 1 bits in sc2723. */
static int sprd_audio_power_sleep_ctrl(void *data, int en)
{
	...
	case SPRD_AUDIO_POWER_HEADMICBIAS:
		reg = ANA_PMU0;
		mask = HMIC_SLEEP_EN_MASK << HMIC_SLEEP_EN;
		value = (en & HMIC_SLEEP_EN_MASK) << HMIC_SLEEP_EN;
...
SPRD_AUDIO_POWER_SIMPLE_LDO(BG_IBIAS, "VB", 4, ANA_PMU0,
		BIT(BG_IBIAS_EN), NULL, 0, 0);
SPRD_AUDIO_POWER_LDO(VCOM, "BG_IBIAS", 5, ANA_PMU0, BIT(VCM_EN), NULL,
		     ANA_PMU3, VB_V_MASK, VB_V, VB_VSEL_table, 0, 0);
SPRD_AUDIO_POWER_SIMPLE_LDO(VCOM_BUF, "VCOM", 6, ANA_PMU0, BIT(VCM_BUF_EN),
			    NULL, 0, 0);
SPRD_AUDIO_POWER_LDO(VHEADMICBIAS, "VCOM", 7, 0, 0, NULL, ANA_PMU5,
		     HMIC_BIAS_V_MASK, HMIC_BIAS_V, VMIC_VSEL_table, 0, 0);
SPRD_AUDIO_POWER_LDO(VMICBIAS, "VCOM", 8, 0, 0, NULL, ANA_PMU5,
		     MIC_BIAS_V_MASK, MIC_BIAS_V, VMIC_VSEL_table, 0, 0);
SPRD_AUDIO_POWER_SIMPLE_LDO(MICBIAS, "VMICBIAS", 9, ANA_PMU0,
			    BIT(MIC1_BIAS_EN), NULL, 0, 0);
SPRD_AUDIO_POWER_SIMPLE_LDO(AUXMICBIAS, "VMICBIAS", 10, ANA_PMU0,
			    BIT(MIC2_BIAS_EN), NULL, 0, 0);
SPRD_AUDIO_POWER_LDO(HEADMICBIAS, "VHEADMICBIAS", 11, ANA_PMU0,
		     BIT(HMIC_BIAS_EN), sprd_audio_power_sleep_ctrl,
		     ANA_HDT0, HEAD_SBUT_MASK, HEAD_SBUT, HIB_VSEL_table, 0,
		     0);
...
static int sprd_audio_power_config_reg_vars(void)
{
	...
	node = of_find_compatible_node(
		NULL, NULL, "sprd,sc2731-audio-codec");
	if (!node) {
...
static int sprd_audio_power_add_devices(void)
{
	...
	/* Check if sc2731 codec is available. */
	...
	if (!of_device_is_available(np)) {
		pr_info(" node 'sprd,sc2731-audio-codec' is disabled.\n");
		return 0;
	}
