...
typedef int (*sprd_vbc_mixerdg_mainpath_set) (int id, int mixerdg_main_l,
					      int mixerdg_main_r);
typedef int (*sprd_vbc_mixerdg_mixpath_set) (int id, int mixerdg_mix_l,
					     int mixerdg_mix_r);
...
typedef int (*sprd_vbc_src_set) (int id, int32_t fs);
struct vbc_src {
	int32_t src_fs[VBC_SRC_MAX - VBC_SRC_DAC0];
...
struct vbc_codec_priv {
	...
	unsigned short vbc_call_mute[VBC_CALL_MUTE_MAX-VBC_CALL_MUTE_START];
	unsigned short sys_iis_sel[SYS_IIS_MAX-SYS_IIS_START];
	unsigned short ag_iis_ext_sel[AG_IIS_MAX];
	unsigned int agdsp_access_en;
	unsigned int dsp_reg;
	unsigned short ap_access_agcp_en;
	struct pinctrl *pctrl;
	unsigned int aud_iis0_master_setting;
	unsigned int dac_iis_width[VBC_DA_MAX];
	unsigned int adc_iis_width[VBC_AD_MAX];
	unsigned int dac_iis_lr_mod[VBC_DA_MAX];
	unsigned int adc_iis_lr_mod[VBC_AD_MAX];
	...
	/*true: open_vbc_dump, false: close_vbc_dump*/
	int32_t open_vbc_dump_flag;
...
#define IS_SPRD_VBC_PROFILE_RANG(reg) ((reg) >= SND_VBC_PROFILE_START && \
				       (reg) < (SND_VBC_PROFILE_MAX))
#define SPRD_VBC_PROFILE_IDX(reg) ((reg) - SND_VBC_PROFILE_START)
#define IS_SPRD_VBC_MDG_RANG(reg) ((reg) >= VBC_MDG_START && \
				   (reg) < (VBC_MDG_MAX))
...
#define IS_SPRD_VBC_SMTHDG_RANG(reg) ((reg) >= VBC_SMTHDG_START && \
				      (reg) < (VBC_SMTHDG_MAX))
#define SPRD_VBC_MIXERDG_IDX(reg) ((reg) - VBC_MIXERDG_START)
#define IS_SPRD_VBC_MIXERDG_RANG(reg) ((reg) >= VBC_MIXERDG_START && \
				       (reg) < (VBC_MIXERDG_MAX))
#define IS_SPRD_VBC_MUX_RANG(reg) ((reg) >= VBC_MUX_START && \
				   (reg) < (VBC_MUX_MAX))
#define SPRD_VBC_MUX_IDX(reg) ((reg) - VBC_MUX_START)
#define IS_SPRD_VBC_ADDER_MUX_RANG(id) (((id) >= VBC_ADDER_START) && \
					((id) < VBC_ADDER_MAX))
