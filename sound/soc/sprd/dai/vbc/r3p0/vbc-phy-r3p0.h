...
#define VBC_AUD_CHNL_INT_SEL	(VBC_OFLD_ADDR + 0x004C)
#define VBC_AUD_DMA_EN		(VBC_OFLD_ADDR + 0x0050)
...
#define IS_AP_VBC_RANG(reg)		(((reg) >= VBC_OFLD_ADDR_START) &&\
					 ((reg) <= VBC_OFLD_ADDR_END))
#define AP_VBC_BASE_HI			(VBC_OFLD_ADDR & 0xFFFF0000)
#define IS_DSP_VBC_RANG(reg)		(((reg) >= VBC_MEM_ADDR_START) &&\
					 ((reg) <= VBC_CTL_ADDR_END))
...
/*VBC_AUD_DMA_EN*/
#define AUDRCD_DMA_AD1_EN		(3)
#define AUDRCD_DMA_AD0_EN		(2)
#define AUDPLY_DMA_DA1_EN		(1)
#define AUDPLY_DMA_DA0_EN		(0)

/* ap vbc register end */

#define VB_AUDPLY_FULL_WATERMARK	(160)
#define VB_AUDPLY_EMPTY_WATERMARK	(320)
#define VB_AUDPLY_FRAGMENT	(80)
#define VB_AUDRCD_FRAGMENT	(160)
...
int vbc_try_vbc_mix_left_mux_sel_set(struct snd_soc_codec *codec, uint32_t id);
int vbc_try_vbc_mix_right_mux_sel_set(struct snd_soc_codec *codec, uint32_t id);
int vbc_try_vbc_mix_left_out_sel_set(struct snd_soc_codec *codec, uint32_t id);
int vbc_try_vbc_mix_right_out_sel_set(struct snd_soc_codec *codec, uint32_t id);
...
int ap_reg_update(unsigned long reg, int val, int mask);
int ap_reg_write(unsigned long reg, int val);
unsigned int ap_reg_read(unsigned long reg);
uint32_t vbc_phy_ap2dsp(uint32_t addr);

int vbc_of_setup(struct platform_device *pdev);
int dsp_vbc_get_src_mode(unsigned int rate);

/* vbc driver function start */
int vbc_dsp_func_startup(struct device *dev, int id, int stream,
			 const char *name);
int vbc_dsp_func_shutdown(struct device *dev, int id, int stream,
			  const char *name);
...
int get_codec_digital_addr_virt_phy_vbc(unsigned long *virt_out,
